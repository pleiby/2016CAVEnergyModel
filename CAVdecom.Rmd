...
DemScenParams %>% gather(key = Sens, value = value, Low:Zero)  %>%  # following assigns the same 
  ...
  DemScenParams
DemScenParams = DemScenParams[,c( "VC","Parameter", "Zero", "Low", "Med", "High")] # reorder columns

kable(x = DemScenParams, digits = 4,
...
VTCost = select(VehTravelCostCompons, -c(LDVAlt, HDVOther, Units, Note)) %>%
...
ChkErrorDiff =
  VTCost %>% filter(CostCat=="Time") %>% select(VTCost_cpm)/100 -
...
cat("Check: Input VTCost Time cost for LDVs and HDV matches Average TTC per mile to within", paste(ChkErrorDiff),
  "cents/mi.\n")
...
kable(x = spread(VTCost, key = VehType, value = VTCost_cpm), 
      caption= "Base Vehicle Travel Cost Components by Vehicle Type (cents/mi)")


```

```{r readVTcostComponReduction, echo=echoWorking}
# Note: definition of automation scenarios is by impacts on component costs, not CAV penetration scenarios
