...
cat(
  "Check: Calculated Net Energy Intensity Changes match old workbook test values to within",
...
cat(
  "Check: Input VTCostBase Time cost for LDVs and HDV matches Average TTC per mile to within", paste(ChkErrorDiff),
  "cents/mi.\n"
)
```
...
cat(
  "Check: Calculated test output to Kaya (VMTIncrease table) matches examples for CombScen in",
...
CaseEffectsEI <- EnergyIntensityChanges %>%
  ungroup() %>%
...
scenData <- scenData %>%
  filter(VehType == VehTypeWanted) %>%
  spread(key = CostCat, value = VTCost_cpm)
scenData <- scenData %>%
  rename(
    C_F = Fuel, # fuel costs, c/mi
...
ElasPKT <- RelTotCost %>%
  filter(DemScen %in% DemScenWanted, VehType == VehTypeWanted, Case == "Low") %>%
  .$ElasVKT
```
