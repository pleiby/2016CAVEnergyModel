...
```
<!---
!!! NOTE THIS VARIANT ENERGY INTENSITY (FUEL COST) FOR TEST!!: LDV Energy Intensity Reduction is halved for TechScen 4 (for both LDVs and HDVs)
-->
...
cat("Check: Calculated Net Energy Intensity Changes match old workbook test values to within",
...
cat("Check: Calculated VoTT for LDV average value matches VMT-weighted average to within",
...
cat("Check: Calculated test output to Kaya (VMTIncrease table) matches examples for CombScen in",
...
GatherScenInputsForOneTechScen <- function(CurrTS, CurrYr){
  ...
  VMTIncrease = RelTotCost %>% 
    ...
    group_by(VC, DemScen, ElasCase) %>%
    summarise_at(
      vars(C_Fuel, C_Time, C_VNonF, C_Total, ElasVKT, fracVMTIncr),
      .funs = mean
...
CaseEffects <- CaseEffects %>% # reorder the columns to put the scenario selection indices first
  select(c(CombScen, DemScen, ElasCase, TechScen, VC, Year), everything())
```
...
# Note: these Demand results are specific to the Currently select TechScen, since that alters energy costs.
# Compute Energy Use impact from fractional intensity change and fractional VMT change
...
# use select from the dplyr package and its everything() function.
# In this example we are sending to the end:
CaseAssumps <- CaseAssumps %>% select(names(CaseEffects), everything())
```
...
(Note: so far, ran all combinations, and just choose ones wanted to report.)
...
CaseWantedEffects = CaseEffects %>% 
  ...
  semi_join(CombScen_Combinations, by=c("Year", "ElasCase", "VC", "TechScen", "DemScen"))
...
cat("Check: Calculated values match Scenario Results test values (intensity, VMT, EUse) to within total absolute error ",sum(abs(CWE_test$testDiff)), ".\n")
```
