...
CombScenResultsForOneTechScen <- function(CurrTS, CurrYr){
  ...
  EffectsD <- VMTIncrease %>% ungroup %>% 
    ...
    mutate(CombScen_Number = as.integer(str_sub(CombScen,-1,-1)))
...
CombScen :VC   :TechScen :DemScen :Notes
CS1      :HDV  :1        :2       :Cost-base response (with ElasVKT) only
CS2      :HDV  :2        :3       :Cost-base response (with ElasVKT) only
CS3      :HDV  :3        :5       :Cost-base response (with ElasVKT) only
CS4      :HDV  :4        :6       :Cost-base response (with ElasVKT) only
CS5      :HDV  :5        :2       :HDV Same as DemScen 4
CS6      :HDV  :6        :0       :HDV Same as DemScen 4
CS7      :HDV  :7        :2       :HDV Same as DemScen 3
CS1      :LDV  :1        :2       :Cost-base response (with ElasVKT) only
CS2      :LDV  :2        :3       :Cost-base response (with ElasVKT) with Underserved multiplier effect option 1
CS3      :LDV  :3        :5       :Cost-base response (with ElasVKT) with Underserved multiplier effect option 3
CS4      :LDV  :4        :6       :Cost-base response (with ElasVKT) with Underserved multiplier effect option 4
CS5      :LDV  :5        :2       :scenario 4 with higher LDV elasticity
CS6      :LDV  :6        :0       :scenario 5 with shift to ridehailing_ridepooling
CS7      :LDV  :7        :2       :scenario 3 with shift to ridehailing_ridepooling
"),header=TRUE,sep=":",strip.white=TRUE) #
```
