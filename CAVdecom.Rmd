...
We establish estimates of vehicle travel cost per mile by component category (those related to fuel cost, non-fuel financial costs, and time costs). Gather these for Base assumptions and Alternative assumptions.
...
scenData = scenData %>% select(DemScen, C_F, C_H, C_T, C_V)
```
...
# Keep only Base and the Alt scenario of interest
#scenData = scenData %>% filter(DemScen == "Base" | DemScen == "DS3")
scenData = scenData %>% filter(DemScen %in% DemScenWanted)
...
occupancyEffects = crossing(scenData, occupancyEffects, )
