...
# re-organize the row order to be consistent with prior displays
x = spread(VTCostBase, key = VehType, value = VTCost_cpm) %>% 
  arrange(factor(CostCat, levels = costComponOrder$CostCat))

kable(x = x, row.names = F, digits=3,
...
VTCShrBase = VTCostBase %>% spread(key = CostCat, value = VTCost_cpm) %>%
  ...
  mutate(Total = 1.0) %>%
  gather(key = CostCat, value=VTCost_cpm, -VehType)  # gather for pivot from cols as CostCat to cols as VehType

VTCShrBase = VTCShrBase %>%  
...
kable(VTCShrBase, row.names = F, digits=3,
...
kable(x = DemScenCostChange, digits=3,
...
x = left_join(x, select(DemScenCostChange, -Description), by=c("DemScen")) # appends Dem Scen multipliers
VTCShrAlt = x
```
