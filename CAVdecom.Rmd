...
mechScenDescriptors = mechScenAssumps %>% 
  select(Scenario_Number, Demand_Scenario, Scenario_Name, Scenario_Description)
...
*??? Specify VoTT assumptions for LDV and HDV by Scenario here?*

The total energy intensity multiplier for each scenario *j* is the product of all mechanism multipliers^[Here mode *m* and fuel *f* are initially fixed and suppressed, i.e. *m* = road_vehicle and *f* = petro.] 
...
The following are the default parameter values.  (Note: Need to be clear which version of certain parameters dominate in subsequent application, e.g. from "DemScenCostChange" vs "DemRespParams" tables. 
The "Low, Med, High" cases in DemdRespParams table are separate from the Demand Scenario cases in DemScenCostChange table (section 4.3 below).
...
InsurCostRed     : Reduction in insurance premiums (fraction) (superceded by DemScenCostChange)
ExclVehCapCost   : Exclude Veh Capital Cost (wear & ownership costs) (Choose 0 or 1),
C_deltaCAV       : Incr vehicle capital cost (fraction) from automation (wear and ownership cost/mi),
...
# Transcribe current net intensity changes (nie) to the I_deltaCAV variable in DemRespParams
...
cat("Average TTC per mile for HDV average value matches VoTT/speed to within",
  ...
  "$/mi.\n")
```
Note: travel time parameter values for LDVs are an average for Car and Light-truck vehicle types
...
x = VTCostBase %>% 
  filter(!CostCat %in% c("MilesDriven", "Source", "Total")) %>%  # may be redunant, these are already gone
  group_by(VehType) %>% 
...
Automation _demand_ scenarios are defined by assumptions regarding CAV impacts on selected component costs (e.g. travel time costs, internal accident and insurance costs, and vehicle costs), and on the sensitivity of road travel demand to costs. They are not CAV penetration scenarios. 
Penetration is exogenous, and initially assumed 100%.


```{r readDemScenCostChange, echo=echoWorking}

# Should we add VehCapCost shifts to this Demand Scenario Table?
#  If so, conflicts with C_deltaCAV elsewhere

#DemScenCostChange <- read.delim(textConnection("
#DemScen :C_deltaVoTT :C_deltaInsur :C_deltaCAV :Description
...
### Vehicle Travel Cost Shares by Component - CAV Scenario

Costs are adjusted relative to Base for cost and other assumptions of each Demand Scenario and for the Energy Intensity change associated with the current Technology Scenario. Energy intensity drives the energy cost, but this component is typically less than 15% of total costs for LDVs, and less than 35% of total costs for HDVs.

Base cost components $\sigma_{ivt}^B$ are normalized and add to 1.0. Demand Scenario *relative* costs for each cost component *i* are relative to the Base (Manual Vehicle) level. The total relative cost can be greater than or less than 1.0. Denoted $\sigma_{ivtdj}$, they depend on cost component *i*, Vehicle class *v*, Year *t* and Demand Scenario *d*, and TechScenario *j*.

- Fuel costs per mile are adjusted by the Scenario Multipliers for energy intensity, $\mu_{jtv}$ for TechScenario *j*, Year *t*, Vehicle class *v*. The perceived change in fuel cost effects for consumers/riders also depends on the fraction $f_{fuelvis,v})$ of fuel costs that are assumed visible to or perceived by them. That is, for cost component $i = fuel$:
$$\sigma_{ivtdj} = \sigma_{fuel,vt}^B \cdot (1+\mu_{jtv} \cdot f_{fuelvis,v})$$

Other cost components *i* have similar scenario-based adjustments:
$$\sigma_{fuel,vtdj} = \sigma_{ivt}^B \cdot (1+\gamma_{dtv})$$
...
x = VTCShrAlt %>% select(-VTCost_shr) %>% gather(key = DemScen, value = VCRel, DS1:DS4)
x = x %>% spread(key=CostCat, value = VCRel)

# Get desired Demand Response Parameters (ElasVKT, C_deltaCAV, ExclVehCapCost, and  I_deltaCAV), for desired cases and append to
...
Calculate cost component fractions for all demand scenarios. The Technology (energy intensity) scenario is held 
...
VTCShrAlt = VTCShrAlt %>% mutate(
  ...
  AccAndIns    = AccAndIns * (1 + C_deltaInsur), # (1+InsChange[DemScen]) # = "DemandScenarios!$O3"
  VehCapCost   = VehCapCost * (1.0 + C_deltaCAV),  # (1+CapitalCostChange)
  TollsFees    = TollsFees * 1.0,
  Parking      = Parking   * 1.0,
  Time         = Time      * (1 + C_deltaVoTT), #(1-VoTTChang[DemScen]) # = "DemandScenarios!N3" 
...
```{r CalcTravelCostComponentsAdjusted2, echo=echoWorking}
...
$$c_{rel,vtdj} = RelTotalCost_{vtdj} \equiv \frac {C_{tot,vtdj}} {C_{tot,vt}^B}$$


$$fractionalVKTIncr_{dvc} = (RelTotalCost_{vtdj})^{ElasVKT_{cv}}-1$$

Note: The Elasticity of VKT with respect to (generalized) travel cost is a key assumption. Does this include mode switching, and longer-run locational choices, etc. as well the costs considered here (travel-time, vehicle capital and operation costs, including fuel, insuarance, maintenance, etc.)?
Sources for ElasVKT: (HERS-ST technical report, August 2005 + Graham and Glaister 2002)


Note: Additional adjustments to the fractional change in VKT for LDV travel, beyond those from the elastic demand response to changing full generalized cost, follow from:
...
VMTIncrease = RelTotCost %>% filter(Case=="Low") %>%
  select(-VehType) %>%
  group_by(VC, DemScen) %>%
  summarise(fracVMTIncr = mean(fracVMTIncr)) # this is an unweighted mean for Sedans and LtTrucks
```

```{r testVMTIncreaseFromCostElas, echo=echoWorking, include=echoWorking}
...
LDV :VMTdemand  :lowElas            :0.159002099		 :0.186371875		 :0.538838618		 :0.888586295
LDV :VMTdemand  :highElas           :0.159002099		 :0.186371875		 :0.538838618		 :0.888586295
HDV :VMTdemand  :lowElas            :0.133159862		 :0.156008958		 :0.428904653		 :0.682936788
HDV :VMTdemand  :highElas           :0.294018792		 :0.348394295		 :1.087345456		 :1.924954964
"),header=TRUE,sep=":",strip.white=TRUE)
...
# Check: Compare calculated VMT Increase (from cost-elastic response) with Test Values
# VMTIncreaseTest 
x = VMTIncreaseTest %>% select(-varName) %>%
  gather(key = DemScen, value = fracVMTIncr, DS1:DS4) %>%
...
# Check that input tables (VTVCostBase and TravelTimeCostBasePars) are consistent with regard to time cost per mile,
#   for 2 LDV categories (Lt Truck and Car) and one HDV categopry
ChkErrorDiff = sum(abs(x$lowElas - x$fracVMTIncr))
cat("Check: Calculated VMTIncrease from cost-elastic response matches Test values to within", paste(ChkErrorDiff),
  "(fractional change).")

```

### Effects on VMT Demand from Underserved Population, and Car Sharing/Ride Pooling
...
rideSharingPoolingMult = 0.8 # multiplier is for total LDV VMT. This is the high-end estimate. (Mult for HDV=1.0)
...
# Danger! Following assumes a particular ordering of rows
...
#  bind_rows(select(UnderservedVMTmult, description), )
```

```{r checkVMTAdjustments, echo=echoWorking, include=echoWorking}
...
DS2      :HDV  :0.15600896  :0.15600896  :Cost-base response (with ElasVKT) with Underserved multiplier effect option 1
DS3      :HDV  :0.42890465  :0.42890465  :Cost-base response (with ElasVKT) with Underserved multiplier effect option 3
DS4      :HDV  :0.68293679  :0.68293679  :Cost-base response (with ElasVKT) with Underserved multiplier effect option 4
DS5      :HDV  :0.68293679  :0.68293679  :HDV Same as DemScen 4
DS6      :HDV  :0.68293679  :0.68293679  :HDV Same as DemScen 4
DS7      :HDV  :0.68293679  :0.68293679  :HDV Same as DemScen 4
DS1      :LDV  :0.15900210  :0.15900210  :Cost-base response (with ElasVKT) only
DS2      :LDV  :0.21410345  :0.21410345  :Cost-base response (with ElasVKT) only
DS3      :LDV  :0.64978123  :0.64978123  :Cost-base response (with ElasVKT) only
DS4      :LDV  :1.08895536  :1.08895536  :Cost-base response (with ElasVKT) only
DS5      :LDV  :1.08895536  :1.08895536  :scenario 4 with higher elasticity
DS6      :LDV  :0.67116428  :0.67116428  :scenario 5 with shift to ridesharing_ridepooling
DS7      :LDV  :0.31982499  :0.31982499  :scenario 3 with shift to ridesharing_ridepooling
"),header=TRUE,sep=":",strip.white=TRUE) #
...
FracVMTIncreaseExample = FracVMTIncreaseExample %>% select(DemScen, VC, y2050)  %>% 
  filter(DemScen %in% c("DS1","DS2","DS3")) %>%
  left_join(VMTIncrease, by=c("VC", "DemScen"))
...
VMT Demand Response: pick up chosen Demand and Technology Scenario, and value for chosen scenario, for years 2035 & 2050
