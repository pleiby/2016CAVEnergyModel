...
#DemScen :VoTT  :Insurance  :C_deltaCAV :Description
...
y = DemRespParams %>% select(-c(Zero, Med)) %>% 
  ...
  select(-InsurCostRed)  # UGLY: Insurance cost reduction determined by Demand Scenario
...
VTCShrAlt= VTCShrAlt %>% 
  ...
  gather(key = CostCat, value = value, AccAndIns:VehCapCost) %>% # reshape
  spread(key = DemScen, value = value)

VTCShrAlt = VTCShrAlt %>% gather(key=DemScen, value =value, c("DS1","DS2","DS3","DS4")) %>%
  select(-VC) %>% spread(key=VehType, value=value) %>%
  arrange(DemScen)
```

```{r dispVTCShrAlt, echo=echoWorking}
kable(VTCShrAlt, row.names = F, digits=3,
...
x =  VTCShrAlt %>% filter(CostCat != "Total") %>%
  gather(key = VehType, value=VTCost_shr, -c(CostCat, DemScen))

x %>% filter(DemScen == "DS1") %>%
  ggplot() + 
    geom_bar(aes(x=VehType, y=VTCost_shr, fill=CostCat), stat="identity") +
    ggtitle("Fig: Alt Cost Components for Vehicle Travel, DemScen=1", subtitle = "Automated Vehicle, Current Year, Relative to Base MV") +
    scale_fill_brewer(palette="Spectral") +
    # scale_fill_brewer(palette="Blues")
    scale_y_continuous(limits = c(0,1.0))

x %>% filter(DemScen == "DS2") %>%
  ggplot() + 
    geom_bar(aes(x=VehType, y=VTCost_shr, fill=CostCat), stat="identity") +
    ggtitle("Fig: Alt Cost Components for Vehicle Travel, DemScen=2", subtitle = "Automated Vehicle, Current Year, Relative to Base MV") +
    scale_fill_brewer(palette="Spectral") +
    # scale_fill_brewer(palette="Blues")
    scale_y_continuous(limits = c(0,1.0))

x %>% filter(DemScen == "DS3") %>%
  ggplot() + 
    geom_bar(aes(x=VehType, y=VTCost_shr, fill=CostCat), stat="identity") +
    ggtitle("Fig: Alt Cost Components for Vehicle Travel, DemScen=3", subtitle = "Automated Vehicle, Current Year, Relative to Base MV") +
    scale_fill_brewer(palette="Spectral") +
    # scale_fill_brewer(palette="Blues")
    scale_y_continuous(limits = c(0,1.0))

x %>% filter(DemScen == "DS4") %>%
  ggplot() + 
    geom_bar(aes(x=VehType, y=VTCost_shr, fill=CostCat), stat="identity") +
    ggtitle("Fig: Alt Cost Components for Vehicle Travel, DemScen=4", subtitle = "Automated Vehicle, Current Year, Relative to Base MV") +
    scale_fill_brewer(palette="Spectral") +
    # scale_fill_brewer(palette="Blues")
    scale_y_continuous(limits = c(0,1.0))
```
...
[??? is elasticity case to be governed by demand scenario?]
...
$$fractionalVKTIncr_{dvc} = (RelTotalCost_{vtdj})^{ElasVKT_{dv}}-1$$
...
Note: Additional adjustments to the fractional change in VKT for LDV travel follow from:
