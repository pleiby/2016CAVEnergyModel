...
The following are the default parameter values.  (Note: Need to be clear which version of certain parameters dominate in subsequent application, e.g. from "DemScenCostReduction" vs "DemRespParams" tables. 
The "Low, Med, High" cases in DemdRespParams table are separate from the Demand Scenario cases in DemScenCostReduction table (section 4.3 below).
...
```{r updateTravCostPerMil, echo=echoWorking}
...
VTCShrBase %>% filter(CostCat != "Total") %>%
  ...
  ggplot() + 
    ...
    ggtitle("Fig: Base Cost Shares for Vehicle Travel", subtitle = "Manual Vehicle, Current Year")
...
VTCShrAlt = VTCShrAlt %>% mutate(
  ...
  VehCapCost   = VehCapCost * (1.0 + C_incrCAV),  # (1+CapitalCostChange)
...
$$RelTotalCost_{vtdj} \equiv \frac {C_{tot,vtdj}} {C_{tot,vt}^B}$$
