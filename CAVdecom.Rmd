...
# 8. Simple Examination of VMT and PMT Given Pooling
...
```{r initOccupancyEffects}
occupancyEffects = data.frame("o" = seq(0.8, 1.5, 0.05))

```
We establish estimates of vehicle travel cost and its primary components (those related to fuel cost, non-fuel finacial costs, and time costs).

```{r poolingScenData}

scenData = VTCostBase
scenData$DemScen = "Base"
scenData = rbind(scenData, select(VTCostAlt, CostCat, VehType, VTCost_cpm, DemScen))
...
scenData = scenData %>% 
  rename(C_F = Fuel,C_T = Time) %>% 
  mutate(C_H = Total - C_F - C_T,  # combine all (financial) costs other than Fuel and Time. Includes accident
         C_V = C_F + C_H)
...
occupancyEffects = crossing(occupancyEffects, scenData)

VoTT_elas_wrt_o = 0.5  # an assumption regarding how occupancy (pooling) increases time cost VoTT
...
occupancyEffects = occupancyEffects %>%
  mutate(C_Vpmt = C_V/o,            # Variable vehicle travel cost per pass-mi [c/pass-mi]
         rho = o^VoTT_elas_wrt_o,   # Time-cost multiplier due to disutility of pooling [unitless]
...
ElasPMT = RelTotCost %>% filter(DemScen %in% DemScenWanted, VehType==VehTypeWanted, Case == "Low") %>%
  .$ElasVKT
```

```{r poolingEffectsOnVMTandPMT}
occupancyEffects = occupancyEffects %>%
  ungroup() %>%
  # relative PMT and VMT (unitless, ref = 1.0 for Base scen, occupancy=1)
  mutate(
    rPMT = (C_Mpmt/C_Mpmt[o==1.0 & DemScen=="Base"])^ElasPMT, 
    rVMT = rPMT/o
    ) %>%
  group_by(DemScen) %>%
  mutate(  # This is a direct effect of pooling on VMT with no demand response to pooling costs
    rVMT_direct = rPMT[o==1.0]/o # excludes the effect of pooling on travel costs (time and financial)
    ) %>% # but does include the effect of the Alt case on cost and PMT
  ungroup()
```

```{r plotOccupancyEffects}
TempoccupancyEffects = occupancyEffects %>% 
  select(DemScen, o, rPMT, rVMT, rVMT_direct) %>%
  gather(key= travelEffect, value = value, rPMT:rVMT_direct)
...
TempoccupancyEffects %>%
  ...
  ggplot( aes(x=o, y=value , color=travelEffect, linetype=DemScen)) + 
    ...
    xlab("Pooling Level (base=1.0)") + ylab("VMT and PMT Relative to Base")
