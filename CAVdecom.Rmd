...
# Case Effects on Demand 
CaseEffectsD <- VMTIncrease %>% ungroup %>% 
  select(CombScen, DemScen, VC, ElasCase, fracVMTIncr) %>% 
  mutate(Scenario_Number = as.integer(str_sub(CombScen,-1,-1)))

# This completes the calculation of VMT fractional for one TechScen and DemScen

# Join Energy Effects (by Year) and Demand Effects (by ElasCase) and compute Energy Use
#  by Year, VC, CombScen, DemScen, Scenario_Number, ElasCase 
# Note: this is actually only valid for CurrTechScen
CaseEffects <- left_join(CaseEffectsEI, y = CaseEffectsD, by = c("Scenario_Number", "VC")) %>%
  mutate(EnergyUse = (1+NIE)*(1+fracVMTIncr) - 1.0)
