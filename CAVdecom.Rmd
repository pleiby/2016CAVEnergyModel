...
echoWorking = TRUE # set to FALSE for an exported (non-working version)
...
* Demand response to Automation and Shared Mobility
    ...
    * Utility maximization subject to joint budget and time constraints
* Endogenous interactions
    * Speed (highway free-flow) endogenous to trade-off energy cost, time and safety costs
    * Congestion – endogenous to vehicle mix, VKT, automation (determines average achieved speed)
    * Travel demand - endogenous to costs, including energy, congestion and safety, as well as travel time utility (VTT)
    * Distinguish between VKT and PKE, given car-sharing and ride-pooling
        * Compact representation of ride pooling choice
* CAV fuel efficiency follows one of three models
    * CAFE is binding (AVs achieve same MPG as CAFE average, counting some off-cycle benefits)
    * Cost effective case - AVs may be more efficient than standard, _if_ further efficiency reductions are cost-effective from a private perspective
    * Technically feasible case
...
* How will travel demand change?
    * See EEMS017 study of travel demand for different VOTT reports 30-50% increase in energy, mostly from demand, depending on VOTT [ToDo: see what this implies for VKTDemandElas]
    * Travel patterns - details omitted
        * Impact: Trip chaining patterns
        * Impact: detailed drive cycle changes (power use over different segments of trip)
...
* What are the implications of shared-mobility for energy use and VKT?
    ...
    * Model though occupancy, incremental VKT from re-positioning, and implications to travelers' cost
...
This paper describes a new aggregated framework, building on established travel demand literature, that account for key features of vehicle automation, and estimates demand and energy use implications. Unlike prior known [aggregate] work, which relied largely on fixed-coefficient scenario analysis or accounting, it integrates technological and economic factors, including fuel, vehicle and other travel costs, and incorporates energy and travel behavior responses to economic incentives. This work abstracts from much detail offered by microsimulation models, but complements that evolving work, which models detailed travel and energy implications of specific AV technologies using agent/microsimulation of travel and traffic in real-world spatial and road network models (MATSIM, BEAM, POLARIS, others). Our framework also is novel in the integration of financial incentives, in the form of taxation or subsidy of transport energy or road use (following Small and Parry 2005, Leiby and Rubin 2017). It seeks to address the important issue of how to promote the advantageous use of CAV technologies while deterring potential adverse outcomes with large unaccounted social costs.
...
Transportation GHG emissions are then the sum over all transportation modes, vehicle types, and fuels of {Transportation Services^[We differentiate between transportation services (PKT) and vehicle travel (VKT).]} x {Shares of each mode-vehicle-fuel type} x {Energy Intensity} x {Fuel GHG Intensity}.

$$G_t = \Sigma_{m,v} A_t ~ \sigma_{tmv} ~I(\vec x_{tmv}) ~\Sigma_f g_{tf} \phi_{tmvf}$$

Energy use for particular vehicle type is given by total activity (travel demand) level $A_t$, 
the share of travel on mode *m* and the ??? [index mismatch]
$$E_{tvf} = A_t ~ \sigma_{tmv} ~I(\vec x_{tmv}) \phi_{tmvf}$$
...
^[1 Additional potential disaggretations include by region, vehicle size class, drivetrain type, and time of day (e.g. operating costs vary by time of day, as described in Bösch et al. 2017).]

The variables are

- $A_{tv}$ = the transportation activity services provided by type (passenger v.s freight, or LDV/HDV) [passenger-km or tonne-km traveled/yr]
- $o_{tv}$ = occupancy of each vehicle type v in year t [pass/veh, or PKT/VKT, for passenger travel]
- $\sigma_{tmv}$ = share of energy services in transportation mode m by vehicle type v in year t [unitless]
- $\phi_{tmvf}$ = the share of energy-service share $s_{tvm}$ produced by fuel type f [unitless]
- $I_{tvmf}$ = the energy intensity of vehicle v in mode m using fuel type f in year t [MJ/veh-km]
- $g_{tf}$ = the GHG intensity of fuel f in year t [g CO2e/MJ (MegaT CO2e/EJ)]
- $E_{tvf}$ = energy use in form of fuel f by vehicle type v in year t [MJ/y]
...
$$[MJ/yr] = [pkm/yr] \cdot [vkm/pkm] \cdot [unitlessShare] \cdot [unitlessShare] \cdot [MJ/vkm]$$

GHG Emissions
$$G_{tv} = \Sigma_f E_{tvf} g_{tf}$$
$$[gCO2e/yr] = [pkm/yr] \cdot [vkm/pkm] \cdot [unitlessShare] \cdot [unitlessShare] \cdot [MJ/vkm] \cdot [g/MJ] = [MJ/yr] \cdot [g/MJ]$$
...
---------------------------
We identified a set *K* of "Mechanisms" or "Technologies," each $k \in K$ of which corresponds to a multiplier $\mu_k$ that can increase or decrease a component term in the ASIF decomposition. A mechanism can affect Activities *A*, Intensities *I*, or mode and fuel shares $\sigma, \phi$. 
In theory, a mechanism could also effect vehicle class shares or occupancy, but we do not yet consider such effects. 

In the Scenario Approach, we can construct scenarios *s* that combine technology cases $j \in J$ and demand cases $d \in D$. Multiplier values are indexed by year *t* (for selected years), vehicle class *v*, mechanism *k* and by the Technology and Demand scenario *s*, i.e. $\mu_{tvks}$.

``` {r defineIndexSets, echo=echoWorking}
SetK_Mechs = c(   # mechanisms by which automation alters energy and/or demand
  "Platooning",
...
SetV_Class = c("LDV", "HDV")  # Vehicle Classes
SetT_Years = c(2035, 2050)
SetJ_TechScen = 1:8
SetD_DmndScen = 1:7
SetS_Sense = c(
  "Opt",
...
The estimation for the midcase energy intensity impacts of each mechanism is based on literature review^[Wadud, MacKenzie and Leiby 2016.] and the external calculations of the authors. Ranges of values, for sensitivity cases and scenarios, are constructed for each mechanism *k*, for sensitivity cases *s* from "zero" through "pessimistic," "midcase," and "optimistic." Mechanism intensity effect values $I_{E,tvks}$ are differentiated by year *t* and Vehicle class *v*.
...
LDV  2035   Increased_feature_content     0.0     5.0    10.0
...
LDV  2050   Increased_feature_content     0.0     5.0    10.0
...
HDV  2035   Increased_feature_content     0.0     0.0     0.0
...
HDV  2050   Increased_feature_content     0.0     0.0     0.0
...
#### Construct multiplicative factors to apply to energy intensities

Multipliers $\mu_{tvks}$ apply for each year *t*, vehicle class *v*, mechanism/technology *k*, sensitivity case *s*.

$\mu_{tvks} = 1.0 + I_{E,tvks}$

A multiplier value of 1.0 indicates no change and less than one implies a multiplicative reduction in energy intensity.

```{r constrIEffects, echo=echoWorking}
...
* Note "Right_sizing" and "Increased_features" apply only to LDV passenger travel.

3.2 Energy Intensity by Vehicle Type and Scenario
-----------------

We construct Scenario Multipliers for energy intensity, $\mu_{jtv}$ for Scenario *j*, Year *t*, Vehicle class *v*.
For each scenario *j*, year *t*, vehicle class *v* and mechanism *k*, the "EffectCase" or sensitivity *s* is specified, i.e. 

$s_{jtmv}$ for 
$s \in \{Zero, Low, Mid, High\}$.

This determines the appropriate intensity multiplier for each mechanism, and the total energy intensity multiplier for each scenario is the product a all mechanism multipliers
$\mu_{jtv} = \prod_{k} \mu_{tvk{s_{jtmv}}}$
...
# Mechanism Impacts by Mechanism, EffectCase-level, Year (2035, 2050), and vehicle type (LDV, HDV) are in ='[CAV Energy ASIF Framework WadudMacKenzieLeiby V20161024Test20170709.xlsx]Energy Intensity Impacts'!$A$1:$Q$26
...
# cat("Constructed Net Energy Intensity Changes match old workbook to within",
#    NEIC_test_err, "total absolute error.\n")
```

Note: Constructed Net Energy Intensity Changes match old workbook to within `r NEIC_test_err` total error.
...
DemScenParamNotes <- read.delim(textConnection("
DemParam         : DemParamNote
ElasVKT          : Elas of travel w.r.t generalized cost,
InsurCostRed     : Reduction in insurance premiums (fraction),
ExclWearCost     : Exclude Wear & Ownership Costs (Choose 0 or 1),
C_incrCAV        : Incr vehicle capital cost from automation (wear and ownership cost/mi),
ShrECostInternal : Share of energy effic cost gains visible to driver (fraction, 0 to 1),
I_deltaCAV       : Fuel energy cost reduction (from effic, fraction)
"),header=TRUE,sep=":",strip.white=TRUE)

DemScenParams<-read.delim(textConnection("
                      Low     Med     High
LDV.ElasVKT           -1      -1      -1
LDV.InsurCostRed      0.6     0.7     0.8
LDV.ExclWearCost      0       0       0
LDV.C_incrCAV         0       0       0
LDV.ShrECostInternal  1       1       1
LDV.I_deltaCAV        -.7186  -.7186  -.7186
HDV.ElasVKT           -0.5    -0.97   -2
HDV.InsurCostRed      0.4     0.6     0.8
HDV.ExclWearCost      0       0       0
HDV.C_incrCAV         0       0       0  
HDV.ShrECostInternal  1       1       1
HDV.I_deltaCAV        -.2815  -.2815  -.2815
"),header=TRUE,sep="",strip.white=TRUE) #;
# Corresponds to data in "Demand Response Key parameters"
...
#   LDV InsurCostRed    Reduction in insurance premiums . internet/celent report
#   LDV C_incrCAV       Wadud 2017
#   HDV ElasVKT         Cambridge Sytematics, 2009

DemScenParams$Zero = 0.0   # establish zero effect case (zero reductions, zero elas)
DemScenParams["LDV.ShrECostInternal", "Zero"] = 1.0  # kludge to fix this parameter to non-zero val
DemScenParams["HDV.ShrECostInternal", "Zero"] = 1.0

DemScenParams$case = row.names(DemScenParams)
DemScenParams = separate(data=DemScenParams, col=case, into=c("VC","Parameter"), sep="\\.")
DemScenParamsm = DemScenParams %>% gather(key=EffectCase, value="value", Low:Zero)
```

### Selects a Single Scenario to Examine
...
Energy Intensity change is dependent on the Technology Scenario. 

```{r reportDemScenParams, echo=echoWorking}
EnergyIntensityChanges %>% filter(Scenario_Number == CurrTechScen, Year == CurrYear)
#DemScenParams = EnergyIntensityChange
DemScenParams %>% filter(VC=="LDV", Parameter=="I_deltaCAV")
# ??? Trick is how to do this assignment
DemScenParams %>% filter(Parameter=="I_deltaCAV") %>% select(Low:High) = c(2,1)

kable(x = DemScenParams, 
      caption= "DemScenParams: Important Parameters for the Calculation of Demand by Scenario, Vehicle-type, and Year")
```

4.2 Fractional Increase in VKT for Demand Scenarios
---------------------------------

Calculate fractional change in demand, by DemandScenario *d*, Year *t*, and VehicleClass *v*, based on changes in 
time cost, other vehicle fuel and operating costs, and VMT demand elasticity w.r.t generalized cost.

### Simple Travel Time Cost Calculation
...
kable(x = TravTimeCostBasePars, 
      caption= "Travel Time Cost Base Parameters")

str(TravTimeCostBasePars)
row.names(TravTimeCostBasePars) = TravTimeCostBasePars$var
# Test computed vs. specified values in table
TravTimeCostBasePars["VoTT","average"] - 
  (TravTimeCostBasePars["VMTtot","local"]*TravTimeCostBasePars["VoTT","local"] +
  TravTimeCostBasePars["VMTtot","intercity"]*TravTimeCostBasePars["VoTT","intercity"])/
  (TravTimeCostBasePars["VMTtot","local"]+TravTimeCostBasePars["VMTtot","intercity"])

TravTimeCostBasePars["average_TTC_per_mi","average"] -   #  [$/hr]/[mi/hr]
  TravTimeCostBasePars["VoTT","average"]/TravTimeCostBasePars["average_speed","average"]
```

### Vehicle Travel Cost Components

Specify the primary (private) cost components for (conventional, manual) road vehicle travel.

```{r dataVehCostCompons, echo=echoWorking}
VehTravelCostCompons <- read.delim(textConnection("
CostCat               :temp         :LDVAvgSedan :LDVAvgLtTruck  :HDVOther  :HDVClass8  :Units   :Note
...
AccidentAndInsurance  :7.0          :8.447257384 :8.490909091    :NA        :6.7        :[c/mi]  :Insurance will change for CAVs
WearAndOwnership      :12 7         :29.907173   :43.49090909    :NA        :18.9       :[c/mi]  :
...
Units                 :c/mile       :c/mile      :c/mile         :c/mile    :c/mile     :NA      :
...
kable(x = select(VehTravelCostCompons, -c(temp, HDVOther, Note)), 
      caption= "Vehicle Travel Cost Components by Vehicle Type")

VTCost = select(VehTravelCostCompons, -c(temp, HDVOther, Units, Note)) %>%
  filter(!CostCat %in% c("MilesDriven", "Total", "Units", "Source")) %>%
  gather(key = VehType, value = VTCost_cpm, -CostCat) %>%
  mutate(VTCost_cpm = as.numeric(VTCost_cpm))

# sum across categories (excl miles and total) to check total cost by VehType
VTCost %>% filter(!CostCat %in% c("MilesDriven", "Total")) %>%
  group_by(VehType) %>% 
  summarize(TotCost_cpm = sum(VTCost_cpm, na.rm = T)) # treat NA cost compnents as zero

# Note: definition of automation scenarios by impacts on component costs, not CAV penetration scenarios
# Penetration is assumed 100%
VTcostComponReduction <- read.delim(textConnection("
DemandScenario :VoTT  :Insurance  :Description
...
2              :-0.05 :-0.60      :Driver assistance, but no self-driving, so some benefits of comfort (5% reduction in VoT), lower end of insurance benefits (60%)
3              :-0.50 :-0.80      :Self driving, so large benefits of comfort + in vehicle use of time (50% reduction in VoT), large benefits of insurance (80%)
4              :-0.80 :-0.80      :Extreme case- Self driving, so large benefits of comfort + in vehicle use of time (80% reduction in VoT), large benefits of insurance (80%)
"),header=TRUE,sep=":",strip.white=TRUE)
...
rownames(VTcostComponReduction) = VTcostComponReduction$DemandScenario

# XXX Vehicle Travel Cost Components
```
...
InsReduc :Reduction in insurance premiums                        :0.6   :0.7  :0.8   :internet/celent report
ExclWear :Exclude Wear and Ownership Costs (1 True, 0 False)     :0     :0    :0     :Choose 0 or 1 value
IncrVCst :Incremental veh capital cost from automation           :      :     :      :???  (wear and ownership cost/mi)     
CFuelVis :Share of energy efficiency costs 'visible' to driver   :1     :1    :1     :0 to 1 (1 means full efficiency gain visible) (driver=person making trip choices)
CFuelRed :Fuel energy cost reduction (from energy efficiency)    :0.3420:     :      :Import from 'Policy+Scenario' subsheet calculation for currently chosen technology scenario
"),header=TRUE,sep=":",strip.white=TRUE) #
...
# Note: duplicative: DemandRepsonseKeyParameters duplicates DemScenParams for LDV, but adds "Fuel Energy cost reduction"
rownames(DemandResponseKeyParameters) = DemandResponseKeyParameters$Name

# PL note: ElasVKTwrtCost is key assumption. Does this include mode switching, vehicle efficiency, locational choices, etc?
# Sources for ElasVKT: (HERS-ST technical report, August 2005 + Graham and Glaister 2002)

# XXX

#% increase in vehicle travel              :lowElasticity     :0%     :-1%    :2%     :1%  :30%  :22%  :57%  :41%
#% increase in vehicle travel              :highElasticity    :0%     :-1%    :2%     :1%  :30%  :22%  :57%  :41%
#% increase LD personal vehicles VMTdemand :lowElasticity     :-0.55% :1.46%  :26.23% :48.88%
#% increase LD personal vehicles VMTdemand :highElasticity    :-0.55% :1.46%  :26.23% :48.88%

# VMT (Vehicle Travel Increase Due to vehicle automation
VMTIncrease <- read.delim(textConnection("
varName             :ElasCase  :AvgSedan_1   :LtTruck_1     :AvgSedan_2  :LtTruck_2    :AvgSedan_3   :LtTruck_3   :AvgSedan_4   :LtTruck_4
increaseInVehTravel :lowElas   :0.000679779  :-0.011658058  :0.02279916  :0.006459284  :0.301069905  :0.22355823  :0.565120488  :0.412413344
increaseInVehTravel :highElas  :0.000679779  :-0.011658058  :0.02279916  :0.006459284  :0.301069905  :0.22355823  :0.565120488  :0.412413344 
"),header=TRUE,sep=":",strip.white=TRUE) #
...
# ??? pctVMTIncrFromAutomation[case, vehtype, elasCase] = =G28^$I$5-1
...
# ??? fracVMTincr["LDV","2050"] = =1+H32-1
```

Calculate Travel Cost Components Adjusted for Scenario Conditions
---------------------------
...
# adjustements to each cost category in VehTravCostCompon
# XXX VehTravCostMult[C_Categ, VSubClass] = XXX
C_Categ   = c("Fuel", "Maint", "AccAndIns", "WearOwn", "TollsFees", "Parking", "Time", "Registration")
VSubClass = c("LDVAvgSedan", "LDVAveLtTruck", "HDVClass8", "HDVOther")
Fuel = 1 + DemandResponseKeyParameters["CFuelVis","Low"]*
  DemandResponseKeyParameters["CFuelRed","Low"]  # Share of energy efficiency costs 'visible' to the 'driver' (person making trip choices)"*"Fuel energy cost reduction (from energy efficiency)"
# "Fuel energy cost reduction" = 'Policy+Scenario'!D$11
Maint = 1.0
AccAndIns = 1 - DemandResponseKeyParameters["InsReduc","Low"] # (1-InsReduction[DemScen]) # = "DemandScenarios!$O3"
AccAndIns = 1 - VTcostComponReduction["4", "Insurance"]
WearOwn   = 1.0
TollsFees = 1.0
Parking   = 1.0
Time      = 1 - VTcostComponReduction["4", "VoTT"] # DemandResponseKeyParameters["VoTCost_mult","Low"] # (1-VoTTReduction[DemScen]) # = "DemandScenarios!N3" 
Registration = 1.0
# Total = CostMult weighted by base CostFraction

# Chang in VMT due to vehicle automation: for LDVCar, LDVLtTrk, HDVClass8
# Pct increase in vehicle travel: low elasticity
# Pct increase in vehicle travel: high elasticity
# Pct increase LD personal vehicles VMTdemand: low & high elasticity; arith ave of LDVCar and LDVLtTrk
# Pct increase LD personal vehicles VMTdemand: high elasticity
```
...
# Scenario Summary 2050 Energy and VMT Changes at Full Adoption
ScenarioNum :LDVEnergyIntensity :LDVVMTperveh :RoadTranEnergyUse :HDVEnergyIntensity :HDVVMTperveh :HDVEnergyUse :LDVEnergyUse :ScenarioName
...
kable(x = pmd, 
      caption= "Percent MPG Decreases With Speed (Rows are Ref Speed)")
```
...
-------------------
* Notes from [Speed and Accident Risk](http://ec.europa.eu/transport/wcm/road_safety/erso/knowledge/Content/20_speed/speed_and_accident_risk.htm), European Road Safety Observatory
...
* European Road Safety Observatory 20XX [Speed and Accident Risk](http://ec.europa.eu/transport/wcm/road_safety/erso/knowledge/Content/20_speed/speed_and_accident_risk.htm).
