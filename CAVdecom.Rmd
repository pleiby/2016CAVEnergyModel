...
#   mutate(fracVMTIncr = as.numeric(fracVMTIncr)) %>%
  ...
  left_join(VMTIncrease, by= c("VC", "DemScen", "ElasCase"))
...
LDV     :CS5     :DS4     :4       :0           : scenario 4 with higher elasticity
LDV     :CS6     :DS4     :4       :1           : scenario 5 with shift to carsharing (ride hailing/pooling)
LDV     :CS7     :DS3     :3       :1           : scenario 3 with shift to carsharing (ride hailing/pooling)
HDV     :CS1     :DS1     :0       :0           :
HDV     :CS2     :DS2     :0       :0           :
HDV     :CS3     :DS3     :0       :0           :
HDV     :CS4     :DS4     :0       :0           :
...
CS2      :HDV  :0.15600896  :0.15600896  :Cost-base response (with ElasVKT) with Underserved multiplier effect option 1
CS3      :HDV  :0.42890465  :0.42890465  :Cost-base response (with ElasVKT) with Underserved multiplier effect option 3
CS4      :HDV  :0.68293679  :0.68293679  :Cost-base response (with ElasVKT) with Underserved multiplier effect option 4
...
CS2      :LDV  :0.21410345  :0.21410345  :Cost-base response (with ElasVKT) only
CS3      :LDV  :0.64978123  :0.64978123  :Cost-base response (with ElasVKT) only
CS4      :LDV  :1.08895536  :1.08895536  :Cost-base response (with ElasVKT) only
...
Energy intensity changes (net, accounting for the influence of all identified mechanisms) in `EnergyIntensityChanges` 
have been calculated by year *t*, vehicle class *v*, and technology scenario scenario *j*:
$$\Delta I_{tkv} = 1 - \mu_{jtv}$$
Travel demand or VMT fractional changes in `VMTIncrease` have been calculated by year *t*, vehicle class *v*, and technology scenario scenario *j*
$$m_{vtdj}(c_{rel,vtdj}) ~=~fractionalVKTIncr_{dvc} ~=~ (RelTotalCost_{vtdj})^{ElasVKT_{cv}}-1$$

```{r}

CaseEffects <- EnergyIntensityChanges %>% ungroup %>% 
  select(Scenario_Number, Year, VC, NIE)
...
# The VMT results need to be recalulated here for each TechScen and matching DmndScen assumptions
#   in the full combined scenario.

CaseEffectsB <- VMTIncrease %>% ungroup %>% 
  select(CombScen, DemScen, VC, ElasCase, fracVMTIncr) %>% 
  mutate(Scenario_Number = as.integer(str_sub(CombScen,-1,-1)))

CaseEffects <- left_join(CaseEffects, y = CaseEffectsB, by = c("Scenario_Number", "VC"))
...
```{r singleCombScenarioSelect, echo=echoWorking}
CurrTechScen = 4
CurrYear     = 2050
