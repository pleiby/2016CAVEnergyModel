...
- $A_{tv}$ = the transportation services Activity provided by type (passenger v.s freight, or LDV/HDV) [passenger-km or tonne-km traveled/yr]
...
In the Scenario Approach, we can construct scenarios *s* that combine technology cases $j \in J$ and demand cases $d \in D$. Multiplier values are indexed by year *t* (for selected years), vehicle class *v*, mechanism *k* and by the Technology/Demand scenario *s*, i.e. $\mu_{tvks}$.
...
We construct Scenario Multipliers for energy intensity, $\mu_{jtv}$, for Scenario *j*, Year *t*, Vehicle class *v* based on the combined the effect of all the mechanisms.

For each technology scenario *j*, year *t*, vehicle class *v* and mechanism *k*, the "EffectCase" or sensitivity case *s* is specified, i.e. 
...
$\Delta I_{tvs} \equiv \mu_{jtv}-1 ~=~ \prod_{k} \mu_{tvk{s_{jtmv}}}-1$

```{r constructIntensityByScenario2, echo=echoWorking, warning=FALSE, message=FALSE}
...
Note/Aside: Constructed Net Energy Intensity Changes match old workbook to within `r NEIC_test_err` total error.
...
TranspEnergyUse_AVScenario = 
   TranspEnergyUseRef*((1+ModalEfficiencyFactors)^ReboundElas)*(1+FuelCarbonIntensities)*(1+Modal Shares/Activity Factors)
