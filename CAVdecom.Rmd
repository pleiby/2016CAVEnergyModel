...
Energy Intensity change is dependent on the Technology Scenario, and year of interest. For example consider Scenario `r CurrTechScen`, the "`r mechScenAssumps[[CurrTechScen,"Scenario_Name"]]`" scenario, in year `r CurrYear`. 

Demand response is a function of full (generalized) travel cost, and energy intensity affects the energy component of travel cost. So the full scenario requires updating Demand Response parameters with energy intensity reduction (by vehicle class and year) for this Technology Scenario and year
...
x = VTCostBase %>% 
  filter(!CostCat %in% c("MilesDriven", "Total"))
x = x %>%  # may be redundant, these are already gone
  group_by(VehType) %>% 
  bind_rows(summarise_all(., funs(if(is.numeric(.)) sum(., na.rm = T) else "Total")))

x = VTCostBase %>% 
  filter(!CostCat %in% c("MilesDriven", "Total")) %>%  # may be redunant, these are already gone
  group_by(VehType) %>% 
...
costCompOrder = VehTravelCostCompons %>% 
  filter(CostCat!="MilesDriven" & CostCat!="Source") %>% 
  select(CostCat) %>%
...
# xxx
# x = spread(x, key = VehType, value = VTCost_cpm)
# x = arrange(x, factor(CostCat, levels = costCompOrder$CostCat))
