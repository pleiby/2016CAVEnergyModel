...
abstract: A compact and aggregated model of road vehicle travel and energy use is
  ...
  and activity. Travel demand and vehicle use derives from a standard utility-theoretic
...
```
!!! NOTE THIS VARIANT ENERGY INTENSITY (FUEL COST) FOR TEST!!: Energy Intensity Reduction is halved for TechScen 4 (for both LDVs and HDVs)

```{r fixVariantTechCaseIntensity, echo=echoWorking}
# !!! NOTE THIS VARIANT ENERGY INTENSITY (FUEL COST) FOR TEST!!: Energy Intensity Reduction is halved
EnergyIntensityChanges = EnergyIntensityChanges %>%
  mutate( # only alter the TechScen==2 energy intensity (doubling
#    NIE = ifelse((TechScen==2) & (VC=="LDV"), (1+NIE)*2.44-1, NIE)
...
# ggplot(data, aes(x=CombScen, calcValue, fill=condition, y=value, x=specie)) + 
...
rbind(maxvals, minvals) %>%
#%>%
  gather(key= costcat, value=value, -c(VehType, variable)) %>%
  spread(key=variable, value=value) %>%
  filter(maxval > minval) %>%
  kable(caption="Table: Parameter that vary across current Demand Scenarios 'DemScen'")  
...
DemandCaseWanted = "Low"

VTCShrAlt= VTCShrAlt %>% 
  filter(Case==DemandCaseWanted) %>% # to this point the Low and High are the same (they only differ in the applied ElasVKT)
...
plotStackedBarShares <- function(df, ds, ts="/TS4") {
  ...
  ggplot() + 
    geom_bar(aes(x=VehType, y=VTCost_shr, fill=CostCat), stat="identity") +
    ggtitle(paste0("Fig: Alt Cost Components for Vehicle Travel, Scen=",ds,ts), subtitle = "Automated Vehicle, Current Year, Relative to Base MV") +
    ylab("Share") +
    scale_fill_brewer(palette="Spectral") +
    # scale_fill_brewer(palette="Blues")
...
```{r gatherAndReportCombScenInputs}
GatherScenInputsForOneTechScen <- function(CurrTS, CurrYr){
  ...
  # note this summing operation relies on component cost share columns being in this alphabetic order, and Total = 0
  VTCShrAlt %>% select(AccAndIns:VehCapCost) %>% rowSums(na.rm=TRUE) -> VTCShrAlt$Total
  VTCShrAlt = cbind(VTCShrAlt, VTCShrAlt$Total)
  # RelTotCost
  # Get main result (total relative cost) for all Demand scenarios, VehType, Cases, and elasticity of VKT
  # RelTotCost = VTCShrAlt %>% select(DemScen, VehType, VC, Case, Total, ElasVKT)
  RelTotCost = VTCShrAlt # keep all data
  ...
  VMTIncrease = RelTotCost %>% 
    ...
    rename(ElasCase = Case) %>%     # actually, this Case could reflect sensitivity Case variations in other cost compons
    group_by(VC, DemScen, ElasCase) %>%
    summarise_at(vars(Fuel, Time, Total, ElasVKT, fracVMTIncr), .funs = mean
              ) # this is an unweighted mean for Sedans and LtTrucks
  ...
  # Case Effects on Demand (single Year, TechScen, multiple VC, DemScen, ElasCase)
  AssumpsD <- VMTIncrease %>% ungroup %>% 
    # select(CombScen, DemScen, VC, ElasCase, fracVMTIncr) %>% 
    mutate(CombScen_Num = as.integer(str_sub(CombScen,-1,-1))) %>%
    mutate(DemScen_Num  = as.integer(str_sub(DemScen, -1,-1)))
  # add labels for year and TechScen, and associated Energy Intensity NIE
  AssumpsD <- left_join(x = AssumpsD, 
                        filter(EnergyIntensityCompons, TechScen == CurrTS, Year == CurrYr),
                        by = c("VC"))

} # GatherScenInputsForOneTechScen
```
...
for (CurrTechScen in 1:7) {
  ...
  #CaseAssumps = rbind(CaseAssumps, GatherScenInputsForOneTechScen(CurrTechScen, CurrYear))
}

```

```{r multCombScenarioAssumptionsLoop, echo=echoWorking}
CurrYear     = 2050
# CurrElasCase = "Low"
# CurrTechScen = 1
# CurrDemScen = "DS1"
...
CaseAssumps = data.frame() # simplest empty dataframe
CaseAssumps = rbind(CaseAssumps, GatherScenInputsForOneTechScen(CurrTechScen, CurrYear))

for (CurrTechScen in 1:7) {
  CaseAssumps = rbind(CaseAssumps, GatherScenInputsForOneTechScen(CurrTechScen, CurrYear))
}
CaseAssumps <- CaseAssumps %>% mutate(EnergyUse = (1+NIE)*(1+fracVMTIncr) - 1.0)
```
Calculate fractional change in Energy Use by sector, for all cases.
...
CS1      :2050  :Low      :HDV  :1            :2           :Cost-based response (with ElasVKT) only
CS2      :2050  :Low      :HDV  :2            :3           :Cost-based response (with ElasVKT) only
CS3      :2050  :Low      :HDV  :3            :5           :Cost-based response (with ElasVKT) only
CS4      :2050  :Low      :HDV  :4            :6           :Cost-based response (with ElasVKT) only
CS5      :2050  :Low      :HDV  :5            :2           :HDV dmnd mult Same as Scen 4
CS6      :2050  :Low      :HDV  :6            :0           :HDV dmnd mult Same as Scen 4
CS7      :2050  :Low      :HDV  :7            :2           :HDV dmnd mult Same as Scen 3
CS1      :2050  :Low      :LDV  :1            :2           :Cost-based response (with ElasVKT) only
CS2      :2050  :Low      :LDV  :2            :3           :Cost-based response (with ElasVKT) with Underserved mult option 1
CS3      :2050  :Low      :LDV  :3            :5           :Cost-based response (with ElasVKT) with Underserved mult option 3
CS4      :2050  :Low      :LDV  :4            :6           :Cost-based response (with ElasVKT) with Underserved mult option 4
CS5      :2050  :Low      :LDV  :5            :2           :Scen 4 dmnd mult with higher LDV elasticity
CS6      :2050  :Low      :LDV  :6            :0           :Scen 5 dmnd mult with shift to ridehailing_ridepooling
CS7      :2050  :Low      :LDV  :7            :2           :Scen 3 dmnd mult with shift to ridehailing_ridepooling
"),header=TRUE,sep=":",strip.white=TRUE) #
...
```{r gatherTestValueForScenarios, echo=echoWorking}
...
```{r testCalcVsRefScenarios, echo=echoWorking}
...
kable(summaryScenarioResultsTableExample, 
      caption="Summary Scenario Results from Example in Worksheet", digits=4)
```
...
data %>% filter(VC == "LDV") %>%
  ...
  ggplot(
    ...
    facet_wrap(~TechScen) +
    scale_fill_brewer(palette="Spectral") +
    ggtitle("Fig: Energy-Intensity, VMT, and Energy Scenarios", subtitle = "LDV CAVs, Excludes Pooling/Sharing") +
    ylab("Fractional Change")
    # scale_fill_brewer(palette="Blues")
...
```{r examineCaseCombinations, echo=echoWorking}
...
Consider a range of vehicle occupancy levels *o*, where $o = 1.0$ corresponds to the current averable level of vehicle occupancy.
...
We establish estimates of vehicle travel cost per mile by component category (those related to fuel cost, non-fuel financial costs, and time costs). Gather these for Base assumptions and Alternative scenario assumptions.

```{r poolingScenData, echo=echoWorking}

# extract essential cost vars, and combine base and alt scenario cases
scenData = VTCostAlt %>% select(DemScen, VehType, CostCat, VTCost_cpm) # extract essential vars
VTCostBase$DemScen="Base"              # label base as another scenario
VTCostBase = VTCostBase[names(scenData)] # order cost to match alt cost scenario data, and bind
scenData = rbind(scenData, VTCostBase) # conbine alt and base scenario data
```

```{r poolingScenDataSelectedCases, echo=echoWorking}
# Focus on the base and alt scen cost components for LDV cars
VehTypeWanted = "LDVAvgSedan"
DemScenWanted = c("Base", "DS3")

scenData = scenData %>% filter(VehType==VehTypeWanted) %>% spread(key = CostCat, value = VTCost_cpm)
scenData = scenData %>% 
  ...
  mutate(
    C_H = C_Total - C_F - C_T,  # non-fuel (and non-time) vehicle financial costs. Includes accident
...
scenCostData = scenData %>% select(DemScen, C_F, C_H, C_T, C_V)
# Get the travel cost components for scenarios of interest.
scenCostData = scenCostData %>% filter(DemScen %in% DemScenWanted)

# # !!! NOTE THIS VARIANT ENERGY INTENSITY (FUEL COST) FOR # TEST!!:
# scenCostData = scenCostData %>%
#   mutate( # only alter the non-Base case fuel cost, and total # variable cost
#     C_V = ifelse(DemScen=="Base",C_V, C_V + C_F),
#     C_F = ifelse(DemScen=="Base",C_F, 2*C_F)
#     )

kable(scenCostData, caption="Table: Vehicle travel cost data [c/mi] at Base occupancy, for these test cases of alternative pooling.",
      digits=3)
```
...
occupancyEffects = crossing(scenCostData, occupancyEffects)

# TravTimeCostBasePars["average_speed.LDV","average"]

occupancyEffects = occupancyEffects %>%
  mutate(C_Vpmt = C_V/o,            # Variable vehicle travel cost per pass-mi [c/pass-mi]
         rho = o^elas_VoTT_wrt_o ,  # Time-cost multiplier due to disutility of pooling [unitless]
         ...
         C_Tdph = C_T/Pi,           # Hourly Time-cost per passenger-hour (for info only) [$/pass-hr]
...
Pooling, or increased occupancy, reduces costs by sharing vehicle operating costs over all passengers, and reducing total VMT as pooling contracts the vehicle distance traveled in completing a tour of multiple trips (considered later). It can also increase costs to passengers by increasing the costs per unit travel time (relative to solo travel), and by adding some detour distance to a passenger's travel.  

We account for these changes, and the resulting demand response through the elasticity of travel demand with respect to full, generalized cost.

Obviously the response of travelers will be quite heterogenous and situation-dependent, but the aggregate response can be estimated.
...
ElasPKT = RelTotCost %>% filter(DemScen %in% DemScenWanted, VehType==VehTypeWanted, Case == "Low") %>%
  .$ElasVKT
```

The estimated long-run elasticity of LDV passenger travel demand with respect to cost use here is `r ElasPKT` (for the Demand Scenario `r DemScenWanted`, "Low" case).
...
occupancyEffects = occupancyEffects %>%
  ...
  mutate(
    PMT = (C_Mpmt/C_Mpmt[o==1.0 & DemScen=="Base"])^ElasPKT, #	pass-mi (normalized w.r.t Base scenario, base Occupancy)
    VMT = PMT/o,                                             #	veh-mi (normalized) (defining relation for Occupancy "o")
    EnergyUse = VMT * C_F/C_F[DemScen=="Base" & o==1.0]   # relative fuel cost/km indicates relative fuel intensity
    )
...
occupancyEffects %>%
  ...
  kable(caption="Table: check relative Cost per mile traveled at no-pooling level (occupancy=1.0")
...
occupancyTravelEffects = occupancyEffects %>% 
  select(DemScen, o, PMT, VMT, VMT_direct) %>%
  gather(key= travelEffect, value = value, PMT:VMT_direct)
...
occupancyTravelEffects %>%
  ...
  ggplot( aes(x=o, y=value , color=travelEffect, linetype=DemScen)) + 
    geom_line(size=1.0) +
    ggtitle("Effect of Pooling on VMT and PMT") +
...
```{r checkOccupancyEffects, echo=echoWorking}
# This check out to match calculations elsewhere
# occupancyEffects %>% select(DemScen, o, PMT, VMT, EnergyUse) %>%
#   filter(DemScen=="DS3")
```

### 8.2. Consider various degrees to which pooling contracts tour length and adds detours to passenger trips

```{r calcPMTandVMT, echo=echoWorking}
occupancyEffects = occupancyEffects %>% mutate(
...
AltVMT %>% filter(DemScen == "DS3") %>%
  ...
  kable(digits=4, caption="Energy Use Relative to Base (DS3), by Occupancy and Route Length Elasticity")
...
baseenergy = pull(occupancyEffects[occupancyEffects$DemScen=="DS3" & occupancyEffects$o==1.0,],EnergyUse)
