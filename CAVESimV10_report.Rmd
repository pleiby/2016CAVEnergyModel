...
```{r aggregatesSelectedCaseAssumps, echo = echoWorking}
# Aggregated Case results across Automation type, Fuel type, and shared Use
CaseWantedAggAssumps <- CaseAssumpsResults %>%
  # filter(Year==CurrYear) %>%
  right_join(select(CombScen_Combinations_subset, -Notes), 
             by = c("Year", "ElasCase", "VC", "TechScen", "DemScen")) %>%
...
# Aggregated Case results across Automation type, Fuel type, and shared Use
CaseWantedAggEffects <- CaseEffects %>%
  # filter(Year==CurrYear) %>%
  right_join(select(CombScen_Combinations_subset, -Notes), 
             by = c("Year", "ElasCase", "VC", "TechScen", "DemScen")) %>%
#  inner_join(select(CombScen_Combinations_subset, -Notes), 
...
# scenAggEffects <- CaseWantedAggEffects %>% 
#   filter(
#     DemScen %in% DemScenWanted,
#     ElasCase == ElasCaseWanted,
#     # VehType == VehTypeWanted
...
VTCostBaseTotal <- VTCostBase %>%
  filter(CostCat == "Total") %>%
  select(-CostCat) # first get Base total cost for each VehType
C_tot_base = VTCostBaseTotal %>% filter(VehType == VehTypeWanted) %>% pull(VTCost_cpm)
...
# # These tests were passed with text differences of ~0
# occupancyEffects  %>%
#   filter(o == 1.0) %>%
#   mutate(
...
# occupancyEffects %>% select(DemScen, o, PMT, VMT, Energy) %>%
#   filter(DemScen=="DS3")
```
...
baseenergy <- pull(occupancyEffects[occupancyEffects$DemScen == test_DemScen & occupancyEffects$o == 1.0, ], Energy)
