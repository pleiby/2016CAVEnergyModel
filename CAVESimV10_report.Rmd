...
DetailedDemScens <- DetailedDemScens[DetailedDemScens!="Zero"] # not of interest, Zero does what is says
```
...
aggregateSelectedCaseAssumpsResults <- function(case_assumps_results, 
  ...
  case_assumps_results %>%
    ...
    mutate(totshare = ifelse(totshare == 0, 1E-8, totshare)) %>%
    mutate_at(vars(fracVMTIncr, NIE, F_A_Shares, U_Shares, 
                   C_Fuel, C_Time, C_VNonF, C_Total,
                   ElasVKT, RidePoolAdjFrac, USDmult, mI_A, mI_C, mI_FA, mI_FB),
              funs(. / totshare))
}
...
```{r testCasesForSelectedSegments}
...
data <- CaseWantedAggAssumpsPrivateCAV %>%
  select(c(CombScen, Year, VC, ElasCase, DemScen, TechScen, NIE, fracVMTIncr, EnergyUse)) %>%
  # CaseWantedAggEffects %>%
  arrange(CombScen, Year, VC, ElasCase) %>%
  gather(key = variable, value = calcValue, testVars) %>%
...
data %>% filter(VC == "LDV") %>%
  ...
  facet_wrap(~CombScen) +
  scale_fill_brewer(palette = "Spectral") +
  ggtitle("Fig: Energy-Intensity, VMT, and Energy Scenarios", subtitle = "Private LDV CAVs, Excludes Ride-Pooling") +
  ylab("Fractional Change")
