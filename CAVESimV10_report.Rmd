...
shares_by_U <- 
  shares_by_U %>%
  ungroup() %>%
  spread(key = DemScen, value = U_Share) %>%
  mutate( # new Scenarios - full automation, no automation
    Test = ifelse(Use == "Shared", 1, 0),
    Zero = ifelse(Use == "Private", 1, 0)
  ) %>%
  gather(key = DemScen, value = U_Share,-c(VC, Use))
...
```{r gatherAndReportCombScenInputsAndResults}
CombScenInputsAndResultsForOneTechScen <- function(CurrTS, CurrYr) {
...
Iterate, building a dataframe of Combined Scenario (CombScen) assumptions and results.
...
for (CurrTechScen in techScensWanted) {
  CaseAssumpsResults_many <- rbind(CaseAssumpsResults_many, CombScenInputsAndResultsForOneTechScen(CurrTechScen, CurrYear))
}
```

Construct details of scenario, disaggregated by vehicle class, automation, fuel techology, use
.

```{r postProcessCombScenarioResults, echo=echoWorking}
...
CaseAssumpsResults <- CaseAssumpsResults %>%
  left_join(shares_by_F_A) %>% # add Fuel and Automation shares
  left_join(shares_by_U) # add Use (Sharing, Hailing/Pooling) shares
CaseAssumpsResults <- CaseAssumpsResults %>%
  mutate(
    ...
    # Adjustments to Demand based on vehicle Use
...
CaseAssumpsResults <- CaseAssumpsResults %>%
  mutate(
    ...
    EnergyUse = ((1 + NIE) * (1 + fracVMTIncr) - 1) * F_A_Shares * U_Share
  )

# Aside: Test summation of disaggregated shares
temp = CaseAssumpsResults %>%
  mutate(share_prods = F_A_Shares * U_Share) %>%
  group_by(DemScen, TechScen, ElasCase, VC, Year) %>%
  summarize(TotShare = sum(share_prods)) %>%
  ungroup
sum(temp$TotShare != 1.0) # should all equal 1.0

temp = shares_by_U %>%
  group_by(DemScen, VC) %>%
  summarize(TotShare = sum(U_Share)) %>%
  ungroup
sum(temp$TotShare != 1.0) # should all equal 1.0
...
CaseEffects <- CaseAssumpsResults %>% # extract a key set of results and identifying indices
  select(c("DemScen", "ElasCase", "TechScen", "VC", "Year", 
           "Automation", "Fuel", "Use", "F_A_Shares", "U_Share",
...
```{r aggregateForCaseWantedEffectsAndAssumps, echo=echoWorking}
...
```{r aggregatesSelectedCaseEffects, echo = echoWorking}
# Aggregated Case results across Automation type, Fuel type, and shared Use
...
#             by = c("Year", "ElasCase", "VC", "TechScen", "DemScen")) 
  mutate(Joint_Share = F_A_Shares * U_Share) %>%
  mutate(
    shrWtd_fracVMTIncr = fracVMTIncr*Joint_Share,
    shrWtd_NIE = NIE*Joint_Share
  ) %>%
  select(-c(fracVMTIncr, NIE)) %>%
  group_by(CombScen, DemScen, TechScen, ElasCase, VC, Year) %>%
  summarize(
    fracVMTIncr = sum(shrWtd_fracVMTIncr),
    NIE = sum(shrWtd_NIE),
    EnergyUse = sum(EnergyUse),
    totshare = sum(Joint_Share)
    ) %>%
  ungroup # %>%
  # filter(DemScen %in% ) - get rid of DemScens with no results DS0, 2, 3, 5, 6

```
```{r checkAggregatesOverSelectedCaseEffects, echo = echoTests}
testAggregation <- CaseWantedEffects %>%
  mutate(
  ...
  select(-Year)
cat("Check: Calculated aggregate `EnergyUse` changes match product of aggregate energy intensity `NIE` and VMT changes `fracVMTIncr` to within total absolute error ",
