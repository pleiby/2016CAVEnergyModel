...
CaseAssumpsResults <- CaseAssumpsResults %>%
  mutate(
    ...
    # if (TRUE) {
    # Now need to adjust Fuel Cost based on NIE
    ...
    old_fracVMTIncr = (1 + old_fracVMTIncr) * mD_U - 1,
    # }
...
CaseAssumpsResults <- CaseAssumpsResults %>%
  mutate(
    ...
    old_EnergyUse = ((1 + NIE) * (1 + old_fracVMTIncr) - 1) * F_A_Shares * U_Share,
...
temp %>% # Note: no Full Auto Private Gas vehicles were altered
  ...
  filter(testTEST) #pull(testTEST) %>% sum()
cat(
  "Check: Calculated differences in I_deltaCAV and NIE disagg vs agg and they match to within total error",
  paste(sum(abs(temp$testTEST))), ", due to composition effects.\n"
)
```
...
aggregateSelectedCaseAssumpsResults <- function(case_assumps_results, comb_subset) {
