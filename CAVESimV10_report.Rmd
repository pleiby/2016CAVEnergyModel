...
aggregateSelectedCaseAssumpsResults <- function(case_assumps_results, 
  ...
  case_assumps_results %>%
    ...
    mutate_at(vars(fracVMTIncr, NIE, F_A_Shares, U_Shares, 
              ...
              funs(. / totshare)) %>%
    mutate(ifelse(totshare == 1E-8, 0, totshare)) # restore zeros to zero from EPS
}
...
occupancyEffects1 <- data.frame("o" = seq(from = 0.8, to = 1.5, by = 0.05)) #  occupancy, unitless (pass/veh, normalized)

occupancyEffects1 <- scenAggData %>%
  select(DemScen, C_F, C_H, C_T, C_V, fracVMTIncr, NIE, EnergyUse, ElasVKT) %>%
  crossing(occupancyEffects1)
...
occupancyEffects1 <- occupancyEffects1 %>%
  ...
  # Energy = Energy_Pool1 * o^ElasTourLengthWithPooling
```


```{r calcPMTandVMT, echo=echoWorking}
occupancyEffects1 <- occupancyEffects1 %>% mutate(
  TourContraction = o^Elas_TourLength_wrt_Pooling,
  RideDetourExpansion = o^Elas_RiderDetour_wrt_Pooling,
  C_T_mult = RideDetourExpansion, # passenger time cost per mile rises with RideDetourExpansion
  # C_Vp
)
```
