...
VTCostBaseTotal <- VTCostBase %>% # get base travel costs, total, cents/m9
  filter(CostCat == "Total") %>%
  select(-CostCat) # first get Base total cost for each VehType
C_tot_ref = VTCostBaseTotal %>% filter(VehType == "LDVAvgSedan") %>% pull(VTCost_cpm)

scenData <- scenData %>%
  # construct components wanted, from cost share to monetary values in c/mi
  mutate(
    C_F = C_tot_ref * C_Fuel, # fuel costs, c/mi
    C_T = C_tot_ref * C_Time, # time costs [c/mi]
    # C_K = VehCapCost,
    C_tot = C_tot_ref * C_Total
...
scenCostData %>% 
  ...
  ggplot() +
    ...
    ggtitle("Fig: Vehicle travel cost data [c/mi] at Base occupancy.") +
...
baseenergy <- pull(occupancyEffects[occupancyEffects$DemScen == test_DemScen & occupancyEffects$o == 1.0, ], EnergyUse)
...
```
### Estimate Scenario effects

### Estimate Scenario effects, with 
