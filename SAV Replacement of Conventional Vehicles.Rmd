---
title: "SAV Replacement Ratio: Replacement of Private Conventional Vehicles"
author: "Paul N. Leiby"
date: "3/14/2019 - draft"
output:
...
Comment: It appears that essentially _all_ estimates entail a central or global system optimization, i.e. equivalent to a _single_ coordinated ride-service operator or TNC, seeking to provide acceptable wait and travel times with minimal SAV fleet size.^[Thus they imply either a single coordinated SAV fleet under city/government control or a utility-like arrangement where a service monopoly is granted to, or achieved by, a single operator. Perhaps more realistically, in a competitive market with potentially substantial profits, we would expect the entrance of multiple competing ride services, and lower levels of SAV utilization until costs and profits equilibrate. This would imply lower SAV Replacement Ratio]
...
We show simple calculations of how the total vehicle fleet (parc) size could decline, if the SAVs introduced replace the rides provided by multiple private cars (according to the SAV Replacement Rate), while the total number of rides remains constant.

Consider the size of the vehicle fleet, or parc, $N_{tot0}$ in the base case (Case 0). Assuming for simplicity that all vehicles are private (conventional?) in the Base Case, the number of Private vehicles $N_{PV0{} equals the total number of vehicles:

$$ParcSize_0 \equiv N_{tot0} = N_{PV0}$$

This number of vehicles satisfies the base demand for rides/trips, $Rides_0$.
$$Rides_0 = 1*N_{tot0}$$

where we normalize the number of rides per private vehicle to 1.0. Note that with this normalization, the number of rides provided per SAV is equal to the Replacement Ratio $RR$

In the alternate case (Case 1), there are $N_{PV}$ private vehicles, $N_{SV}$ shared vehicles, 
...
The total rides provided in the alternate case 
$$Rides_1 = 1 \cdot N_{PV} + RR \cdot N_{SV} = 1 \cdot N_{tot0}$$

Assuming no change in travel demand (Rides) between Case 0 and Case 1:
$$Rides_1 = Rides_0$$

$$ParcSize_1 \equiv N_{tot1} = = N_{PV} + N_{SV} $$

$\rho$ denotes the Relative Vehicle Parc Size, i.e.
$$\rho = N_{tot1}/N_{tot0}$$

$$\rho = \frac {(N_{PV} + N_{SV})}{(1 \cdot N_{PV} + RR \cdot N_{SV})}$$
Dividing by the total number of vehicle in the alternative case 
$$N_1 \equiv N_{SV} + N_{PV}$$

$$\rho = \frac {(N_{PV} + N_{SV})/N_{1}}{(1 \cdot N_{PV} + RR \cdot N_{SV})/N_{1}}$$


$$\rho = \frac {1}{(N_{PV}/N_1 + RR \cdot N_{SV}/N_{1})}$$


So for any posited fraction $S_{s}$ of shared vehicles out of total vehicles $N$
$$S_{s0} \equiv N_{SV0}/N_0 = 0$$
$$S_{s1} == N_{SV1}/N_1$$
$$1-S_{s1} == N_{PV1}/N_1$$

$$\rho = \frac {1}{(1-S_{s1}) + RR \cdot S_{s1})}$$

Note that this result checks out, in that we can compute the number of rides provided by this number of vehicles:
$$Rides = 1 \cdot N_{PV} + RR  \cdot  N_{SV}$$
$$Rides = N_1  \cdot  [1 \cdot (1-S_{s1}) + RR  \cdot  S_{s1})]$$


### Extension to More Vehicle Types
- Differentiate between Replacement Ratio for SAVs and SMV (Shared Manual Vehicles may not serve as many rides)
- Differentiate between Replacement Ratio for SAVs and Private AVs (which may still serve more rides than Private MVs)
- All relative to ride provision of Private Manual Vehicles
