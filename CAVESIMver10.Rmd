...
echoWorking <- T # set to FALSE for an exported (non-working version)
...
# LDV  2035   Automation_accessory_load     0.0    11.0    15.0
...
Actually:
- NIE <- (1+NIE)*2 - 1 = 2 + 2 NIE - 1 = 1 + 2 NIE
- ((1-0.7186037)*2 - 1)/-0.7186037 = 0.6084124 # for LDV in 2050
- NIE = 0.6084124 * NIE # for LDV in 2050
- sqrt(0.6084124) = 0.7800079
NIE = prod(IM) - 1
prod(IM)` - 1 = 1 + 2 (prod(IM) - 1) = 1 - 2 + 2 prod(IM) = 2 prod(IM) -1 
-->
...
CombScenResultsForOneTechScen <- function(CurrTS, CurrYr) {
  ...
  VMTIncrease <- VMTIncrease %>%
    ...
    mutate(fracVMTIncr = (1 + fracVMTIncr) * USDmult * (1 + RidePoolAdjFrac) - 1) # apply demand shifts
  ...
  EffectsD <- left_join(x = EffectsD, filter(TechScenEffectsEI, TechScen == CurrTS, Year == CurrYr), by = c("VC"))
...
CS_B_low :2050  :Low      :HDV  :4            :B_low       :
CS_B_high:2050  :Low      :HDV  :4            :B_high      :
CS_C_low :2050  :Low      :HDV  :4            :C_low       :
CS_B_high:2050  :Low      :HDV  :4            :C_high      :
...
CS_B_low :2050  :Low      :LDV  :4            :B_low       :
CS_B_high:2050  :Low      :LDV  :4            :B_high      :
CS_C_low :2050  :Low      :LDV  :4            :C_low       :
CS_C_high:2050  :Low      :LDV  :4            :C_high      :
"), header = TRUE, sep = ":", strip.white = TRUE) #
...
```{r keepCaseWantedEffectsAndAssumps, echo=echoWorking}
...
CaseWantedEffects <- CaseEffects %>%
  ...
  right_join(select(CombScen_Combinations, -Notes), 
             by = c("Year", "ElasCase", "VC", "TechScen", "DemScen"))

CaseWantedAssumps <- CaseAssumps %>%
  right_join(CombScen_Combinations, by = c("Year", "ElasCase", "VC", "TechScen", "DemScen"))
```
...
cat("Check: Calculated values match Scenario Results test values (intensity, VMT, EUse) to within total absolute error ",
