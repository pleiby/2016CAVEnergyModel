...
CaseAssumpsResults <- data.frame() # simplest empty dataframe

for (CurrTechScen in 1:7) {
  CaseAssumpsResults <- rbind(CaseAssumpsResults, CombScenInputsAndResultsForOneTechScen(CurrTechScen, CurrYear))
}

# Compute Energy Use impact
CaseAssumpsResults <- CaseAssumpsResults %>% mutate(EnergyUse = (1 + NIE) * (1 + fracVMTIncr) - 1.0)

CaseEffects <- CaseAssumpsResults %>% # extract a key set of results and identifying indices
  select(c("DemScen", "ElasCase", "TechScen", "VC", "Year", "fracVMTIncr", "NIE", "EnergyUse"))
