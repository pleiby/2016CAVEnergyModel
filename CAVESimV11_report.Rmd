...
mechsBarChart <- function(mechs_df) {
  ...
  # IEffects %>%
    ...
    mutate(
      ...
      uplim = ifelse(Opt < 0, Opt-Pess, Pess-Opt),
      xlolim = ifelse(Opt < 0, Pess, Opt),
      Mech = str_replace_all(Mech, "_", " ")
    ) %>%
    select(Mech, uplim, xlolim) %>% # name this way because bars plotted alphabetically
  ...
  temp_df %>%
    ...
    scale_fill_manual(values = c("steelblue", NA)) + # suppress the ploting of the lower/second bar
    coord_flip() +
    theme(legend.position = "none") + 
    scale_y_continuous(breaks=seq(-50, 60, 10), limits = c(-50.0, 60.0)) + # still the y axis even though flipped
...
```{r summaryIntensityMechanisms, echo=echoWorking, dpi=300, fig.width=7}
# IEffects[,order(ncol(IEffects):1)] %>% # reverse column order (has no effect)
# IEffects$Mech <- factor(IEffects$name, levels = IEffects$name[order(x$val)])
IEffects$Mech = with(IEffects, factor(Mech, levels = rev(levels(Mech))))
IEffects %>%
