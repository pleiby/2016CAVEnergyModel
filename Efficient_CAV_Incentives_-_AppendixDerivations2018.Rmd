---
title: "Efficient CAV Taxation - Appendix - Derivations"
author: "Paul Leiby and Jonathan Rubin"
date: "June 19, 2018"
output:
  word_document:
    reference_docx: Efficient_CAV_Taxation_-_Template.docx
...
$$U(C,M,F,H,N) \equiv u(\psi(C,M,T,\bar G), N) - \phi(P) - \delta(A)$$
$$\begin{aligned}
s.t. ~&M = m(H, F) ~~~~~~~~~~~~~~~~~~&~~\text{Miles traveled production [mi/cap-yr]}\\
     & T = \pi(\bar M) \cdot M       &~~\text{Travel Time [hrs/cap-yr]}\\
     & P = P_F(\bar F) + P_M(\bar M) &~~\text{Pollution from fuel use and miles traveled}\\
     & A = a(\bar M) \bar M          &~~\text{Accident risk, with per-mile risk based on average driving}
\end{aligned}
\label{privateOptEq}
$$
...
$$\text{budget constr [\$/yr]: } C + (q_F+t_F) F + H ~=~ I ~=~ (1-t_L) L$$
$$\text{time constr [hrs/yr]: } L +N + T ~=~ \bar L$$
$$\text{govt budget constr [\$/yr]: } t_L L + t_F F ~=~ \bar G$$
(gen equilibrium closure constraint)

**Substituting** the expression for Travel Time *T* in terms of average pace $\pi$ (1/speed) and miles driven, combining the budget constraint and time constraint to solve for labor *L*, all constraints combine into 2. These are the constraints reuiring VMT production to equal VMT demand, and the household budget constraint. Solving the Lagrangian maximization for private households yields the indirect utility function $V$.
$$M \le m(H, F)  ~~~:\mu$$
$$ C + (q_F+t_F) F + H \le (1-t_L)(\bar L -N -\pi M) ~~~:\lambda$$

$$
\begin{aligned}
V(t_F, t_L, q_F, P, A, \pi, \bar L,\bar G) = \max_{C,M,F,H,N} &u(\psi(C,M,\pi(\bar M) M,\bar G), N) - \phi(P_F(\bar F)+P_M(\bar M)) - \delta(a(\bar M) \bar M) \\
                       & +\left ( m(H, F) - M \right ) \mu \\
                       & +\left ((1-t_L)(\bar L -N -\pi M) - C - (q_F + t_F)F -H \right) \lambda
\end{aligned}
$$
$$
...
Written this way, we expect shadow values $\mu \ge 0, \lambda \ge 0$. 


Consider first derivatives of the Lagrangian
--------------
Using shorthand notation where subscripts denote some derivatives with respect to the variables $X \in \{ C, M, T, G, N \}$, marginal utility with respect to *X* is denoted
$$ \frac {\partial u}{\partial \psi} \frac {\partial \psi}{\partial X} \equiv \frac {\partial u}{\partial \psi} \psi_X \equiv u_X$$
Then
$$\frac {\partial L}{\partial C} = \frac {\partial u}{\partial \psi} \psi_C - \lambda = 0$$ Hence $\lambda$ is the marginal utility of consumption (or post-tax income)

Let "marginal benefit" of" $B_X$ denote marginal utility of any consumed good *X* converted to equivalent dollars (by dividing by marginal utility of after-tax income $\lambda$). The marginal benefit of consumption is (normalized to) 1:

$$B_C \equiv u_C / \lambda = 1$$
$$u_C = \lambda$$


$$\frac {\partial L}{\partial N} = \frac {\partial u}{\partial \psi} \psi_C - (1-t_L) \lambda = 0$$
$$B_N \equiv u_N /\lambda = 1-t_L$$
for wage $w=1$.

For the private optimum, which omits the dependence of society-wide variables pace $\pi$, pollution $P_M$ *and* accident risk *A* on $\bar M$, privately optimal mileage or VMT per capita satisfies:
$$\frac {\partial L}{\partial M} = \frac {\partial u}{\partial \psi} \psi_M + \frac {\partial u}{\partial \psi} \psi_T \pi(\bar M) - \mu - \pi(\bar M)  (1-t_L) \lambda = 0$$
with units
$$[MU/mi] + [MU/travelHr][travelHr/mi] - [MU/mi] - [travelHr/mi] (1-[unitless])[MU/\$] = 0$$

Or, simplifying
$$u_M + u_T \pi(\bar M) - \mu - \pi(\bar M) (1-t_L) \lambda = 0$$
$$u_M/ \lambda =   \frac {\mu}{\lambda} + \pi(\bar M) \left ( 1-t_L - \frac {u_T} {\lambda} \right )$$
with units
$$[\$/mi] = \frac {[MU/mi]} {[MU/\$]} + [hr/mi]* \left ( 1 - [unitless] - \frac {[MU/travelHr]}{[MU/\$]} \right )$$

To solve for $\mu$, the marginal utility of increased VMT production, consider the private choice of fuel F and other inputs *H* (in $) used to produce VMT *M*:
$$\frac {\partial L}{\partial H} = \frac {\partial m}{\partial H} \mu -  \lambda = 0$$
$$m_H \mu =  \lambda $$
$$[mile/\$]*[MU/mile] = [MU/\$]$$

For the privately optimal fuel use, which omits the dependence of "pollution" (or other fuel-related externalities like energy security) $P_F$ on $\bar F$:
$$\frac {\partial L}{\partial F} = \frac {\partial m}{\partial F} \mu - (q_F + t_F)  \lambda = 0$$
$$m_F \mu =  (q_F + t_F)\lambda$$
$$[mile/gal]*[MU/mile] = [\$/gal]* [MU/\$]$$

Marginal benefit (in $) per mile produced equals marginal non-fuel cost per mile is
$$\mu/ \lambda =  1/m_H$$
Also, marginal benefit (in $) per mile produced equals marginal fuel cost per mile. 
$$\mu/ \lambda =  (q_F + t_F)/m_F$$
These last two conditions together assure that fuel and non-fuel use are adjusted until their marginal benefit equals their marginal costs [???Interpretation of above two conditions may be off]
...
allows solving for $\mu$, the optimal shadow cost of the production of miles in the condition equating marginal benefit of miles with the marginal production cost
$$(q_F + t_F) F/M + H/M = {\mu/ \lambda} $$
Thus at the optimum, the marginal cost of both inputs to VMT production equals the average total cost per mile.

Substituting into the optimal condition for VMT _M_ yields the condition balancing marginal private benefit of travel with marginal private-cost. $\mu/\lambda$ is the optimal private cost per mile, counting the cost of fuel _F_ and other inputs _H_. [Note: _H_ includes vehicle plus private accident costs associated with the production of VMT, but _not_ private time costs.]
$$u_M/\lambda = \mu/\lambda +  \pi(\bar M)  \left ( (1-t_L) -  u_T/\lambda \right)$$
$$u_M/\lambda  = \left ( (q_F + t_F) F/M + H/M \right ) + \pi(\bar M)  \left ( (1-t_L) - u_T/\lambda \right )$$
$$[MB \$/mi] = [fuel+veh cost \$/mi] + [travelHr/mi] \left ( 1- [unitless] - [MB \$/travelHr] \right )$$

Have derived the private-optimal condition for VMT, matching the marginal benefit (utility-per-mile in dollars) of miles with the full cost per mile $c_M$ (Note: P&S2004 call this RHS $p_M$), including the vehicle capital and operation costs per mile $c_V$ plus the net marginal cost of time per mile, where $c_T$ is the net opportunity cost of time spent traveling per hour: 
...
Implying demand functions for the final consumption goods *C,M* and factors *L,F,H* of the form
$$C = C(c_M,t_L)$$
$$M = M(c_M, t_L)$$ (Note: this function for VMT would also be a function of $t_M$ if imposed)
$$L = L(c_M, t_L)$$
...
* [Note: these last two equations would include the term $t_M dM/dt_F$ on the eft-hand-side and the denominator, respectively, if VMT _M_ was also taxed.]
...
* _File: "Efficient_CAV_Taxation_-_AppendixDerivations 2016MMDD.Rmd"_
