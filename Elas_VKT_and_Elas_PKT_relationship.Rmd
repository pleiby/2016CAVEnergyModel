...
ElasTourContractionWithPooling = 0.0 #, elasticity of Vehicle tour length (for mult trips) w.r.t. occupancy
...
# establish values of Occupance "o" to be evaluated
PMTVMT = data.frame("o" = seq(from = 0.8, to = 1.5,by = 0.05)) #	occupancy, unitless (pass/veh, normalized)
...
PMTVMT = PMTVMT %>% mutate(
  EnergyUse = VMT * C_F/C_F[DemScen=="Base" & o==1.0]
)
```


```{r}
PMTVMT %>% select(-VehType) %>%
...
## 3. Consider various egrees to which pooling contracts tour length and adds detours to passenger trips
