...
date: Rev. 2020-03-03, Printed `r Sys.Date()`
output:
  html_document:
    df_print: paged
    fig_caption: yes
...
abstract: A compact and aggregated model of road vehicle travel and energy use,
  in basic form with little commentary.
---
...
From this *Kaya* or *ASIF* decomposition approach the energy use for a particular
vehicle type is given by total activity (passenger travel demand) level $A_{tv}$ 
...
Vehicle energy intensity *I* is a function of:

- $\vec x_{tmv}$ a vector of CAV mechanism/technology levels,
  as well as 
- $\phi_{tmvf}$ the mix of fuel-drivetrains used (extent of electrification).

- $G_t$ Transportation GHG emissions
...
- f = fuel type
- t = time period (year)
- k = mechanisms that alter energy intensity or demand
- j = technology case
- d = demand case
...
- K = set of mechanisms *k* that alter energy or demand
- J = set of technology cases *j*
- D = set of demand cases *d*.
...
---------------------------
We identified a set *K* of "Mechanisms" or "Technologies" associated with vehicle automation, each $k \in K$ of which has an effect represented by multiplier $\mu_k$ that can increase or decrease a component term in the ASIF decomposition. In general, a mechanism can affect 

- travel activity levels *A*, 
- vehicle energy intensities *I*, or 
- mode and fuel shares $\sigma, \phi$.

Furthermore, a mechanism could also effect vehicle class shares or occupancy, but we do not yet consider such effects. 

Taking a Scenario Approach, we can construct scenarios *s* that combine technology cases $j \in J$ and demand cases $d \in D$. For each scenario values are indexed by year *t* (for selected years), vehicle class *v*, mechanism *k* and by the Combined Technology/Demand scenario *s*, i.e. $\mu_{tvks}$ for $s_{jtvk}$ defines an alternative `EffectCase` for mechanism *k* and technology scenario *j*(???).
...
SetK_Mechs <- c( # mechanisms k by which automation alters energy and/or demand
  ...
  # Demand Mechanism set includes these
  # "D_Underserved_travelers",
  # "D_Ride_Hailing_Empty_Miles",
  # "D_Induced_VMT_Demand"
)
...
SetS_Sense <- c( # Sensitivity EffectCases s for mechanism effects
  "Opt",
  "Mid",
  "Pess",
  "Zero"
...
library(stringr)
library(janitor)
library(patchwork)  # for composing compound graphs
```


# 3. Estimating Energy Impacts

- *s* indexes Effect sensitivity cases from "zero" through "pessimistic," "midcase," and "optimistic." 
- $I_{E,tvks}$ Mechanism intensity effect values indicating the fractional change in energy intensity for a particular mechanism *k*, and are differentiated by year *t*, vehicle class *v*, and effect sensitivity *s*.
...
- $\mu_{tvks}$ multipliers (`IEffectsm$IM`) constructed from the fractional changes in energy intensity, for each mechanism/technology *k*, year *t*, vehicle class *v*, and effect sensitivity case *s*.
    -  (supressing subscripts *m, f*, which would indicate possible dependence on mode and fuel_tech)
...
# Intensity effects as fractional change and multiplying factors by (VC, Year, Mech, EffectCase)

# spread out IM for EffectCases and display table
IEffectsm %>%
  select(-IE) %>%
...
# sort columns same way as original data frame
  select(colnames(IEffects)) %>%
  kable(
    caption = "Table: Intensity Multipliers by Vehicle-type, Year, Mechanism, and Case"
  )
```
...
- $\mu_{jtv}$ Combined Scenario Multipliers (aggregating over mechanisms *k*, supressing subscripts *m, f*)
  - for the vehicle energy intensity of each Technology Scenario *j*, for Year *t* and Vehicle class *v*.
  - These overall intensity multipliers for technology scenario *j* reflect the combined the effect of all the mechanisms on vehicle energy intensity.

- $s_{jtvk}$ for $s \in \{Zero, Low, Mid, High\}$ the estimated `EffectCase` or effect sensitivity case *s* is specified for each mechanism *k*, and for each technology scenario *j*, year *t*, vehicle class *v*.
    - The `EffectCase` indicates a discrete point in the range of potential mechanism impacts provided as multipliers $\mu_{tvks}$ in the array `IEffectsm`

#### Establish the (exogenous) mapping between Technology Scenarios and the `EffectCase` for each Mechanism

Read a data file with scenario names, descriptions, `TechScen` number.
Note: `Demand_Scenario` number in this file is unused/deprecated.

```{r constructIntensityByScenMech, echo=echoWorking, warning=FALSE, message=FALSE}
# Load the descriptions of the scenarios
scenario_description_filename <- "CAV_Scenario_Descriptions.csv"
technology_scenario_data_filename <- "CAV_TechScenario_Master.csv"

mechScenAssumps <- read_csv(paste0("./Data/", scenario_description_filename), comment = "#")
# This table contains: tech scenario number and (unused) demand scenario number;
# scenario name and description. 
# Assumptions re setting levels (Zero, Low, Mid, High) for each mechanism now in separate file.
...
```
The scenario desription data file is named `r scenario_description_filename`.

Read a data file defining Technology Scenario Assumptions:
for each `TechScen` number, indicate `EffectCase` level assumed for each
mechanism `Mech` by vehicle class `VC` and `Year`.
The Technology Scenario assumption data file is named `r technology_scenario_data_filename`.

Join selected intensity multipliers for each technology mechanism effect case
from (Zero, Low, Mid, High) values in `IEffectsm`

```{r readTechScenCases, echo=echoWorking} 
# warning=FALSE, message=FALSE}
TechScenAssumps <- read_csv(paste0("./Data/", technology_scenario_data_filename), comment = "#")
...
msa <- TechScenAssumps %>%
  ...
  # IEffectsm has 144 = (2 x 2 x 4 EffectCases x 9 Mechs) rows.
  # while msa has 324 = (2 x 2 x 9 TechScen x9 Mechs) rows.
...
msa %>%
  select(-IM) %>%
  # select(c("TechScen", "VC", "Year", "Mech", "EffectCase")) # re-order cols
  spread(key = TechScen, value = EffectCase) %>%
  arrange(Year, desc(VC)) %>%
  kable(
    caption = "Table: Effect Sensitivity Cases by Vehicle Class, Year, AV Mechanism and Tech Scenario"
    )
```
...
For each technology technology scenario *j* the effect sensitivity case is (exogenously) specified for each mechanism *k* on vehicle class *v* in year *t*, and thus the technology-scenario-specific effect case *s* is subscripted ${s_{jtvk}}$.
...
Actual achieved vehicle energy intensity will depend on the economic responses of travelers based on costs and the utility maximization.

Note: Here 

- $\mu_{tvk{s_{jtvk}}}$ = `IM`
- $\mu_{tjv}$ = `prod(IM)`
- $\Delta I_{jtv}$ = `NIE` = $\mu_{jtv} - 1$
...
EnergyIntensityChanges <- msa %>%
  ...
  arrange(TechScen, VC, Year)
```

```{r constructIntensityComponsByScenario, echo=echoWorking, warning=FALSE, message=FALSE}
# get all energy intensity changes by scenario and with component values
EnergyIntensityCompons <- msa %>%
  select(-EffectCase) %>%
  spread(key = Mech, value = IM) %>%
  left_join(EnergyIntensityChanges, by = c("TechScen", "VC", "Year"))
```
The results for Net Energy Intensity changes ($\Delta I_{jtv}$) 
by fixed technology scenario are as follows.
...
### 3.3 Plots of Energy Intensity Changes by Technology Scenario, and by Mechanism
...
```{r defFnMechsBarChart, echo=echoWorking}
# If you want the heights of the bars to represent values in the data, use geom_col(),
#   or geom_bar() with  stat_identity().
# geom_col(mapping = NULL, data = NULL, position = "stack", ...,
#   width = NULL, na.rm = FALSE, show.legend = NA,
#   inherit.aes = TRUE)

mechsBarChart <- function(mechs_df, 
                          VC_chosen = "LDV",
                          Year_chosen = "2050") {
  # mechs_df expected to have same structure as IEffects dataframe
  # Produced horizontal bar chart of effects by mechanism
  mechs_df %>%
    filter(VC==VC_chosen & Year==Year_chosen) %>%
    mutate(
      # uplim = ifelse(Opt < 0, Opt-Pess, Pess-Opt),
      # lolim = ifelse(Opt < 0, Pess, Opt)
      # try to generalize fn mechsBarChart() to account for bars spanning from negative to postive values (fails)
      neglim = ifelse(Opt < 0 & Pess > 0, Opt, 0), # plot this neg bar segment if cross 0
      uplim = ifelse(Opt < 0 & Pess > 0,
                     Pess,
                     ifelse( Opt < 0,  
                             Opt-Pess, Pess-Opt)
                     ),
      xlolim = ifelse(Opt < 0 & Pess > 0,
                     0, # omit
                     ifelse(Opt < 0, Pess, Opt)
                     ),
      Mech = str_replace_all(Mech, "_", " ")
    )  %>%
    select(Mech, uplim, xlolim, neglim) %>% # name this way because bars plotted alphabetically
    gather(key = case, value = value, -Mech) -> temp_df
  # str(temp_df)
  # temp_df$Mech = with(temp_df, factor(Mech, levels = rev(levels(Mech))))

  temp_df %>%
    ggplot(aes(x=factor(Mech), y = value, fill=case))+
    geom_bar(stat="identity", width=0.7) + #fill="steelblue")+
    # scale_fill_manual(values = c("blue", "steelblue", "red")) + # for testing the 3 segments
    scale_fill_manual(values = c("steelblue", "steelblue", NA)) + # suppress the plotting of the lower/second bar
    coord_flip() +
    theme(legend.position = "none") + 
    scale_y_continuous(breaks=seq(-50, 70, 10), limits = c(-50.0, 70.0)) + # still the y axis even though flipped
    xlab("Mechanism") + ylab("Percent Change") +
...
#### Energy Intensity Mechanisms Summary

Here is a summary of the mechanisms affecting energy intensity, and their impact ranges.

```{r summaryIntensityMechanisms_wide, echo=echoWorking, dpi=300, fig.width=7}
# IEffects[,order(ncol(IEffects):1)] %>% # reverse column order (has no effect)
# IEffects$Mech <- factor(IEffects$name, levels = IEffects$name[order(x$val)])
# IEffects$Mech = with(IEffects, factor(Mech, levels = rev(levels(Mech))))
IEffects %>%
#  select()
  mechsBarChart()
```

### 3.4 Establish Cases for Exogenous Demand Effects

```{r buildIEffectsTemp, echo=echoWorking, dpi=300, fig.width=7}
DEffectsExog <- read.delim(textConnection("
VC  : Year  : Mech                        : Opt : Mid : Pess  : Zero
LDV : 2035  : D_Underserved_travelers     : 0   : 6   : 10    : 0
LDV : 2035  : D_Ride_Hailing_Empty_Miles  : -20 : 30  : 50    : 0
LDV : 2035  : D_Induced_VMT_Demand        : 15  : 25  : 67    : 0
LDV : 2050  : D_Underserved_travelers     : 0   : 6   : 10    : 0
LDV : 2050  : D_Ride_Hailing_Empty_Miles  : -20 : 30  : 50    : 0
LDV : 2050  : D_Induced_VMT_Demand        : 15  : 25  : 67    : 0
"), header = TRUE, sep = ":", strip.white = TRUE) # ;

DEffectsExog <- DEffectsExog %>%
  mutate(
    VC = as.character(VC),
    Mech = as.character(Mech)
  )
# Difficulty controlling order of bars in bar chart:
#Effects$Mech = with(IEffects, factor(Mech, levels = rev(levels(Mech)))) # does not work
# DEffectsExog %>%
#  mechsBarChart()

```

#### Combining Demand Mechanisms with Energy Intensity Mechanisms in Summary Bar Chart

```{r combBar, echo=echoWorking, dpi=300, fig.width=7}
# Following renaming is a crude way to force ordering of mechanism/effects
#   in this bar chart
IEffects %>%
  mutate(
    Mech = str_replace(Mech, "Congestion_mitigation", "Lower congestion"),
    Mech = str_replace(Mech, "Automation_accessory_load", "Sensor/Computer load"),
    Mech = str_replace(Mech, "Ride_Hailing_Empty_Miles", "Ride_Hailing_Empty_Miles"),
    Mech = str_replace(Mech, "De_emphasized_performance", "De-emphasized_performance")
  ) %>%
bind_rows(DEffectsExog) -> CombIDEffects
...
**Note**: This is actually `ElasPKT`, which equals `ElasVKT` for fixed occupancy at the reference level. `ElasVKT` will vary with occupancy, for fixed `ElasPKT`.
...
The former `DemScenCostChange` only provides C_deltaInsur and C_deltaVoTT.)
...
case                  Zero    Low     Med     High
LDV.ElasVKT           0.0     -1.0    -1.1    -1.2
LDV.ExclVehCapCost    0.0     0       0       0
LDV.C_deltaMaint      0.0     0       0       0
LDV.C_deltaInsurX     0.0     -0.40   -0.60   -0.80
LDV.C_deltaCapCost    0.0     0.10    0.10    0.10
LDV.C_deltaTolls      0.0     0       0       0
LDV.C_deltaPrkng      0.0     0       0       0
LDV.C_deltaRegis      0.0     0       0       0
LDV.C_deltaVoTTHwy    0.0     -0.10   -0.30   -0.65
LDV.C_deltaVoTTArt    0.0     -0.10   -0.30   -0.65
LDV.ShrECostInt       1.0      1       1       1
LDV.I_deltaCAV        0.0     -.7186  -.7186  -.7186
HDV.ElasVKT           0.0     -0.97   -0.97   -2
HDV.ExclVehCapCost    0.0     0       0       0
HDV.C_deltaMaint      0.0     0       0       0  
HDV.C_deltaInsurX     0.0     -0.40   -0.60   -0.80
HDV.C_deltaCapCost    0.0     0.05    0.05    0.05
HDV.C_deltaTolls      0.0     0       0       0
HDV.C_deltaPrkng      0.0     0       0       0
HDV.C_deltaRegis      0.0     0       0       0
HDV.C_deltaVoTTHwy    0.0     -0.10   -0.30   -0.65
HDV.C_deltaVoTTArt    0.0     -0.10   -0.30   -0.65
HDV.ShrECostInt       1.0     1       1       1
HDV.I_deltaCAV        0.0     -.2815  -.2815  -.2815
"), header = TRUE, sep = "", strip.white = TRUE) # ;
...
kable(
  ...
  caption = "Table: Key Parameters for Demand Response (for all Demand Scenarios)"
)
```
...
DemScen :C_deltaVoTT :C_deltaInsur :C_deltaCapCostX:Description
DS1     :0           :-0.60        : 0.00          :Partial automation. Little benefits of comfort (0% reduction in VoT), lower end of insurance benefits (60%)
DS2     :-0.05       :-0.60        : 0.00          :Partial automation. Some benefits of comfort (5% reduction in VoT), lower end of insurance benefits (60%)
DS3     :-0.50       :-0.80        : 0.00          :Self driving. Large benefits of comfort + in vehicle use of time (50% reduction in VoT), large benefits of insurance (80%)
DS4     :-0.80       :-0.80        : 0.00          :Extreme Self driving case. Large benefits of comfort + in vehicle use of time (80% reduction in VoT), large benefits of insurance (80%)
DS5     :-0.80       :-0.80        : 0.00          :Same as DemScen 4, with different exogenous demand effects
DS6     :-0.80       :-0.80        : 0.00          :Same as DemScen 4, with different exogenous demand effects
DS7     :-0.50       :-0.80        : 0.00          :Same as DemScen 3, with different exogenous demand effects
Test    :-0.80       :-0.80        : 0.00          :Same as DemScen 6
Zero    : 0.00       : 0.00        : 0.00          :No cost changes
Base    : 0.00       : 0.00        : 0.00          :No cost changes
B_Low   :-0.10       :-0.40        : 0.10          :SMART Scen Sharing is Caring low
B_High  :-0.30       :-0.40        : 0.10          :SMART Scen Sharing is Caring high
C_Low   :-0.10       :-0.40        : 0.00          :SMART Scen All About Me low
C_High  :-0.50       :-0.40        : 0.00          :SMART Scen All About Me high
"), header = TRUE, sep = ":", strip.white = TRUE)
# DS0     :0           : 0.0         :Base
...
kable(
  ...
  caption = "Table: Vehicle Travel Cost Component Change (Reduction) by Demand Scenario"
)
```

### 4.1.2 Establish the Reference Mix of Vehicle Types in the Desired Demand Scenarios

We adopt reference vehicle mix from DOE's SMART Mobility FY19 SMART Workflow Scenario Common Assumptions.
For each scenario, this provides vehicle share by class, fuel-technology
(drivetrain), degree of automation, and the fraction of base trips provided
by shared vehicle services (TNCs).
...
Vehicle shares by fuel/drivetrain type, automation level, and use-type are specified
exognously for SMART Consortium main scenarios.

```{r reshapeAndSugmentSMARTFY19CommonAssumptions, echo=echoWorking}
...
shares_by_F_A <-
  ...
  mutate( # new Scenarios
    Test = ifelse((Automation == "Full") & (Fuel == "Gas"), 1, 0), # Full Automation Gaso
    Zero = ifelse((Automation == "No") & (Fuel == "Gas"), 1, 0) # No Automation Gaso
  ) %>%
...
shares_by_U <-
  ...
  mutate( # new Scenarios
    Test = ifelse(Use == "Shared", 1, 0), # full Shared
    Zero = ifelse(Use == "Private", 1, 0) # no Shared (full Private)
  ) %>%
  gather(key = DemScen, value = U_Shares, -c(VC, Use))
```

### 4.1.3 Select a Single Technology Scenario and Year to Examine
...
Energy Intensity change is determined by on the Technology Scenario, and year of interest. For example consider Technology Scenario `r CurrTechScen`, the "`r mechScenAssumps[[CurrTechScen,"Scenario_Name"]]`" scenario, in year `r CurrYear`. 

Demand response is a function of full (generalized) travel cost, and energy intensity affects the energy component of travel cost.[^*] The full scenario requires updating Demand Response parameters with energy intensity reduction (by vehicle class and year) for this Technology Scenario and year, and updating other costs (travel time cost, accident/insurance costs, vehicle capital and maintenance costs) based on the Demand Scenario assumptions.[^x]

[^*]: Note that the energy-cost implications of automation are also influenced by the choice of drivetrain/fuel, i.e. the extent to which CAVs are electrified compared to Manual Vehicles. This vehicle technology choice could also alter other capital and operating costs. We take the share of vehicles electrified (i.e., the share of gasoline, PHEV, and BEV vehicles), by vehicle class, automation, and use-case, as exogenous scenario inputs.

[^x]: *Note:* For variant cost-sensitivity exploration of the scenarios, 
we could add an endogenous energy-cost dependent change in intensity at this point, 
based on historical information about the elasticity of vehicle fuel economy.

```{r defFn_nieForScenYear, echo=echoWorking}
# define function to extract net energy intensity change by Veh Class for TechScen and Year
nieForScenYear <- function(EIChanges, TechScenWanted, YearWanted) {
  EIChanges %>%
    ungroup() %>%
    filter(TechScen == TechScenWanted, Year == YearWanted) %>%
    select(VC, NIE) # returns a small df with net intensity changes for each vehicle class
}
```

```{r defFn_updateI_deltaCAVinDemRespParams, echo=echoWorking}
# Update I_deltaCAV, net energy intensity change, to be consistent with current TechScen
# Transcribe current (computed) net intensity effects (nie) to the I_deltaCAV variable in DemRespParams

# DemRespParams$I_deltaCAV <- DemRespParams$NIE <- EnergyIntensityChange$NIE, for matching VC, Year, TechScen
updateI_deltaCAVinDemRespParams <- function(
    local_DemRespParams, # df with VC, Parameter, and values for alt EffectCase(s)
    local_nie # net energy intensity changes for Current TechScen and Year, both vehicle classes
  ) {
  local_DemRespParams %>%
    gather(key = Sens, value = value, -c(VC, Parameter)) %>% 
    spread(key = Parameter, value = value) %>% # cols are now parameter values, by VA and "Sens" (EffectCase)
    left_join(local_nie, by = c("VC")) %>% # add column for nie over VC
    mutate(
      I_deltaCAV = NIE # update/replace with NIE for current TechScen
    ) %>%
    select(-NIE) %>%
    gather(key = Parameter, value = value, -c(VC, Sens)) %>% 
    spread(key = Sens, value = value) %>%
    select(c("VC", "Parameter", "Zero", "Low", "Med", "High")) # reorder columns after gather/spread
}
...
# Select net energy intensity changes and other cost and demand response parameters
#   for the current Technology and Demand Scenario, and Year

# First select net energy intensity changes for the current Technology and Year
nie <- nieForScenYear(EnergyIntensityChanges, CurrTechScen, CurrYear)
...
DemRespParams <- updateI_deltaCAVinDemRespParams(DemRespParams, nie)

kable(
  x = DemRespParams, digits = 4,
  caption = "Table DemRespParams: Important Parameters for the Calculation of Demand Reponse, by Vehicle-type, and Year"
)
```
...
In this test, for each vehicle, we use the VMT-weighted average of local and intercity speed^[**ToDo** distinguish between local and intercity speed and time cost per mile.] to construct the *base* average time cost per mile:
...
TravTimeCostBaseParsOrig <- read.delim(textConnection("
...
speed                 : LDV     :27.6       :27.6       :27.6       :mi/hr :EPA
TTC_per_mi            : LDV     :           :           :0.49994933 :$/mi  :=VoT/AveSpeed
...
speed                 : HDV     :39.98      :39.98      :39.98      :mi/hr :ATRI 2012 'average industry operational speed'
TTC_per_mi            : HDV     :0.611      :0.611      :0.611      :$/mi  :2011 Driver wages & Benefits, atri-online.org/2012/09/17/an-analysis-of-the-operational-costs-of-trucking-a-2012-update/"),
  header = TRUE, sep = ":", strip.white = TRUE
) #
...
TravTimeCostBaseParsOrig %>%
  select(-Source_Notes) %>%
  kable(caption = "Table: Base Travel Time Cost Parameters", digits = 3)
...
For *v* vehicle class {LDV, HDV} and *r* region {local, intercity}

$$c_{Tmvr} = c_{Thvr}/s_{vr}$$
Average cost per unit time for each vehicle class *V* is the total time cost of all "regions" *r* (road type), divided by the total driving time across all regions.
$$c_{Thv} = \frac {\sum_r c_{Thvr} T_{vr}}{\sum_r T_{vr}}$$
Equivalently, this is the cost per unit time in each "region" *r* (road type)
weighted by the amount of time spent on each type of driving, which is
distance divided by speed.

$$c_{Thv} = \frac {\sum_r c_{Thvr} T_{vr}}{\sum_r T_{vr}} = \frac {\sum_r c_{Thvr} M_{vr}/s_{vr}}{\sum_r M_{vr}/s_{vr}}$$


Average cost per unit distance (mile) for each vehicle class *V* is the
total time cost of all "regions" *r* (road type), divided by the total miles
across all regions.

$$c_{Tmv} = \frac {\sum_r c_{Thvr} T_{vr}}{\sum_r M_{vr}} = \frac {\sum_r c_{Thvr} M_{vr}/s_{vr}}{\sum_r M_{vr}} = c_{Thv} \frac {\sum_r M_{vr}/s_{vr}}{\sum_r M_{vr}}$$
Note that this average time cost per mile is _not_ the same thing and the
VMT-weighted cost of time for each type of driving, _unless_ the speeds are
the same for each type of driving.


```{r checkTravelTimeCost, echo=echoWorking, include=echoTests}
TravTimeCostBasePars <- TravTimeCostBaseParsOrig %>%
  select(-Units, -Source_Notes)

TravTimeCostBasePars <- TravTimeCostBasePars %>%
  gather(key = roadtype, value = value, c(local, intercity, average)) %>%
  spread(key = var, value = value)

# Following calculation yields LDV Time cost per mile
# Test computed vs. specified values in table
# Calculated average VoTT for each vehicle class as VMT-weighted average of local and intercity VoTT.
#   Correction: should be time-weighted average of local and intercity VoTT,
#   which is only the same if speeds are the same in the regions.

# ToDo!!!  Update average VoTTs
#  "VoTT.LDV", "average" =
#     ("VMTtot.LDV", "local"] * "VoTT.LDV", "local"] +
#       "VMTtot.LDV", "intercity"] * ["VoTT.LDV", "intercity"]) /
#       (["VMTtot.LDV", "local"] + ["VMTtot.LDV", "intercity"])

#TravTimeCostBasePars <- TravTimeCostBasePars %>%
#  mutate(VoTTalt = )  # for V_Class = LDV, HDV, region = local, intercity, average
...
# Following calculation yields Travel Time Cost per mile
TravTimeCostBasePars <- TravTimeCostBasePars %>%
  mutate(TTC_per_mi = VoTT / speed)  # for V_Class = LDV, HDV, region = local, intercity, average
...
row.names(TravTimeCostBasePars) <- paste(TravTimeCostBasePars$V_Class, TravTimeCostBasePars$roadtype, sep = ".")
...
Update (base) time costs per mile base on latest calculation.
...
ChkErrorDiff <-
  ...
  c(
    TravTimeCostBasePars["LDV.average", "TTC_per_mi"],
    TravTimeCostBasePars["LDV.average", "TTC_per_mi"],
    TravTimeCostBasePars["HDV.average", "TTC_per_mi"]
  )
  ...
  VTCostBase <- VTCostBase %>%
    mutate(
      VTCost_cpm = ifelse((CostCat == "Time" & substr(VehType, 1, 3) == "LDV"),
        100 * TravTimeCostBasePars["LDV.average", "TTC_per_mi"], VTCost_cpm
      ),
      VTCost_cpm = ifelse((CostCat == "Time" & substr(VehType, 1, 3) == "HDV"),
        100 * TravTimeCostBasePars["HDV.average", "TTC_per_mi"], VTCost_cpm
      )
    )
}
```
...
VTCostBase %>%
  ...
  kable(
    ...
    caption = "Table: Base Vehicle Travel Cost Components by Vehicle Type (cents/mi)"
  )
```
...
VTCShrBase %>%
  ...
  kable(
    x = ., row.names = F, digits = 3,
    caption = "Table: Base Vehicle Travel Cost: Components Share by Vehicle Type, Year=2018"
  )
```
...
```{r baseCostComponStackedBarGraph_NoTitle, echo=echoWorking, dpi=300}

VTCostBase %>%
  filter(CostCat != "Total") %>%
  ggplot() +
  geom_bar(aes(x = VehType, y = VTCost_cpm, fill = CostCat), stat = "identity") +
  ggtitle(" ", subtitle = "Manual Vehicle, Current Year, cents/mi") +
  ylab("cents/mi") +
  scale_fill_brewer(palette = "Spectral") +
    theme(
    plot.title = element_text(size=14, face="bold"),
...
<!---
ToDo: Increase flexibility in accomodating differenet fractions of automated,
electrified and shared mobility in the scenario. 
-->
...
x <- left_join(x, select(DemScenCostChange, -c("C_deltaCapCostX", "Description")), by = c("DemScen")) # appends Dem Scen multipliers for other vars (VoTT and Insurance)
VTCShrAlt <- x
...
Updated cost component table based on *Demand* Scenario adjustments to VoTT and other travel costs.
...
# **ToDo**: generalize C_deltaXxx to include potential shifts on _all_ travel cost components, and vary by Veh technology
VTCShrAlt <- VTCShrAlt %>% mutate(
  ...
  Time = Time * (1.0 + C_deltaVoTT), # This multiplier, from DemScenCostChange, Applies only to Private, Full CAV
...
#   for specified demand scenario ds,
#   plot stacked bar graph with one column for each VehType.
plotStackedBarCostCompons <- function(df, 
                                      ds, 
                                      ts = "TS4",
                                      curr_title = "",
                                      curr_subtitle = ""
                                      ) {
  if (curr_title =="") curr_title = paste0("Fig: Alt Cost Components for Vehicle Travel, Scen=", ds, "/", ts)
  if (curr_subtitle == "") curr_subtitle = "Automated Vehicle, Current Year, cents/mi"
  df %>%
    filter(DemScen == ds) %>%
    filter(CostCat != "Total") %>%
    ggplot() +
...
```{r altCostComponStackedBarGraph_NoTitles, echo=echoWorking, dpi=300}
# ToDo: Need to out base cost compons in the VTCostAlt df, so it can be plotted with this function.
# plotStackedBarCostCompons(VTCostBase, "Base", "TS1", curr_title = " ", curr_subtitle = "Manual Vehicle, Current Year, cents/mi")
...
-->
- multiplier factors on VKT to account for increase or decline in response to ride-hailing/ride-pooling, for the same level of PKT.
<!---
...
- a shift (increase or reduction) from ride hailing/pooling.
...
Shared  :No         :0.0     :-0.2    :0.3    :0.3
Shared  :Partial    :0.0     :-0.2    :0.3    :0.3 
Shared  :Full       :0.0     :-0.2    :0.3    :0.6 
Private :No         :0.0     :0.0     :0.0    :0.0  
Private :Partial    :0.0     :0.0     :0.0    :0.0 
Private :Full       :0.0     :0.0     :0.2    :0.4
"), header = TRUE, sep = ":", strip.white = TRUE) #
# refRideHailingPoolingAdj <- -0.2 # fractional change for total LDV VMT.
# Only applies for DemScen and VC with flag for non-Zero RidePoolAdjCase (Mult for HDV=1.0).
```

Specify Combined Scenarios. A combined scenario $CS_d$ associates each Demand Scenario $DS_d$ with and Underserved Multiplier Option `USOption` and possible further VMT adjustment for ride hailing/pooling (`refRideHailingPoolingAdj`). Note that all of these demand adjustments, for cost-based demand response $m_{vtdj}(c_{rel,vtdj})$, underserved-demand multiplier $\gamma_{USD,vd}$, and ride hailing/pooling demand shift $\rho_{HailPool,vd}$, are applied multiplicatively to the base demand level through net effect $M_{vtdsj}$.

Given the relative cost per unit distance for vehicle class *v* in time *t*, demand scenario *d*, technology scenario *j* and elasticity case *s*

$$c_{rel,vtdsj} ~\equiv~ RelTotCost_{vtdsj}$$
(Note: relative total cost is also indexed by ElasCase *s*, for $s \in \{High, Low\}$)

$$\begin {align}
m_{vtdsj}(c_{rel,vtsdj}) &~=~ (c_{rel,vtdj})^{ElasVKT_{cvs}}-1\\
...
$$\Delta M_{vtdsj} = M_{vtdsj} - 1$$

Note: $\Delta M_{vtdsj}  \equiv \text{fracVMTIncr}$ after final net adjustments.
...
Note: This approach assumes that all VKT demand, after exogenous adjustments, responds to VKT cost changes according to the elasticity `ElasVKT`.

$$\begin {align}
M_{vtdsj} &~=~ \left ( (c_{rel,vtdj})^{ElasVKT_{cvs}} \right ) \cdot \gamma_{USD,vd} \cdot (1+\rho_{HailPool,vd}) 
\end {align}
$$

Note: a more refined analysis would account for the different travel costs faced by ride-hailing/pooling travelers.
...
VC      :DemScen :USOption:RidePoolAdjCase : Comments
...
# Exogenous numerical multipliers/shifts to demand for Underserved Demand
#   and RideHailing are now compiled in table `DemScenAdjustments`,
#   Then they are applied to the calculated demand change 
#   VMTIncrease$fracVMTIncr

# Pivot for Zero to High RidePoolAdjCase case adjustments for all Use, Automation
RideHailingPoolingAdj <- refRideHailingPoolingAdj %>%
  gather(key = "RidePoolAdjCase", value = RidePoolAdjFrac, -c(Use, Automation)) 

# Initially, restrict focus to only Shared, Full Automation cases
temp_RideHailingPoolingAdj <- RideHailingPoolingAdj %>%
  filter(Use == "Shared", Automation == "Full") %>%
  select(-c(Use, Automation))

DemScenAdjustments <- DemScenAdjustments %>%
  # ToDo: make these VMT adjustments on the disaggregated basis of vehicle Use and Automation level
  mutate_at(.vars = c("VC", "DemScen", "RidePoolAdjCase"), .funs = as.character) %>%
  # Use ride Hailing/Pooling multiplicative adjustment factor for all Demand Scenarios based on RidePoolAdjCase
  left_join(temp_RideHailingPoolingAdj, by = c("RidePoolAdjCase")) # match VMT adjustment fractions for ride hailing/poolin to case
...
#  Need to apply FAU shares to this dataframe, by adding rows and columns for 
#  subcategories of vehicle type, e.g.
#  - Automation (Base Non-Automated fraction)
#  - Fuel/Drivetrain (electric fraction)
#  - Use type (shared fraction)
# Then can easily establish multiply I by VMT, calc desired subtotals and report.
```

# 5. Policy and Combined Scenario Calculations

We generate bar graphs of fractional changes in energy intensity, travel demand, energy use (each by LDV/HDV), and Total Energy Use (both sectors).
...
$$\Delta I_{tjv} = 1 - \mu_{jtv}$$
The _cost-elastic_ travel demand or VMT fractional changes have been calculated by year *t*, vehicle class *v*, and technology scenario scenario *j* in `VMTIncrease`.^[Note that this first pass analysis aggregates all vehicle types at the Vehicle Class level, describing a single or average Fuel (drivetrain), Automation level, and Use type.]
...
- $\Delta I_{tjv}$ = `I_deltaCAV` = `NIE`
- $c_{rel,vtdj}$ = `VTCShrAlt`
- $m_{vtdj}$ = `fracVMTIncr`
- $r_s$ = `RidePoolAdjFrac`

VMT demand is further adjusted based on exogenous demand shifts associated with new use categories and the effect of ride-hailing/ride-pooling on VMT.

$$M_{vtdsj} ~=~ 1 + m_{vtdj} ~=~ (c_{rel,vtdj})^{ElasVKT_{cvs}} \cdot \gamma_{USD,vd}\cdot (1+\rho_{HailPool,vd})$$
...
TechScenEffectsEI <- EnergyIntensityChanges %>% # ***UNUSED***
  ungroup() %>%
...
CombScenInputsAndResultsForOneTechScen <- function(CurrTS, CurrYr, demScensWanted) {
  ...
  #   - vehicle clases VC
  # Requires globals:
  #   - EnergyIntensityCompons: dataframe of energy intensity changes by TechScen, VC, Year
  #                    (intensity change columns include Net Intensity Effects NIE and all compon mechanisms)
  ...
  #   - VTCShrBase: df of base cost component shares `VTCost_shr`, by VehType, CostCat. 
  #   - DemScenCostChange: vehicle travel cost parametes by DemScen, used only for C_deltaInsur and C_deltaVoTT
    
  # VTCShrAlt

  # Start with dataframe replicates the Base Vehicle Travel Cost Shares for each Demand Scenario
  VTCShrAlt <- VTCShrBase
  # duplicate Base column, for desired Demand Scenarios
  oldDemScens = c("DS1", "DS2", "DS3", "DS4", "DS5", "DS6", "DS7", "Test", "Zero") # not really needed
  VTCShrAlt[c(oldDemScens, demScensWanted)] = VTCShrAlt$VTCost_shr
  ...
  # Select energy intensity changes for the Current Technology Scenario and Year, for both main vehicle classes
  #   leaves a small df with net intensity changes for each vehicle class
  nie <- nieForScenYear(EnergyIntensityCompons, CurrTS, CurrYr)

  # Better to skip this step calling updateI_deltaCAVinDemRespParams,
  #   instead updating I_deltaCAV, and just use NIE and needed parts of DemRespParams directly.
  # Update I_deltaCAV, net energy intensity change, to be consistent with current TechScen
  # Transcribe current (computed) net intensity changes (nie) to the I_deltaCAV variable in DemRespParams
  #  Get updated change in net energy intensity (based on product of active mechanisms in current tech scenario and year)
  # DemRespParams <- updateI_deltaCAVinDemRespParams(DemRespParams, nie)
  ...
  y <- DemRespParams %>%
    select(-c(Zero, Med)) %>% # drop these unused cases
    gather(key = Case, value = value, Low:High) %>% # for two cases (Low and High)
    spread(key = Parameter, value = value) %>%
    right_join(nie, by="VC") %>%
    mutate(I_deltaCAV = NIE) %>% # update to net intensity effect for curr TechScen
    rename(TEMP_NIE = NIE) # kept only for check
  
  # combine base cost shares in VTCShrAlt and Demand Reponse Parameters (Fractional Adjustments) in y for their adjustment
  VTCShrAlt <- left_join(VTCShrAlt, y, by = c("VC")) # note this also duplicates VTCShrAlt (base shares) rows by Sensitivity Case in y
  # append correct Dem Scen multipliers for other vars (VoTT and Insurance)
  VTCShrAlt <- left_join(VTCShrAlt, select(DemScenCostChange, -c("C_deltaCapCostX", "Description")),
                         by = c("DemScen")) # currently _not_ Sensitivity Case-dependent, and not disagg by vehicle Automation level or Use case (sharing)
  # Do disaggregation by F,A,U shares later in 2nd pass
  ...
  # Apply adjustments to Cost (per mi) Components, across Demand Scenario (DemScen), Veh Type, and Case (Low - High)
  # Note that this is perceived/internalized cost (may not include all energy gains)
  # **ToDo**: generalize C_deltaXxx to include potential shifts on _all_ travel cost components,
  #   and vary by Veh technology
  # **ToDo**: apply vehicle-technology specific I_deltaCAV to costs prior to application of ElasVKT. 
  #   I.e., I_deltaCAV to vary with level of Automation and Electrification.
  VTCShrAlt <- VTCShrAlt %>% mutate(
    Fuel = Fuel * (1.0 + I_deltaCAV * ShrECostInt),
    # Fuel energy intensity reduction (from energy efficiency) * Share of energy efficiency costs internalized by traveler
    Maintenance = Maintenance * (1.0 + C_deltaMaint),
    AccAndIns   = AccAndIns   * (1.0 + C_deltaInsur),
    VehCapCost  = VehCapCost  * (1.0 + C_deltaCapCost),
    TollsFees   = TollsFees   * (1.0 + C_deltaTolls),
    Parking     = Parking     * (1.0 + C_deltaPrkng),
    Time        = Time        * (1.0 + C_deltaVoTT), # This multiplier, from DemScenCostChange, Applies only to Private, Full CAV
    Registration = Registration * (1.0 + C_deltaRegis)
  ...
  VMTIncrease <- RelTotCost %>%
    ...
    mutate(
      ...
      C_VNonFC = C_Total - C_Fuel - C_Time - C_Vcap, # non-fuel, non-capital (and non-time) vehicle financial costs. Includes accident.
      C_Veh = C_Fuel + C_VNonFC + C_Vcap # vehicle capital and operating costs, c/mi (excl. time)
    )
  ...
  VMTIncrease <- VMTIncrease %>%
    ...
    summarise_at(
      vars(
        ...
        C_Fuel, C_Time, C_Vcap, C_VNonFC, C_Total, ElasVKT, fracVMTIncr, I_deltaCAV, C_deltaVoTT, C_deltaCapCost
      ),
      .funs = mean
  ...
  VMTIncrease <- VMTIncrease %>%
    ...
    #   to apply `RidePoolAdjFrac` only to Shared Use vehicle travel
    #   Not: mutate(fracVMTIncr = (1 + fracVMTIncr) * USDmult * (1 + RidePoolAdjFrac) - 1)
  ...
  AssumpsD <- VMTIncrease %>%
    ungroup() %>%
    # add intensity mechanism effects for this Year and TechScen, and associated Energy Intensity NIE
    left_join(
...
DetailedDemScens <- DetailedDemScens[!(DetailedDemScens %in% c("Zero", "A_Low", "A_High", "B_Low", "C_Low", "Test"))] # not of interest, Zero does what ii says
# DetailedDemScens ***UNUSED***
```

```{r multCombScenarioResultsAndAssumptionsLoop, echo=echoWorking}
CurrYear <- 2050
techScensWanted <- unique(msa$TechScen) # all TechScens in data set
...
for (CurrTechScen in techScensWanted) {
  # this type of loop is slowed by the building/copying of the dataframe each iteration
  #  faster to fill a list of df with the loop, then unnest. But still fast.
...
We now have dataframe of cases, one row for each case, with assumptions and
aggregated results (for aggregate Vehicle Class and a single Year).
Number of cases is card(VC) x card(ElasCase) x card(DemScen) x card(TechScen).
Currently, this is 2 x 2 x 3 x 9 cases.

5.1 Construct Detailed Scenarios Disaggregated by Vehicle Sub-types
-------------------------------------------------------------------

Construct details of scenario, disaggregated by vehicle class, automation, fuel technology, use type.

For a target subset of `DemScen` in `DetailedDemScens` (e.g. Base, B_High and C_High), and for each vehicle class `VC`, specify and apply disaggregated demand and intensity adjustment factors by Vehicle Fuel, Automation and Use (*F*, *A* and *U*) categories.

```{r tablesForDisaggEffectsByVehSubcat, echo=echoWorking}
#    # Note these are all adjustments to the incremental effect of full automation
#    # Fraction of estimated Net Energy Intensity Effect gained by Partial or No Automation
#    mI_A   Automation Value 
#    mI_A   "Full",    1.0,
#    mI_A   "Partial", 0.5,
#    mI_A   "No",      0.0,
#    mI_A   otherwise, NA
#
#    # Relative Energy Intensity effect of full automation by Vehicle Class (LDV/HDV)
#    mI_C   VC    Value
#    mI_C   "LDV", 1.0,
#    mI_C   "HDV", 0.5,
#    mI_C   otherwise, NA
...
#    #  for the effect on energy use of electrification
#    mI_FA  Fuel,  Value
#    mI_FA  "Gas", 1.0,
#    mI_FA  "PHEV", 0.9,
#    mI_FA  "BEV", 0.5,
...
#    mD_U   "Shared", (1 + RidePoolAdjFrac)  # should be more disagg, based on `refRideHailingPoolingAdj`
#    mD_U   otherwise NA
#
#    # Adjusments to the Cost of Time (deltaVoTT) base on level of Automation (relative to Full Automation)
#    mCT_A   Automation Value 
#    mCT_A   "Full",    1.0,
#    mCT_A   "Partial", 0.5,
#    mCT_A   "No",      0.0,
#    mCT_A   otherwise, NA
...
```{r defFn_appendDisaggParams, echo=echoWorking}
# here detail parameters are defined for 
#   Intensity multipliers by vehicle type (A, F), and
#   demand shift multipliers by Use case U
appendDisaggParams <- function(DemScenData) {
  # Expand table to disaggregate by Vehicle category: Fuel Type, Automation, Use
  #  (replicates rows card(A) x card(F) x card(U) times and appends shares and multipliers
  #  for each vehicle category (Fuel, Automation, Use) for matching VC and DemScen)
  # Note these are adjustments to the base vehicle energy efficiency, time cost, or demand shift,
  #  for the effect on energy use of electrification, automation, sharing
  RideHailingPooling_mults_by_A_U <- RideHailingPoolingAdj %>%
    mutate_at(.vars = c("Use", "Automation"), .funs = as.character) %>%
    mutate(RidePoolAdjMult = RidePoolAdjFrac + 1) %>% # exog Dem change from empty VMT by Use and Automation
    select(-RidePoolAdjFrac)
  
  DemScenData %>%
  left_join(shares_by_F_A, by = c("VC", "DemScen")) %>% # add Fuel, Automation and their joint shares by VC and DemScen
  left_join(shares_by_U, by = c("VC", "DemScen")) %>% # add Use (Sharing, Hailing/Pooling) and shares
  left_join(RideHailingPooling_mults_by_A_U, by = c("Use", "Automation", "RidePoolAdjCase")) %>% # get demand change for empty VMT by Use and Automation, and case
  mutate(
    # Note these are all adjustments to the incremental effect of full automation, which is the base calculated value
    # Fraction of estimated Net Energy Intensity Effect gained by Partial or No Automation
    ...
    # Adjustments to VMT Demand (not necessarily PMT demand) based on scenario vehicle Use
    #  Should depend on combined Automation level and Use
    mD_U = RidePoolAdjMult, # exog demand mult for ridehailing/empty VMT, disagg by A and U
    # mD_U = ifelse(Use == "Private", 1.0,
    #   # estab Shared repositioning demand mults/shifts
    #   ifelse(Use == "Shared", (1 + RidePoolAdjFrac), # should be more disagg, based on `refRideHailingPoolingAdj`
    #     NA
    #   )
    # ),
    # Fraction of estimated Time Cost Effect (C_deltaVoTT) gained by Partial or No Automation
    #   "Partial" value based on ratio of VoTT effects for Automation==High vs Automation==Low in ScenAssumps_VoTT
    mCT_A = ifelse(Automation == "Full", 1.0,
      ifelse(Automation == "Partial", 0.5, # leads to ~67%- 82% of Multiplier (1-C_deltaVoTT) for Partial
        ifelse(Automation == "No", 0.0, # assures not reduction
          NA
        )
      )
    ),
    # Fraction of estimated Time Cost Effect gained by Use (ride-hailing) and Automation interaction
    mCT_UA = ifelse(Automation == "Full", 1.0, # ***UNUSED***
      ifelse(Automation == "Partial", 0.5,
        ifelse(Automation == "No", 0.0,
          NA
        )
    ...
    # Fraction of estimated Vehicle Capital Cost Effect (C_deltaCapCost) borne by Partial or No Automation
    #   "Partial" value based on ratio of VoTT effects for Automation==High vs Automation==Low in ScenAssumps_VoTT
    mCC_A = ifelse(Automation == "Full", 1.0,
      ifelse(Automation == "Partial", 0.3, # 
        ifelse(Automation == "No", 0.0, # assures no incremental
          NA
        )
      )
    )
  ) # produces and therefor returns the DemScenData with FAU-based modifying factors
} # end fn appendDisaggParams

# following has VC x DemScen x Fuel x Automation x Use = 2 x 3 x 3 x 3 x 2 = 108 demand adjustment combos
DisaggDemScenAdjustments <- DemScenAdjustments %>%  # For info only ***UNUSED***
  select(-Comments) %>%
...
#### Compute disaggregated Intensity, VMT Demand and Energy Use impact

We want to compute disaggregated Intensity, VMT Demand and Energy Use impact
for each vehicle category (Automation, Fuel, Use), vehicle class and scenario.

$\Delta I_{jtv} = \mu_{jtv} - 1$ = `NIE` 

$$\Delta I_{jtv, fau} = \Delta I_{jtv} \cdot m_{IC,v} \cdot m_{IA,a} \cdot  m_{IFA,f} \cdot  m_{IFB,f} $$

Also adjust other costs accordingly, for correct demand response calculation.

Note: total relative travel cost per mile and its fuel and non-fuel components will vary with
Automation *A* and with Fuel/Drivetrain type *F* interacting with Automation. 
Previous cost components were for Full Automation,
and each associated Technology and Demand scenario.

Fractional VMT increase (`fracVMTIncr` or $\Delta K$) are based on elastic
response to Total cost, with exogenous demand
shift effects associated with underserved demand groups, as assumed for the
combined technology/demand/vehicle-class/year scenario *x* ($x = jtvd$). 
Further VKT shifts associated with ZOV travel of ride sharing apply only
to vehicles of `Use` category (*U*) `Shared`.

$$\Delta K_{x,FAU} = (1 + R_{Total,x,FAU}^{\epsilon_{K,C,x}}) m_{D,x,US} \cdot m_{D,x,U} - 1.0$$

Here $R$ is relative total cost compared to the base vehicle and $\epsilon_{K,C,x}$ is the elasticity of VKT
for scenario *x*.

$$R_{Total,x,FAU}(\Delta C_{Fuel}(\Delta I_{jtv, FAU}), \Delta C_{VNonFuel}, \Delta C_{Time}) = \frac {C_{Total,x,FAU}} {C_{Total,base}}$$

Energy Use impacts are based on the combined effect of intensity and VKT demand changes at the disaggregated
vehicle segment, or sub-type, `FAU` level:

$$(1 + \Delta I_{jtv, FAU}) \cdot (1 + \Delta K_{x,FAU})$$

Then each energy change by vehicle sub-type (`Automation`, `Fuel` and `Use`)
is weighted by vehicle sub-type shares $s_{x,FA}$ and $s_{x,U}$ for scenario *x*,
allowing summation across subcategories
to get the total energy effect.

$$\Delta E_{x,FAU} = ((1 + \Delta I_{jtv, FAU}) \cdot (1 + \Delta K_{x,FAU}) - 1) \cdot s_{x,FA} \cdot s_{x,U}$$

Create `CaseAssumpsResults` from `CaseAssumptionsResults_Many` for selected `DemScen` in `DetailedDemScens`.

Add vehicle shares by F & A, and by U, to each combination of `VC` and `DemScen` `CaseAssumpsResults`.

```{r defFn_disaggCombScenResults, echo=echoWorking, warning=F}
# Compute Disagg Intensity, Costs, VMT Demand and Energy Use impact by veh type (C, A, F, U) (could do this earlier)
disaggCombScenResults <- function(dfCAR_preDisagg, # df with case assumps/results agg at level of VC, rather than vehicle-subtype (A, F, U)
                                  desiredDemScens, # char vec of DemScens to keep
                                  disaggVoTT = T) {
  dfCAR_preDisagg %>% # expect input df with case results aggregated as the level of VC, rather than vehicel-subtype (A, F, U)
    filter(DemScen %in% desiredDemScens) %>%
    # Expand table to disaggregate by Vehicle category: Fuel Type, Automation, Use
    #  (replicates rows card(A) x card(F) x card(U) times.
    # Appends shares of each vehicle category (Fuel, Automation, Use) for 
    #  matching VC and DemScen (F_A_Shares, U_Shares)
    # Applies adjustments (factors mXX_S) to the base vehicle energy efficiency, time cost, or demand shift,
    #  for the effect of vehicle Fuel (electrification), Automation, and Use (sharing).
    # Does _not_ weight disaggregated segments by share
    appendDisaggParams() %>% # For each vehicle demand segment/subcat: shares and adj factors for NIE, VoTT, VMT
    
    mutate(
      # adjust net intensity change for Veh Class, Automation, Fuel tech
      old_NIE = NIE,
      NIE = (1 + NIE * mI_C * mI_A * mI_FA) - 1.0, # NIE is now disagg by veh sub-type
      
      # if (TRUE) {
      # WARNING: Now need to adjust costs (identity is C_Total = C_Fuel + C_Vcap + _VNonFC + C_Time)
      #   - adjust Fuel Cost based on NIE
      #   Correct old Fuel Cost (and dependent components) for previous calcs, that were for Automated CAV, Gas, vehicle
      old_C_Fuel = C_Fuel,
      old_C_Vcap = C_Vcap,
      old_C_VNonFC = C_VNonFC,
      old_C_Time = C_Time, # C_Time at this point differentiated only by VC and DemScen - should be differentiated Automation, sharing
      # old_C_Veh = C_Vcap + C_VNonFC + C_Fuel, # Not directly tracked in AssumpsResults df: veh costs, excl. Time
      old_C_Total = C_Total,
      old_fracVMTIncr = fracVMTIncr, # this included USDmult by scen for CAVs, but no Ride-Hail/pool asjust for ZOVKT
      # ToDo: do not have ShrECostInt to do next, so count on it being 1.0!
      # C_Fuel = C_Fuel * (1.0 + NIE * ShrECostInt) / (1.0 + I_deltaCAV * ShrECostInt), # undo/redo this calc with NIE
      C_Fuel = old_C_Fuel * (1.0 + NIE  * 1.0         ) / (1.0 + I_deltaCAV * 1.0      ), # undo/redo this calc with NIE
      C_Vcap = old_C_Vcap * (1.0 + C_deltaCapCost*mCC_A)/(1.0 + C_deltaCapCost), # adjust veh cap cost for non full-AVs
      C_VNonFC = old_C_VNonFC,    # No change? # non-fuel non-capital (and non-time) vehicle financial costs. Includes accident.
      # Unwind adjustment for Full CAV, and apply time-cost adjustment based level of Automation (optional for testing)
      vec_disaggVoTT = disaggVoTT, # must define a column of logical conditions or ifelse fails
      C_Time = ifelse(vec_disaggVoTT, # ifelse condition must be a column
                      old_C_Time*(1.0 + C_deltaVoTT*mCT_A)/(1.0 + C_deltaVoTT),
                      old_C_Time),
  
      # C_Veh = C_Fuel + C_Vcap + C_VNonF, # vehicle capital and operating costs, c/mi (excl. time)
      C_Total = C_Fuel + C_Vcap + C_VNonFC + C_Time, 
      ...
      old_fracVMTIncr = (1 + old_fracVMTIncr) * mD_U - 1, # ??? WHY do this? should only apply to Shared travel (LDV)
      fracVMTIncr = (1 + fracVMTIncr) * mD_U - 1,
      
      # then weight VMT demand by vehicle sub-type shares, allowing summation across subcategories
      # actual energy use (relative to base for this vehicle segment)
      # ? EnergyUse = (1 + NIE) * (1 + fracVMTIncr) - 1.0,
      old_EnergyUse = ((1 + old_NIE) * (1 + old_fracVMTIncr) - 1),
      EnergyUse = ((1 + NIE) * (1 + fracVMTIncr) - 1)  # do NOT weight by shares yet
    ) %>%
    select(-vec_disaggVoTT) # remove temp var
}
```

```{r postProcessCombScenarioResults2, echo=echoWorking, warning=F}
CaseAssumpsResultsAgVT <- disaggCombScenResults(CaseAssumpsResults_many, DetailedDemScens, disaggVoTT = F) # more aggregated VoTT case

CaseAssumpsResultsDAVT <- disaggCombScenResults(CaseAssumpsResults_many, DetailedDemScens, disaggVoTT = T) # diaggregated VoTT case
```

Set overall treatment of fine details of disaggregation.

- related to potential variations in VoTT over vehicle Automation Level and Use type
- related to variations in VMT shifts from deadheading/repositioning and empty VMT over Use type and Automation level (for Ridehail vehicles by Automation type and for Private fully Automated vehicles)

```{r}
global_disaggEmptyVMT = T
global_disaggVoTT = T

if (global_disaggVoTT) {
  CaseAssumpsResults <- CaseAssumpsResultsDAVT
} else {
  CaseAssumpsResults <- CaseAssumpsResultsAgVT
}
```

#### Diagnosis/Testing: tables of revised intensities, `NIE` over original (`deltaCAV`)

```{r echo=echoWorking}
# look at changes in NIE (old_NIE = I_deltaCAV)
table(CaseAssumpsResults$VC, round(CaseAssumpsResults$NIE/CaseAssumpsResults$I_deltaCAV, 4))
```

```{r diagnoseDisagg, echo=echoWorking}
temp <- CaseAssumpsResults %>%
  mutate(
    NIEratio = # relative adj to agg change in energy intensity, treating numerical error as 0.0
      ifelse((abs(I_deltaCAV)<1E-10) & (abs(NIE)<1E-10), 1.0, round(NIE/I_deltaCAV, 6)),
    VKEratio = # relative adj to agg change in energy intensity, treating numerical error as 0.0
      ifelse((abs(old_fracVMTIncr)<1E-10) & (abs(fracVMTIncr)<1E-10), 1.0, round(fracVMTIncr/old_fracVMTIncr, 6)),
    EnUEratio = # relative adj to agg change in energy intensity, treating numerical error as 0.0
      ifelse((abs(old_EnergyUse)<1E-10) & (abs(EnergyUse)<1E-10), 1.0, round(EnergyUse/old_EnergyUse, 6))
  )

temp <- temp %>%
  select("VC", "DemScen", "ElasCase", "TechScen",
         "Automation", "Fuel", "Use",
         "C_Fuel", "C_Time", "C_Vcap", "C_VNonFC", "C_Total", 
         "ElasVKT", "I_deltaCAV", "C_deltaVoTT", "C_deltaCapCost", "NIE",
         "mI_A", "mI_C", "mI_FA", "mI_FB", "mD_U", "mCT_A", "mCC_A",
         "old_C_Fuel", "old_C_Time", "old_C_Vcap", "old_C_VNonFC", "old_C_Total",
         "old_fracVMTIncr", "old_EnergyUse",
         "fracVMTIncr",  "EnergyUse",
         "NIEratio", "VKEratio", "EnUEratio",
         "F_A_Shares", "U_Shares"
         ) %>%
  filter(
    VC == "LDV",
...
We focus only on the results from disaggregation on one Vehicle Class (`VC`).


```{r echo=echoWorking}
table(temp$TechScen, temp$VC)
```

Initally, the change in vehicle energy Intensity is uniform for each TechScen

```{r echo=echoWorking}
table(round(temp$I_deltaCAV, 8), temp$TechScen)
```

```{r echo=echoWorking}
table(round(temp$I_deltaCAV, 8), temp$VC)
```

After disaggregation by vehicle sub-type (A, F, U), there is 
substantial variation in Net Intensity Effect (`NIE`), or
net change in vehicle energy intensity, across
`TechScen`:

```{r echo=echoWorking}
table(temp$NIE, temp$TechScen)
```

We can see the effect of disaggregation by vehicle
sub-type by looking at the ratio `NIEratio` of adjusted to unadjusted
`NIE` (ratio of disaggregated to `VC`-aggregate `NIE`).
These `NIEratio` are the disaggregation adjustment factors to original
`NIE` (or equivalently, `I_deltaCAV`).

```{r echo=echoWorking}
table(temp$NIEratio, temp$TechScen)
# tempX <- temp %>%
#   filter(TechScen==6) %>%
#   mutate(bothzero = (abs(I_deltaCAV)<1E-10) & (abs(NIE)<1E-10)) %>%
#   select(I_deltaCAV, NIE, NIEratio, bothzero)
#   pull(I_deltaCAV) #%>%
#   summary()
```

**Note** these are multiplicative adjustments to `I_deltaCAV`,
that is, adjustments to the change (reduction) in energy
intensity estimated for a fully automated CAV, using
conventional gasoline drivetrain, and in private use.
So the adjustments `NIEratio` all reduce, or at best
maintain, the `I_deltaCAV` effect on intensity.

The adjustment to Net vehicle energy intensity change (`NIE`) varies 
strongly by `Automation`:

```{r echo=echoWorking}
table(temp$NIEratio, temp$Automation)
```

Vehicle energy intensity *change* is also adjusted based on `Fuel` (drivetrain) sub-type

```{r echo=echoWorking}
table(temp$NIEratio, temp$Fuel)
```

But vehicle energy intenstity changes are distributed the same way
(unaltered by) vehicle `Use` (sharing or private) ownership.

```{r echo=echoWorking}
table(temp$NIEratio, temp$Use)
#table(temp$NIE, temp$Use)
```

Looking now at effects on estimated changes to Intensity, VKT and Energy Use,
by vehicle sub-type.

```{r echo=echoWorking}
temp %>% 
  mutate(
    #AutomationCode = relevel(as.numeric(Automation),
    Automation = as.factor(Automation),
    Automation = ordered(Automation, levels = c("Full", "Partial", "No"))
  ) %>%
  ggplot(aes(x=NIE, y=fracVMTIncr, size=Automation, color=Fuel, shape=Use, alpha=0.5)) +
  geom_point() +
  ggtitle("Change in Vehicle Intensity and VKT by Vehicle Sub-category")
```

```{r echo=echoWorking}
temp %>% 
  mutate(
    # Fuel = ordered(Fuel, levels = c("Gas", "BEV", "PHEV")),
    Automation = as.factor(Automation),
    Automation = ordered(Automation, levels = c("Full", "Partial", "No"))
  ) %>%
  group_by(Automation, Fuel, Use) %>%
  ggplot(aes(x=NIE, y=EnergyUse, size=Automation, color=Fuel, shape=Use, alpha=0.5)) +
  geom_point() +
  ggtitle("Change in Vehicle Intensity and Energy Use by Vehicle Sub-category")

#table(temp$VKEratio, temp$Fuel)
#table(temp$NIE, temp$Use)
```
...
temp %>% 
  mutate(
    ...
    Automation = as.factor(Automation),
    Automation = ordered(Automation, levels = c("Full", "Partial", "No")),
    shrXEnergyUse = EnergyUse * F_A_Shares * U_Shares # share-weighted Energy Use change
  ) %>%
  group_by(Automation, Fuel, Use) %>%
  ggplot(aes(x=NIE, y=shrXEnergyUse, size=Automation, color=Fuel, shape=Use, alpha=0.5)) +
  geom_point() +
  ggtitle("Change in Intensity and Share-weighted Energy Use by Vehicle Sub-category")
...
```{r echo=echoWorking}
# hist(temp$VKEratio, breaks = 20)
g <- temp %>%
  mutate(
    # Fuel = ordered(Fuel, levels = c("Gas", "BEV", "PHEV")),
    Automation = as.factor(Automation),
    Automation = ordered(Automation, levels = c("No", "Partial", "Full"))
  ) %>%
  ggplot(aes(VKEratio)) + scale_fill_brewer(palette = "Spectral") + 
  geom_histogram(aes(fill=Automation), 
                   binwidth = .02, 
                   col="black", 
                   size=.1) +  # change binwidth
  labs(title="Histogram of VKT Effect Over Cases", 
       subtitle="VKT Effect Ratio across Vehicle Automation")  
g
```
...
temp %>%
  ...
  ggplot(aes(fracVMTIncr)) + scale_fill_brewer(palette = "Spectral") + 
  geom_histogram(aes(fill=Automation), 
                   binwidth = .02, 
                   col="black", 
                   size=.1) +  # change binwidth
  labs(title="Histogram of Fractional VKT Increase Over Cases", 
       subtitle="VKT Effect across Vehicle Automation")  
```
...
temp %>%
  ...
  ggplot(aes(EnergyUse)) + scale_fill_brewer(palette = "Spectral") + 
  geom_histogram(aes(fill=Automation), 
                   binwidth = .02, 
                   col="black", 
                   size=.1) +  # change binwidth
  labs(title="Histogram of Fractional Energy Change Over Cases", 
       subtitle="Energy Use Effect across Vehicle Automation")  

# hist(temp$EnUEratio, breaks = 20)
```

**Note** these EnergyUse changes are already share-weighted, hence appear smaller.
Net Energy Use will be the _unweighted_ sum of the term, for a given scenario.

#### Checks on Shares: that the combined share weights add up to 1.0 as expected

```{r testShareAddingUp, echo=echoWorking, include=echoTests}
# Aside: Test summation of disaggregated shares
temp <- CaseAssumpsResults %>%
  mutate(Joint_FAU_Share = F_A_Shares * U_Shares) %>%
  group_by(DemScen, TechScen, ElasCase, VC, Year) %>%
  summarize(TotShare = sum(Joint_FAU_Share)) %>%
  ungroup()
...
temp = CaseAssumpsResults %>% mutate(testTEST_NIE = abs(I_deltaCAV - NIE)) %>% 
  select(VC, DemScen, ElasCase, TechScen, Automation, Fuel, Use, I_deltaCAV, NIE, testTEST_NIE, 
         old_C_Fuel, C_Fuel, old_C_Vcap, C_Vcap, old_C_VNonFC, C_VNonFC, old_C_Time, old_C_Total, C_Total,
         old_fracVMTIncr, fracVMTIncr,
         old_EnergyUse, EnergyUse)
temp %>% 
...
temp %>% # Note: Expect no alteration of energy use for Full Auto Private Gas vehicles
  filter(
    Automation == "Full",
    Fuel == "Gas",
    Use == "Private"
  ...
  mutate(
    testNIE = (NIE - I_deltaCAV),
    testEnergy = (EnergyUse - old_EnergyUse),
    testfracVMTIncr = (fracVMTIncr - old_fracVMTIncr),
    # testTEST_NIE = (testTEST_NIE  >1E-10)) %>% pull(testTEST_NIE) %>% sum()
    testTEST = (abs(testEnergy) > 1E-6)
  ) %>% 
  # filter(testTEST) %>% 
  select(testTEST, testNIE, testfracVMTIncr, testEnergy) -> testEnergy

cat(
  "\nCheck: Private, Gas CAVs - Maximum observed absolute difference in EnergyUse, disagg vs agg, is",
  paste(max(abs(testEnergy$testEnergy))), ".\n"
)

cat("Check: Private, Gas CAVs - Statistics for observed differences are: \n")
summary(testEnergy)
```

```{r echo=echoWorking}
# testEnergy %>% filter(testTEST == T)

# Look at variations in some cost assumptions:
# CaseAssumpsResults %>% select(DemScen, C_deltaVoTT) %>% group_by(DemScen) %>% table()
# CaseAssumpsResults %>% select(DemScen, C_VNonFC) %>% group_by(DemScen) %>% table() # look for CapCost effect
# VTCostAlt %>% filter(CostCat == "VehCapCost") %>% group_by(VehType) %>% select(VehType, VTCost_shr) %>% table()
```
...
CaseAssumpsResults <- CaseAssumpsResults %>% 
  select(names(CaseEffects), everything()) %>%
  select(-old_C_Time, everything(), old_C_Time) # move old_C_time to end for comparability
```
...
5.2 Define Combined Scenarios of Interest and Develop Functions to Re-aggregate Over Vehicle Sub-types
------------------------

### 5.2.1 Define Combined Scenarios of Interest

Define the Combined Scenarios `CombScen`, in terms of name, `Year`, vehicle class `VC`,
...
and VKT demand elasticity `ElasCase`.
...
Select Predefined combinations `CombScen` of Tech and Demand scenarios from the full set of cross-combinations,
...
WantedCaseAssumpsResults <- CaseAssumpsResults %>%
  # filter(DemScen %in% unique(CaseAssumpsResults$DemScen)) %>%
...
Tabulate assumptions and results for selected Combined Scenarios in `WantedCaseAssumpsResults`.

### 5.2.2 Develop Functions to Re-aggregate Over Vehicle Sub-types (Demand Segments)

We can aggregate Case results across Automation type, Fuel type, and shared Use,
based on disaggregated effects and shares by vehicle sub-type $s_{x,FA}$ and $s_{x,U}$
for scenario *x* associated with each travel demand segment.

```{r defFn_aggregateSelectedCaseAssumpsResults, echo = echoWorking}
aggregateSelectedCaseAssumpsResults <- function(case_assumps_results, # df with precalc assumps & results
                                                comb_subset, # df case subset to be aggregated (CombScen, Year, ElasCase, VC, TechScen, DemScen)
                                                # optionally can call for particular vehicle sub-cat
                                                use_set = c("Private", "Shared"),
                                                fuel_set = c("Gas", "PHEV", "BEV"),
                                                auto_set = c("No", "Partial", "Full"),
                                                grouping_var_list = # list of variables that identify individual cases/scenarios
                                                  list("CombScen", "DemScen", "TechScen", "ElasCase", "VC", "Year")
) {
  # Aggregate Case assumps and results over some or all vehicle sub-types (Automation, Fuel, and Use), by share-weights
  # Returns df with 1 row per Scenario, 6 Scenario selection/ID variables, and 18 intermed and final results vars.
  #   Can be used to determine aggregates for specified subsets of A, F, U.
  
  case_assumps_results %>%
    # filter(Year==CurrYear) %>%
    right_join(comb_subset, # join assumps_results cols for cases matching rows in comb_subset
               by = c("Year", "ElasCase", "VC", "TechScen", "DemScen") # case selectors
    ) %>%
    mutate(
      Joint_FAU_Share = F_A_Shares * U_Shares,
      totshare = Joint_FAU_Share, # will be needed if aggregating sub-segments
    ) %>%
    mutate_at( # convert all result vars to versions weighted by joint FAU share
      vars( # mult each of these variables by joint share
        fracVMTIncr, NIE, EnergyUse,
        C_Fuel, C_Time, C_Vcap, C_VNonFC, C_Total,
        ElasVKT, RidePoolAdjFrac, USDmult, mI_A, mI_C, mI_FA, mI_FB, mD_U, mCT_A, mCC_A
      ),
      funs(. * Joint_FAU_Share)
    ) %>%
    # Allow aggregation of sub segments of the market
    filter(Automation %in% auto_set) %>%
    filter(Fuel %in% fuel_set) %>%
    filter(Use %in% use_set) %>%
    # group_by list of variables that are associated with invidual cases/scenarios (may vary)
    group_by_(.dots = grouping_var_list) %>% # see https://datascience.blog.wzb.eu/2016/09/27/dynamic-columnvariable-names-with-dplyr-using-standard-evaluation-functions/
    summarise_at(
      # construct vehicle share-weighted average of these vars for each "scenario"
      vars(
        fracVMTIncr, NIE, EnergyUse,
        totshare, F_A_Shares, U_Shares,
        C_Fuel, C_Time, C_Vcap, C_VNonFC, C_Total,
        ElasVKT, RidePoolAdjFrac, USDmult, mI_A, mI_C, mI_FA, mI_FB, mD_U, mCT_A, mCC_A
      ),
      .funs = sum # sum of share-weighted variables
    ) %>%
    ungroup() %>%
    ...
    mutate_at(
      vars(
        fracVMTIncr, NIE, EnergyUse, F_A_Shares, U_Shares, 
        C_Fuel, C_Time, C_Vcap, C_VNonFC, C_Total,
        ElasVKT, RidePoolAdjFrac, USDmult, mI_A, mI_C, mI_FA, mI_FB, mD_U, mCT_A, mCC_A
      ),
      funs(. / totshare)
...
resultVars <- c("fracVMTIncr", "NIE", "EnergyUse")
```

```{r defFn_selectCaseAssumpsResults, echo=echoWorking}
selectCaseAssumpsResults <- function(case_assumps_results, # df with precalc assumps & results
                                                comb_subset, # df case subset to be aggregated (CombScen, Year, ElasCase, VC, TechScen, DemScen)
                                                # optionally can call for particular vehicle sub-cat
                                                use_set = c("Private", "Shared"),
                                                fuel_set = c("Gas", "PHEV", "BEV"),
  ...
  # Select Case assumps and results for vehicle sub-types (Automation, Fuel, and Use), by share-weights
  # Returns df with disagg Scenarios, 6 Scenario selection/ID variables, and 18 intermed and final results vars
  case_assumps_results %>%
    # filter(Year==CurrYear) %>%
    right_join(comb_subset, # join assumps_results cols for cases matching rows in comb_subset
      by = c("Year", "ElasCase", "VC", "TechScen", "DemScen") # case selectors
...
aggregateSelectedCaseEffects <- function(case_effects, comb_subset) {
  # Aggregate Case key result vars over all vehicle sub-types (Automation, Fuel, and Use), by share-weights
  ...
  case_effects %>%
    ...
    mutate( # to share-weighted values
      EnergyUse = EnergyUse * Joint_FAU_Share, # no longer already been weighted by joint shares of veh sub-types
      fracVMTIncr = fracVMTIncr * Joint_FAU_Share,
      NIE = NIE * Joint_FAU_Share
    ) %>%
    ...
    summarize(
      ...
      totshare = sum(Joint_FAU_Share) # this is a check
    ) %>%
    ungroup() # %>%
  # should not need to divide by totshare, because this function applies to all vehicle sub-types
}
...
# Create df with row per Scenario, 6 Scenario selection/ID variables, and 18 intermed and final results vars
AggAssumpsResults <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResults, 
  select(CombScen_Combinations_subset, -Notes) # defines scenarios wanted (DemScen, TechScen, Year, VC)
  )

AggAssumpsResultsDAVT <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResultsDAVT, 
  select(CombScen_Combinations_subset, -Notes) # defines scenarios wanted (DemScen, TechScen, Year, VC)
  )

temp <- AggAssumpsResults %>%
  mutate(Case = "AgVT") %>%
  filter(VC == "LDV") # %>%
  # select(-)
```
...
# Aggregate Case results across Automation type, Fuel type, and shared Use
# Create df with row per Scenario, 6 Scenario selection/ID variables, and 4 final results vars (incl TotShare)
...
testAggregation <- CaseWantedAggEffects %>%
  mutate(
    ...
    testEDiff = EnergyUseAlt - EnergyUse,
    testERatio = ifelse((abs(EnergyUseAlt)<1E-10) & (abs(EnergyUse)<1E-10), 1.0, round(EnergyUseAlt/EnergyUse, 6))
  ) %>%
  select(-Year)
cat(
  ...
  sum(abs(testAggregation$testEDiff), na.rm = T), ".\n"
)
cat(
  "Check: Calculated aggregate `EnergyUse` changes are",
  sum(abs(testAggregation$EnergyUse), na.rm = T), ".\n"
)
cat(
  "Check: Statistics on the ratio of aggregate `EnergyUse` the product of aggregate energy intensity `NIE` and `fracVMTIncr` (due to composition effects) are:\n")
summary(testAggregation$testERatio)
```

### 5.2.3 Compare Final Scenario Results Table for 2050
...
SR_test <- SR_test %>%
  gather(key = varname, value = testValue, resultVars) %>%
  filter(VC != "Tot") # ignore Tot case for new
CWE_test <- CaseWantedAggEffects %>%
  gather(key = varname, value = calcValue, resultVars) %>%
...
*Note: This backward check does not yet work because the matching scenarios and scenario IDs have not been
identified.*
...
CaseWantedAggEffects %>%
  ...
  select(DemScen, TechScen, ElasCase, VC, Intensity, VKT, Energy) %>%
  kable(caption = "Table: Summary Scenario Results, Fractional Changes", digits = 4)
```


5.3 Bar Chart Representation of Results
------------------
...
barPlotScenarioSetIntVMTEnergy <- function(thisdata, 
  ...
  thisdata %>%
    ...
    gather(key = variable, value = calcValue, c("Intensity", "VMT", "Energy")) %>%
    # rename variable names, establish as factor in correct order for bar plot
  ...
  # Faceting
    ...
    # } +
    geom_bar(stat = "identity") +
    facet_wrap(~DemScen) + # use DemScen instead of CombScen, because names clearer
...
```{r defFn_tableScenarioSetIntVMTEnergy, echo=echoWorking}
tableScenarioSetIntVMTEnergy <- function(
  CWAggAssumps,
  # title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
  tablecaption = "Private, Non-Automated Vehicles") {
  CWAggAssumps %>% 
    mutate(
      tstEnergyUse = (1+fracVMTIncr)*(1+NIE)-1 # calc approximate Energy Use effect for check
    ) %>% # report cases, key results, total share
    select(CombScen, VC, fracVMTIncr, NIE, EnergyUse, tstEnergyUse, totshare) %>%
    kable(caption = tablecaption, digits = 4)
}

```

```{r defFn_tableScenarioSetIntVMTEnergyVsO, echo=echoWorking}
tableScenarioSetIntVMTEnergyVsO <- function(
  # same as tableScenarioSetIntVMTEnergy() except report Pooled_o
  CWAggAssumps,
  # title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
  tablecaption = "Private, Non-Automated Vehicles") {
  CWAggAssumps %>% 
    ...
    select(CombScen, VC, Pooled_o, fracVMTIncr, NIE, EnergyUse, tstEnergyUse, totshare) %>%
    kable(caption = tablecaption, digits = 4)
}
...
# call the bar plot function for all LDV data
barPlotScenarioSetIntVMTEnergy(AggAssumpsResults,
                               title = "Fig: Energy-Intensity, VMT, and Energy Scenarios", 
                               subtitle = "LDV Mix with Ride-hailing, Excludes Ride-Pooling") 
```


```{r}
tableScenarioSetIntVMTEnergy(
  AggAssumpsResults, 
  tablecaption = "LDV Mix, Shared mobility Excludes Ride-Pooling")
```

The results for Fully-Automated Private Vehicles, Gasoline drivetrain, are more dramatic:

```{r testCasesForSelectedSegments, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
AggAssumpsResultsPrivateCAV <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResults, 
  select(CombScen_Combinations_subset, -Notes),
  auto_set = c("Full"), # must call the aggregation fn to assure rescaled on share of these
  fuel_set = c("Gas"),
  use_set = c("Private")
  )

barPlotScenarioSetIntVMTEnergy(AggAssumpsResultsPrivateCAV,
                               title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
                               subtitle = "LDV Private Gaso CAVs")
...
```{r}
tableScenarioSetIntVMTEnergy(
  AggAssumpsResultsPrivateCAV, 
  tablecaption = "LDV Private Gaso CAVs")
```

#### Results for Electrified Vehicles, BEV or PHEV drivetrain:

```{r testCasesForSelectedSegments2, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
AggAssumpsResultsEVs <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResults, 
  select(CombScen_Combinations_subset, -Notes),
  #auto_set = c("Full"),
  fuel_set = c("PHEV", "BEV"),
...
barPlotScenarioSetIntVMTEnergy(AggAssumpsResultsEVs,
                               title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
...
```{r}
tableScenarioSetIntVMTEnergy(
  AggAssumpsResultsEVs, 
  tablecaption = "LDV BEVs and PHEVs, Excludes Ride-Pooling")
```
...
#### Results for Shared Vehicles, All control and drivetrains:

```{r testCasesForSelectedSegments3, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
AggAssumpsResultsShared <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResults, 
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  # fuel_set = c("Gas", "PHEV", "BEV"),
...
barPlotScenarioSetIntVMTEnergy(AggAssumpsResultsShared,
                               title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
                               subtitle = "Ridehailing Only, Excludes Ride-Pooling")
```

```{r}
tableScenarioSetIntVMTEnergy(
  AggAssumpsResultsShared, 
  tablecaption = "Ridehailing Only, Excludes Ride-Pooling")
```

#### Results for Private Non-Automated Vehicles, All drivetrains:

```{r testCasesForSelectedSegments4, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
AggAssumpsResultsPrivateMV <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResults, 
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  auto_set = c("No"),
...
barPlotScenarioSetIntVMTEnergy(AggAssumpsResultsPrivateMV,
                               title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
                               subtitle = "Private, Non-Automated")
```

```{r}
tableScenarioSetIntVMTEnergy(
  AggAssumpsResultsPrivateMV, 
  tablecaption = "Private, Non-Automated Vehicles")
```

```{r testCasesForSelectedSegments5, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
AggAssumpsResultsPrivateGasMV <- aggregateSelectedCaseAssumpsResults(
  CaseAssumpsResults, 
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  auto_set = c("No"),
...
barPlotScenarioSetIntVMTEnergy(AggAssumpsResultsPrivateGasMV,
                               title = "Fig: Energy-Intensity, VMT, and Energy Scenarios",
                               subtitle = "Private, Gas, Non-Automated")
```

```{r}
tableScenarioSetIntVMTEnergy(
  AggAssumpsResultsPrivateGasMV, 
  tablecaption = "Private, Gas. Non-Automated Vehicles")
```

```{r}
CaseAssumpsResults %>%
  selectCaseAssumpsResults(
    select(CombScen_Combinations_subset, -Notes),
    # auto_set = c("No", "Partial", "Full")
    auto_set = c("No"),
    # fuel_set = c("Gas", "PHEV", "BEV"),
  ...
  filter(VC=="LDV") 
# %>%
  #select(DemScen, TechScen, VC, Automation, Fuel, Use, NIE, fracVMTIncr, EnergyUse, Joint_FAU_Share, totshare)
...
#### Define elasticities:
- $elas_{C_T,o}$ = `Elas_VoTT_wrt_Pooling`, elasticity of perceived Cost of Travel time $VoTT$, where $C_T = VoTT * \pi$, w.r.t. pooling occupancy. This is an assumption regarding how occupancy (pooling) increases time cost VoTT per hour.
- $elas_{K_V,o}$ = `Elas_TourLength_wrt_Pooling`, the elasticity of Vehicle tour length $K_V$ (for mult pooled trips) w.r.t. occupancy;
- $elas_{K_P,o}$ = `Elas_RiderDetour_wrt_Pooling` elasticity of Passenger trip length $K_P$, in _time_ w.r.t. occupancy and pooling
...
scenDisaggData <- WantedCaseAssumpsResults %>%
  filter(
...
For Base assumptions and Alternative scenario assumptions, consider a range of vehicle occupancy levels *o*, where $o = 1.0$ corresponds to the current averable level of vehicle occupancy.^[Historically, U.S. average passenger car occupancy in the U.S. has been steady at about 1.5 to 1.6, with van occupancy slightly over 2 (Davis, Williams, & Boundy 2017, Fig 8.1)]
...
Start with Base cost assumptions for the total travel cost per mile
for a conventional (gasoline, manually-driven, privately operated) vehicle, 
and apply the cost adjustment ratios from the  **aggregate** (???) scenario data.

```{r getCurrTotVehTravelCost, echo=echoWorking}
# Need base total vehicle cost, which is the reference point for all relative cost estimations
C_tot_base <- extractVehTotalTravelCost(VTCostBase, VehTypeWanted, "Total" ) # total base travel cost, c/mi
```

```{r defFn_addMainCostsPerVehMi, echo=echoWorking}
addMainCostsPerVehMi <- function(df_costshr, curr_C_tot_base) {
  # for df with relative component costs of vehicle travel, use Total Cost/VMT to calc main components in c/mi (cpm)
  # return the df with principal cost components, from cost share to monetary values in c/mi
  df_costshr %>% 
    mutate(
      C_F = curr_C_tot_base * C_Fuel, # fuel costs, c/mi
      C_T = curr_C_tot_base * C_Time, # time costs [c/mi]
      # C_K = VehCapCost,
      C_tot = curr_C_tot_base * C_Total
...
scenAggData <- addMainCostsPerVehMi(scenAggData, C_tot_base)

# construct principal cost components, from cost share to monetary values in c/mi
scenDisaggData <- addMainCostsPerVehMi(scenDisaggData, C_tot_base)

```

```{r}
scenAggDataDAVT <- AggAssumpsResultsDAVT %>%
  filter(
    DemScen %in% DemScenWanted,
    ElasCase == ElasCaseWanted,
    # VehType == VehTypeWanted
...
# construct principal cost components, from cost share to monetary values in c/mi
scenAggDataDAVT <- addMainCostsPerVehMi(scenAggDataDAVT, C_tot_base)

scenCostDataDAVT <- scenAggDataDAVT %>%
  select(DemScen, C_F, C_H, C_T, C_V) %>%
  # Get the travel cost components for scenarios of interest.
  filter(DemScen %in% DemScenWanted) # redundant

scenCostDataDAVT %>% kable(
...
The vehicle portion of travel costs (fuel and capital/operating costs, not time costs) can be divided among the occupants, so that pooling (increased occupancy) reduces cost-per-passenger mile while not altering vehicle costs per vehicle mile.^[While a non-automated, manually driven ride-hailing vehicle would have labor costs related to time, these are similar to other vehicle operating costs, are proportional to distance for a given average speed, and can be divided among the occupants like all other vehicle costs.] Pooling has an uncertain and complex implication for the total VMT, since pooled riders typically do not share origins and destinations, and the net change in VMT depends on pooled-ride routing efficiency.
...
Consider a range of average occupancy levels for ride-hailing
vehicles,^[These occupancy-base changes to costs, VMT, and PMT only apply to the ride-hailing, or "shared" vehicle Use segment.] $o$, from
`r min(occupancyEffects$o)`} to `r max((occupancyEffects$o))`. 
Ride-hailing vehicles incur some zero-occupancy travel from "deadheading" and repositioning between rides,
so average occupancy levels can be below 1.0 in cases of low/no ride pooling. 

Variable vehicle travel cost per pass-mi [c/pass-mi]
$C_{Vpmt}(o) = C_V / o$ 
    
Time-cost multiplier due to disutility of pooling [unitless]
$\rho(o) = o^{\epsilon_{VoTT,o}}$

Time-cost per passenger mile adjusted by the disutility of occupancy [c/pass-mi]
$C_{Tpmt}(o) = C_T \rho(o)$

Assumed pace, (constant over o) [hr/veh-mi]
$\pi = 1 /S_{ref}$

Hourly Time-cost per passenger-hour (for info only) [$/pass-hr]
$C_{Tdph} = C_T / (100 * \pi)$

Total cost per passenger mile traveled  [c/pass-mi]
$C_{Mpmt} = C_{Vpmt} + C_{Tpmt}$

Adjust pace (travel time per mile) for automated travel cases, if ReductPaceAutomation factor != 1.0
$\pi = \pi \cdot ReductPaceAutomation, \pi)$


```{r calcOccupancyEffects, echo=echoWorking}
# dplyr fn for full-join with no common vars (all combinations)
# this gives a df with observations for all levels of occupancy o for each row in scenAggData (each value of DemScen).
...
# TravTimeCostBasePars["LDV.average", "speed"]
...
occupancyEffects <- occupancyEffects %>%
  mutate(
    ...
    Pi = 1 / TravTimeCostBasePars["LDV.average", "speed"], # assumed pace, (constant over o) [hr/veh-mi]
...
Pooling, or increased occupancy, reduces costs by sharing vehicle operating costs over all passengers, and reducing total VMT as pooling contracts the distance the vehicle must travel to completing a tour of multiple trips (considered later). It can also increase costs to passengers by increasing the costs per unit travel time (relative to solo travel), and by adding some detour distance to a passenger's travel. We account for these changes, and the resulting demand response through the elasticity of travel demand with respect to full, generalized cost.

Obviously the response of travelers will be heterogenous and situation-dependent, but the aggregate response can be estimated. Other than direct time costs borne by passengers in terms of delays from detours or pickup/dropoff transaction, the disutility of sharing space in pooled rides is represented by an adjustment (increase) in the cost per unit travel time.
...
PMT, pass-mi (normalized w.r.t Base scenario, base Occupancy)
$$K_P(o) = K_V(o=1) \left ( \frac {C_{Mpmt}(o)} {C_{Mpmt}(o_{ref})} \right)^{\epsilon_{K_P,C}}$$


VMT (normalized) (defining relation for Occupancy "o", but only true for o=PoolingLevel if perfect routing efficiency)

$$K_V(o) = K_P(o) / o$$

$$E(o) = E(o_{ref}) \cdot K_V / K_V(o_{ref})$$
...
#   C_t = rho_o *C_T/Pi,            # time cost per hr, adjusted for occupancy [cents/pass-hr]
...
occupancyEffects <- occupancyEffects %>%
  ...
  mutate( # convert from fractional change to multipliers
    ElasPKT = ElasVKT, # treat as the same, since ElasVKT specified for base, non-pooled travel
    NIE_oref = NIE, # *** UNUSED ***
    Energy_oref = 1 + EnergyUse, # computed value from case at reference o=1.0
    VMT_oref = 1 + fracVMTIncr # computed value from case at reference o=1.0
  )
...
#   ) %>%
#   select(Pooled_o, VMT, VMT_oref, VMT_test, Energy, Energy1, Energy_oref, Energy_test)
...
```{r defFn_plotOccupancyEffects, echo=echoWorking}
plotOccupancyEffects <- function(occupancy_df,
                                 curr_title = "Effect of Pooling on PMT and Energy (VMT) - For Ride Hailing",
                                 curr_subtitle = "Case X") {
  occupancy_df %>%
    ...
    # scale_y_continuous(breaks=c(0.7, 0.9, 1.0, 1.1, 1.3, 1.5, 1.7, 1.9)) +
    scale_y_continuous(limits = c(0.70, 1.90), breaks=seq(0.70, 1.9, .2)) +
...
```
Plot the rise in relative PMT and decline in relative VMT (relative to Base) with different levels of pooling (average occupancy of ride-hailing vehicle.)

NOte that for the Base scenario, PMT and E cross at (o=1.0), i.e. E_Base(1) = 1.0 = PMT_Base(1.0).
As expected, E_Bhigh(1.0) and E_Chigh(1.0) stand relative to E = 1.0 the same way as reported in the prior bar charts of Intensity, VMT, and Energy.

This chart hightlights how changing pooling $o$ can change PMT and VMT, and thus E in proportion, given the vehicle energy intensity associated with each scenario.
The vehicle energy intensity (MJ/km, or gal/mi) is assumed invariant with the level of pooling.

```{r plotOccupancyEffects, echo=echoWorking, dpi=300}
plotOccupancyEffects(occupancyEffects,
                     curr_subtitle = "Ideal routing efficiency, no disutility to sharing space. Demand rebound to cost.")
# occupancyTravelEffects %>% filter(o == 1.0)

```

We can confirm that the scenario results match the calculation of occupancy effects for the case of referenced occupancy, $o = 1.0$.
  ...
  bind_rows(
    AggAssumpsResults %>% 
      ...
      mutate(
        VMT = 1 + fracVMTIncr,
        Energy = 1 + EnergyUse,
        PMT = VMT  # valid for base occupancy level, o=1.0
      ) %>%
      select(Source, DemScen, VMT, Energy, PMT)
  )
```
...
The ability of pooling to reduce VMT is currently summarized in an elasticity of "tour" length (the VMT needed to complete a set of trips) with respect to occupancy *o*:

$$L_{vehicle-tour} = L_{tour,r} o^{\epsilon_{K_V,o}}$$

$$L_{trip-passenger} = L_{trip,r} o^{\epsilon_{K_P,o}}$$

These elasticities represent different logistical possibilities for pooled ride-hailing travel. 
The summarize the economic opportunities for reducing VMT through pooling.
...
$$E(o) = VMT(o) \cdot I_{DemScen}$$
This is essentially an assumption of perfectly efficient routing of pooled ride-hail trips. (???)
??? Are we at risk of confusing "occupancy" $o$ and (target) pooling or ridership level $r$?

If we define _effective_ occupancy in terms of $o = PMT/VMT$:

1. Where PMT is original/shortest distance for the passenger trip.
2. o = 1 is achieved for sequential invidual rides with zero deadheading (each pickup is colocated with teh last dropoff)
3. o < 1 occurs for individual rides with some deadheading/repositioning;
4. We can speak of the elasticity *e* of tour length w.r.t. to _ridership_ *r*, which is the "target" pooling level.
    - $o(r) = o_1 r^e$
    - $VMT(PMT, o) = PTM/o$
    - $PMT/VMT(PMT, o) = o$
    - $VMT(PMT, r) = PMT/o(r)$
    - $PMT/VMT(PMT, r) = o(r) = o_1 r^e$
    
Now suppose that there is also a tour routing *penalty* (for route circuity/repositioning to pool a rider) that increases with $o$

- $E(o) = VKT(PMT, o) \cdot I$
- For fixed I, 
- $E(o)/E(o_1) = VKT(PMT, o) / VKT(PMT, o_1) = o_1 /o(r)$
...
- $E(o) = E_b \cdot o^{\epsilon_{K_V, o}}$

for varous values of the elasticity $\epsilon_{K_V, o}$ for Tour distance $K_V$
...
AltVMT %>%
  ...
  kable(digits = 4, caption = paste0("Table: Energy Use Relative to Reference (", test_DemScen, "), by Occupancy and Route Length Elasticity"))


# # Test junk
# # Confirm: Energy Intensity is constant for each DemScen
# occupancyEffects %>% select(DemScen, o, VMT, Energy) %>% 
#   mutate(Icalc = Energy/VMT) %>% 
#   select(-o, -VMT, -Energy) %>% 
#   group_by(DemScen) %>% 
#   table()
# # Confirm: PMT/VMT = o
# occupancyEffects %>% select(DemScen, o, VMT, PMT, Energy) %>% 
#   mutate(oImpliedOvero = ((PMT/VMT)/o)) %>% 
#   select(-o, -VMT, -PMT, -Energy) %>% 
#   group_by(DemScen) %>% 
#   table() 
...
# get the energy intensity of the scenario, at reference pooling level (o=1)
...
This aggregate economic approach allows exploration of variant cases around established detailed mobility scenarios. The variant cases can account account for, e.g. different levels of ride pooling, and the traveler response to economic conditions and incentives including the added delay and potential disutility associated with ride pooling, as well as changes in travel cost on a per vehicle or per passenger mile basis. 

An incremental cost per VMT on all vehicles would discourage all road travel (or TNC travel if only on TNCs), but also encourage substitution between non-pooled ("single passenger") travel and pooled rides. [Not modeling this substitution at present.] Alternatively, a subsidy of pooled riding would have a different effects on the mix and level of travel. The framework allows exploration of VMT, PMT and energy effects, although further data and research is needed to benchmark the direct and substitution responsiveness (elasticities).

### 6.3.1 Estimate Scenario effects, with economic responses to ride pooling
...
Passenger time cost when pooling is affected by two factors: the increased trip travel time caused by any detours or delays in picking up or dropping off other passengers; and the disutility cost associated with sharing ride space. We capture both of these through elasticities with respect to pooling level $o$. Note that passenger Cost of Travel Time in a ride service vehicle ("shared" use mode) with single occupancy is likely to be well below the base cost of travel time when driving $C_{T,ref}$. This time cost reduction from ride-hailing rather than driving a private vehicle is $ \rho_{VoTT, shared}$. But increased occupancy from pooling can erode this cost advantage. Combining the effects of pooling on ride-hailing trip VoTT and duration

$$C_{T,pmt} = C_{T,ref} \cdot \left ( \rho_{VoTT, shared} \cdot o^{\epsilon_{C_T,o}} \right ) \cdot o^{\epsilon_{L_P,o}}$$ 
Passenger's share of the vehicle operating cost $C_V$ changes with pooling also in two ways: pooling decreases the TNC vehicle's tour length (VMT) needed to complete a set of passenger trips, thus the ratio of PMT to VMT is improved relative a non-pooling service; and the costs of VMT are shared over a larger number ($o$) of riders. 

We denote the (aggregate, time and region-dependent) elasticity of TNC vehicle tour length $L_V$ with respect to pooling level as $\epsilon_{L_V,o}$.
Alt notation: $\epsilon_{K_V,o}$

#### Pooling effect on VMT per PMT

**Note**: Still working on best formuation. This is a redef of $\epsilon_{L_V,o}$ from above, which was $VMT(o) = (PMT/o) \cdot o^{\epsilon_{L_V,o}}$, in that $VMT_{o=1}$ below seems to include deadheading from individually served trips and $\epsilon_{L_V,o}$ below includes the $1/o$ factor, i.e. starts from -1 rather than 0.

$$VMT(o) = VMT_{o=1} \cdot o^{\epsilon_{L_V,o}}$$
where at best the passenger trips are coincident ($\epsilon_{L_V,o} = -1.0$, no detours, side-connections or repositioning needed for pooling). At worst there is no savings in tour length (otherwise pooling has no merit.)
$$-1.0 < \epsilon_{L_V,o} < 0.0$$
By scenario assumption, in the non-pooled Shared (ride-hailing) Use case there is some degree of TNC passengerless travel (travelling to service region, repositioning, deadheading), leading to an expansion  $\rho_{VMT,shared}$ of VMT over delivered PMT. I.e. for the reference level of pooling, $o=1$:^[Note that the ride-hailing VMT expansion expansion  $\rho_{VMT,shared}$ in the SMART Mobility scenario reference case may not involve zero pooling, but it is not explicitly indicated. We define this reference as the $o=1$ case, since the effect of any pooling on VMT is including in the scenario reference VMT multipliers for ride-hailing Shared use.]

$$VMT(o=1) = PMT(o=1) \cdot \rho_{VMT,shared} $$
Here $\rho_{VMT,shared}$ represents the multiplier for deadhead and repositioning travel for non-pooled ride-hailing, with a range of estimates of $\rho_{VMT,shared}$ for current TNCs range near $1.1 < \rho_{VMT,shared} < 1.3$ for comparatively efficient operations, and much higher estimated values in some service areas [see Lit.] 

For this study, alternative values are taken for the Ride-hailing (Shared) Use added VMT from the DOE SMART FY19 Scenarios, ranging from a low of 1.0 (Scenario 1 "Sharing is Caring") a mid-value of 1.3 (Baseline, and C_Low) to 1.6 (Scenario C_High, "All About Me"). We take these scenario Shared Use VMT multipliers as the starting points for the impact of pooling on VMT and PMT.
...
$$C_{V,pmt} = (C_{V,ref} +C_{road}) \cdot \frac {VMT(o)}{VMT(o=1)}  = (C_{V,ref} +C_{road}) \cdot \frac {PMT(o) o^{\epsilon_{L_V,o}}}{PMT(o=1)} =  (C_{V,ref} +C_{road})  \cdot o^{\epsilon_{L_V,o}}$$ 
(what happened to $\rho_{VMT,shared}$? Does it cancel in this ratio, which is for ride-hail travel?)

The vehicle cost per (ride-hailing?) passenger mile scales with pooling *o* along with VMT:

$$\frac {C_{V,pmt}(o)}{C_{V,pmt}(o=1)} =  (1 + \frac {C_{road}}{C_{V,ref}}) o^{\epsilon_{L_V,o}}$$ 
Consider now the total costs per (ride-hail?) passenger travel mile, which is composed of both vehicle operation cost and passenger time cost. (We subsume time-related vehicle operation costs, such as the driver for non-automated ride-hailing, in vehicle operation costs $C_V$ for now.) The passenger time cost varies with pooling level differently than vehicle operating cost, i.e. according to $o^{\epsilon_{C_T,o}} \cdot o^{\epsilon_{L_P,o}}$ rather than $o^{\epsilon_{L_V,o}}$.
...
PMT demand for ride-hailing travel responds to the changing costs associated with that travel.
$$
...
ReductPaceAutomation <- 1 #  PaceReduct_A, reduction factor for the pace (hr/km), or quantity of travel time
```
Explore implications of road charge $C_{road}$ at scenario aggregate level.

Warning: this treatment at the aggregate vehicle class level essentially implies all demand is for shared, or ride-hailed mobility.


```{r defFn_calcPoolingEnergyCases, echo=echoWorking}
calcPoolingEnergyCases <- function(dfVMT, # this case df must be a set of scenarios with also: C_road, o
                                   curr_Elas_VoTT_wrt_Pooling,
                                   curr_Elas_TourLength_wrt_Pooling,
                                   curr_Elas_RiderDetour_wrt_Pooling
                                   ) {
  # Calculates PMT, VMT, and Energy relative outcomes (multipliers) based on occupancy o and C_road,
  #   for a range of cases in dfVMT, one case per row.
  # First calculates costs (Vehicle, Time, Total) per passenger mile traveled C_Vpmt, C_Tpmt, C_Mpmt
  # requires external parameters: TravTimeCostBasePars
  dfVMT <- dfVMT %>%
    mutate(
      # time cost adjustment omits time reduction for ride-hail ($\rho_{VMT,shared}$)
      #  because this approach considers relative cost for ride-hail travel
      PoolTimeCostInflation = o^curr_Elas_VoTT_wrt_Pooling, # Time-cost multiplier due to disutility of pooling [unitless]
      PoolTourContraction = o^curr_Elas_TourLength_wrt_Pooling,
      RideDetourExpansion = o^curr_Elas_RiderDetour_wrt_Pooling,
      # C_road must be specified   # hypothesized road charge, [c/veh-mi]
      # Variable vehicle travel cost per pass-mi [c/pass-mi] - but need to augment by empty VMT costs?
      C_Vpmt = (C_V + C_road) * PoolTourContraction, 
      C_Tpmt = C_T * PoolTimeCostInflation * RideDetourExpansion, 
      ...
      # Adjust pace (travel time per mile) for automated travel cases, if ReductPaceAutomation factor != 1.0
      Pi = 1 / TravTimeCostBasePars["LDV.average", "speed"], # assumed pace, (constant over o) [hr/veh-mi]
      Pi = ifelse(DemScen != "Base", Pi * ReductPaceAutomation, Pi), # current applies to ALL vehicle travel segments
      # C_Tdph = C_T / (100 * Pi) # Hourly Time-cost per passenger-hour (for info only) [$/pass-hr]
    ) %>%
    # relative PMT and VMT (unitless, ref = 1.0 for Base scen, occupancy=1)
    group_by(DemScen) %>% # should group by CombScen
    mutate( # calc PMT, VMT, and Energy multipliers (ratios)
      # Costs include Sharing disutility through Elas_VoTT_wrt_Pooling
      # ToDO: Account for deadheading/respositioning miles in ref (No Pooling) case
      C_Mratio = (C_Mpmt / C_Mpmt[o == 1.0 & C_road == 0.0]), # this is the grouped operation, getting the ratio of cost to base
      PMT = VMT_oref * (C_Mratio)^ElasPKT, #  pass-mi (normalized w.r.t Base scenario, base Occupancy)
      # PMT = VMT_oref * (C_Mpmt / C_Mpmt[o == 1.0])^ElasPKT, #   pass-mi (normalized w.r.t Base scenario, base Occupancy)
      VMT = PMT * PoolTourContraction, #  veh-mi (normalized) (defining relation for Occupancy "o", but only true for o=PoolingLevel if perfect routing efficiency)
      # These experiments with occupancy do not alter vehicle energy intensity (which should be Energy_oref/VMT_oref)
      ...
      # VMT_norebound = PMT[o == 1.0 & C_road == 0.0] / o #   veh-mi (normalized) "direct" effect of occupancy, w/o cost responses
      # Update key variables reported and used in aggregation to VC level results
      # NIE unchanged by pooling
      fracVMTIncr = VMT - 1.0,
      EnergyUse = Energy - 1.0
    ) %>% # but does include the effect of the Alt case on cost and PMT
    ungroup()
    # returns transformed df
} # end fn calcPoolingEnergyCases
```

```{r defFn_calcPoolingEnergyFAUCases, echo=echoWorking}
# same as calcPoolingEnergyCases() except grouping calc also by F,A,U (Fuel, Automation, Use)
calcPoolingEnergyFAUCases <- function(dfVMT, # this case df must be a set of scenarios with also: C_road, o
                                   curr_Elas_VoTT_wrt_Pooling,
                                   curr_Elas_TourLength_wrt_Pooling,
                                   curr_Elas_RiderDetour_wrt_Pooling
                                   ) {
  ...
  dfVMT <- dfVMT %>%
    mutate(
      ...
      # C_road must be specified   # hypothesized road charge, [c/veh-mi]
      # Variable vehicle travel cost per pass-mi [c/pass-mi] - but need to augment by empty VMT costs?
      C_Vpmt = (C_V + C_road) * PoolTourContraction, # C_road applies to all road travel segments (shares still exog)
      C_Tpmt = C_T * PoolTimeCostInflation * RideDetourExpansion, 
      # Time-cost per passenger mile adjusted by the disutility of occupancy [c/pass-mi] and 
      #   passenger time cost per mile rises with detour time (RideDetourExpansion)
      C_Mpmt = C_Vpmt + C_Tpmt, # Total cost per passenger mile traveled  [c/pass-mi]
    ...
    # relative PMT and VMT (unitless, ref = 1.0 for Base scen, occupancy=1)
    group_by(DemScen, VC, ElasCase, Fuel, Automation, Use) %>% # should group by CombScen combining DemScen, VC, ElasCase
    mutate( # calc PMT, VMT, and Energy multipliers (ratios)
      # Costs include Sharing disutility through Elas_VoTT_wrt_Pooling
      # ToDO: Account for deadheading/respositioning miles in ref (No Pooling) case
      C_Mratio = (C_Mpmt / C_Mpmt[o == 1.0 & C_road == 0.0]), # this is the grouped operation, getting the ratio of cost to base
    ...
    ungroup()
    # returns transformed df
} # end fn calcPoolingEnergyFAUCases
```
...
occupancyEffects1 <- occupancyEffects1 %>%
  ...
  calcPoolingEnergyCases(# this case df must be a set of scenarios with also: C_road, o
    curr_Elas_VoTT_wrt_Pooling = Elas_VoTT_wrt_Pooling,
    curr_Elas_TourLength_wrt_Pooling = Elas_TourLength_wrt_Pooling,
    curr_Elas_RiderDetour_wrt_Pooling = Elas_RiderDetour_wrt_Pooling
  )

```

```{r plotOccupancyEffects1, echo=echoWorking, dpi=300}
plotOccupancyEffects(occupancyEffects1,
                     curr_title = "Effect of Pooling on PMT and Energy (VMT) - For Ride Hailing",
                     curr_subtitle = "Partial routing efficiency, small pooling disutility, no road charge. Demand rebound to cost.")
```


```{r defFn_get_legend, echo=echoWorking}
# for extracting a figure legend and recomposing it on a combined graph
#  See [ggplot2 - Easy way to mix multiple graphs on the same page](http://www.sthda.com/english/wiki/wiki.php?id_contents=7930)
...
```{r dualOccupancyPlot, echo=echoWorking, dpi=300, fig.width=7, fig.height=3.0}
# grid.arrange() [in gridExtra]:

library("gridExtra")
plt1 = plotOccupancyEffects(occupancyEffects,
                     curr_title = "Effect of Pooling on PMT and Energy",
                     curr_subtitle = "Ideal routing efficiency, no discomfort")
plt2 = plotOccupancyEffects(occupancyEffects1,
                     curr_title = "Effect of Pooling on PMT and Energy",
                     curr_subtitle = "Partial routing efficiency, small discomfort.")

#grid.arrange(plt1 + theme(legend.position="none"), plt2, # + rremove("x.text"), 
#grid.arrange(plt1, plt2, # + rremove("x.text"), ncol = 2, nrow = 1)

legend <- get_legend(plt1)
# 3. Remove the legend from the box plot
# 4. Arrange ggplot2 graphs with a specific width
grid.arrange(plt1 + theme(legend.position="none"),
             plt2 + theme(legend.position="none"), 
             legend, # + rremove("x.text"), 
             ncol = 3, nrow = 1, 
             widths=c(3.2, 3.2, 0.8))

```
In these figures, PMT demand responds to the (full effective) cost of pooled ride-hailing

### 6.3.2 Numerical results for a range of Occupancy, Road Charge ($/VMT) and Route-length Elasticity

Time-cost is multiplied by a factor reflecting the disutility of pooling [unitless]:

$$m_{CT,o} = o^{\epsilon_{VoTT, o}}$$

- $m_{CT,o}$ = `PoolTimeCostInflation`
- $\epsilon_{VoTT, o}$ = `Elas_VoTT_wrt_Pooling`

Ride-hail vehicle tour length (VKT) contracts with the extent of pooling. 
The assumption of monotonic contraction with $o$ is consistent with the idea that pooling will only occur to the degree that it reduced VKT.

$$m_{KV,o} = o^{\epsilon_{KV, o}}$$
where

- $m_{KV,o}$ = `PoolTourContraction`
- $\epsilon_{KV, o}$ = `Elas_TourLength_wrt_Pooling`

Similarly, pooling of ride-hail vehicle trips leads to some expansion
of passenger trip length (measured in time or distance, assuming no change
in average vehicle speed with pooling, apart from pickup and drop-off
transaction times).

$$m_{KP,o} = o^{\epsilon_{KP, o}}$$

where

- $m_{KP,o}$ = `RideDetourExpansion`
- $\epsilon_{KP, o}$ = `Elas_RiderDetour_wrt_Pooling`
...
AltVMT1 <- calcPoolingEnergyCases(AltVMT1,
...
AltVMT1 %>%
  ...
  # select(-DemScen, -VMT, -PMT, -VMT_oref, -Energy_oref, -PoolTimeCostInflation, -PoolTourContraction, -RideDetourExpansion) %>%
...
Plot Energy vs Road Charge, for various Pooling levels

```{r defFN_plotRelEnergyvsC_road, echo=echoWorking}
plotRelEnergyvsC_road <- function(
    df,
    curr_title = "Energy Use with Pooling and Road Charge",
    curr_subtitle = "",
    curr_ylimmin = NA, # leave as NA to compute from data, or override
    curr_ylimmax = 1.1
  ) {
  baseenergy <- df %>%
  filter(
  ...
  my_colors = RColorBrewer::brewer.pal(n = 3, name = "Spectral")
  ...
  df %>%
    ...
    ggtitle(curr_title,
            subtitle = curr_subtitle) +
    xlab("Incremental road charge (c/veh-mi)") +
    ylab("Relative Energy Use (1.0 = base)") +
    ylim(curr_ylimmin, curr_ylimmax) +  
    # annotate("text", x = 1.0, y = 25, label = "Some text") +
    geom_rect(xmin = -1.0, xmax = 1.0, ymin = 0.0, ymax = baseenergy, color = "black", fill = my_colors[3], alpha = 0.5) +
...
```{r defFN_plotfracEnergyChangevsC_road, echo=echoWorking}
plotfracEnergyChangevsC_road <- function(
    df,
    curr_title = "Energy with Pooling & Road Charge",
    curr_subtitle = "",
    curr_ylimmin = NA, # leave as NA to compute from data, or override
    curr_ylimmax = 0.1
  ) {
  ...
  baseEnergyChange = baseenergy - 1.0
  
  my_colors = RColorBrewer::brewer.pal(n = 3, name = "Spectral")

  df %>%
    mutate(Energy = Energy - 1.0) %>% #  change form relative energy use to fractional change in energy from Base
    ggplot(aes(x = C_road, y = Energy)) +
    geom_line(aes(color = Pooling), size = 1) +
    scale_color_brewer(type = "seq", palette = "Blues") + # setting color of lines?
    ggtitle(curr_title,
            subtitle = curr_subtitle) +
    xlab("Incremental road charge (c/veh-mi)") +
    ylab("Change in Energy Use (0.0 = base)") +
    ylim(curr_ylimmin, curr_ylimmax) +  
    # annotate("text", x = 1.0, y = 25, label = "Some text") +
    geom_rect(xmin = -1.0, xmax = 1.0, 
              ymin = min(0.0, baseEnergyChange), ymax = max(0.0, baseEnergyChange),
              color = "black", fill = my_colors[3], alpha = 0.5) +
    geom_hline(yintercept = 0.0, color = "black") +
    # annotate("pointrange", x = 0.0, y = baseEnergyChange, ymin = baseEnergyChange, ymax = baseEnergyChange, colour = "black", size = .50) +
    theme(
      legend.position = c(0.80, 0.75),
      axis.text = element_text(size = 14),
      axis.title = element_text(size = 14, face = "bold"),
...
prepAltVMT2 %>% 
  plotRelEnergyvsC_road(curr_subtitle = "Scenario C-High, Ride-Hailing Vehicles")
# -> print()
```

```{r}
prepAltVMT2 %>% 
  plotfracEnergyChangevsC_road(curr_subtitle = "Scenario C-High, Ride-Hailing Vehicles")
```

## 6.4 Repeat Analysis with Scenarios Disaggregated by Vehicle Subtypes

### 6.4.1 Development of Functions for Analysis Disagg by Vehicle Subtypes with Occupancy and Incentives


#### Review of Prior Data Transformations

    Sec. 5.0 Logical Sequence
    
    # create df of all case combos, assumptions and results, aggregated (at VC level)
    for (CurrTechScen in techScensWanted) {rbind  CombScenInputsAndResultsForOneTechScen(CurrTechScen, CurrYear, DetailedDemScens))} -> CaseAssumpsResults_many
      3 DemScen * 9 TechScen * 2 VC * 2 ElasCase = 108
    
    Sec. 6.0 Logical Sequence is:

    # disaggregate the comb scenarios to the vehicle subcat (FAU) level
    CaseAssumpsResults_many %>% disaggCombScenResults(DetailedDemScens, disaggVoTT = F) %>% CaseAssumpsResults
      (3 DemScen * 9 TechScen * 2 VC * 2 ElasCase = 108)*(3 A * 3 F * 2 U) = 1944 subcat result rows
    CaseAssumpsResults %>% right_join(CombScen_Combinations_subset,xxx) %>% WantedCaseAssumpsResults
      (3 DemScen-TechScen * 2 VC * 1 ElasCase) * (3 A * 3 F * 2 U) = 108 disagg result rows
    CaseAssumpsResults %>% aggregateSelectedCaseAssumpsResults(CombScen_xxx) %>% AggAssumpsResults
      (3 DemScen-TechScen * 2 VC * 1 ElasCase) = 6 agg result rows
    AggAssumpsResults %>% (filter DemScenWanted,ElasCaseWanted, VehClassWanted) %>% scenAggData
      (3 DemScen * 1 VC * 1 ElasCase) = 3 agg result rows
      
    WantedCaseAssumpsResults %>% filter(DemScenWanted, ElasCaseWanted, VehClassWanted) %>% scenDisaggData
      (3 DemScen * 1 VC * 1 ElasCase) * (3 A * 3 F * 2 U) = 54 disagg result rows
    
    # same thing with Disagg VoTT
    CaseAssumpsResults_many %>% disaggCombScenResults(DetailedDemScens, disaggVoTT = T) %>% CaseAssumpsResultsDAVT 
    CaseAssumpsResultsDAVT %>% aggregateSelectedCaseAssumpsResults(CombScen_xxx) %>% AggAssumpsResultsDAVT
    AggAssumpsResultsDAVT %>% {filter DemScenWanted,ElasCaseWanted, VehClassWanted} %>% scenAggDataDAVT

    C_tot_base <- extractVehTotalTravelCost(VTCostBase, VehTypeWanted, "Total" ) # total base travel cost, c/mi
    
    scenAggData %>% {create/mutate C_F, C_T, C_tot, C_H, C_V} %>% scenAggData
    scenDisaggData %>% {create/mutate C_F, C_T, C_tot, C_H, C_V} %>% scenDisaggData
    
    scenAggDataDAVT %>% {create/mutate C_F, C_T, C_tot, C_H, C_V} %>% scenAggDataDAVT
    scenDisaggDataDAVT %>% {create/mutate C_F, C_T, C_tot, C_H, C_V} %>% scenDisaggDataDAVT


    Sec. 6.3 Logical Sequence is:
    
    scenAggData %>% select DemScen and a few other vars, crossing with values of `o` define ElasPKT, NIE_oref, VMT_oref, Energy_oref} %>% occupancyEffects1
      # could to for disagg results, but note occupancy only has impact for Use = Shared
    occupancyEffects1 %>% {set C_road} call calcPoolingEnergyCases()

    scenAggData %>% select DemScen and a few other vars, crossing with values of `o` define ElasPKT, NIE_oref, VMT_oref, Energy_oref} %>% occupancyEffects1
    occupancyEffects1 %>% select variables from occupancyEffects (dropping TechScen, VC, F, A, U) add `C_road` (single val or poss multiple values, crossing with cases) %>% AltVMT1
    AltVMT1 %>% calcPoolingEnergyCases() for specifed Elas_xxx_wrt_Pooling %>% AltVMT1
    AltVMT1 %>% select single scenario %>% ggplot("Energy Decrease with Pooling and Road Charge")

    calcPoolingEnergyCases():
      calc C_Vpmt, C_Tpmt, C_Mpmt, Pi for various `o` given Elax_xxx_wrt_Pooling}} 
      group_by(DemScen) {calc PMT, VMT, Energy multipliers for various levels of `o`} ungroup


```{r defFn_aggregateSelectedCaseAssumpsResultsByOAndCroad, echo = echoWorking}
# same as aggregateSelectedCaseAssumpsResults() except final weighted ave also grouped by `o` and `C_road`
aggregateSelectedCaseAssumpsResultsByOAndCroad <- function(case_assumps_results, # df with precalc assumps & results
                                                comb_subset, # df case subset to be aggregated (CombScen, Year, ElasCase, VC, TechScen, DemScen)
                                                # optionally can call for particular vehicle sub-cat
                                                use_set = c("Private", "Shared"),
                                                fuel_set = c("Gas", "PHEV", "BEV"),
                                                auto_set = c("No", "Partial", "Full"),
                                                grouping_var_list = # list of variables that identify individual cases/scenarios
                                                  list("DemScen", "TechScen", "ElasCase", "VC", "Year", "Pooled_o", "C_road")
                                                ) {
  # Aggregate Case assumps and results over all vehicle sub-types (Automation, Fuel, and Use), by share-weights
  # Returns df with 1 row per Scenario, 6 Scenario selection/ID variables, and 18 intermed and final results vars.
  #   Can be used to determine aggregates for specified subsets of A, F, U.
  aggregateSelectedCaseAssumpsResults(case_assumps_results, # df with precalc assumps & results
                                                comb_subset, # df case subset to be aggregated (CombScen, Year, ElasCase, VC, TechScen, DemScen)
                                                # optionally can call for particular vehicle sub-cat
                                                use_set = use_set,
                                                fuel_set = fuel_set,
                                                auto_set = auto_set,
                                                grouping_var_list = grouping_var_list # vars that ID cases/scenarios
  )
}
```


```{r calcDisaggResultsByIncentiveElasAndOccupancy, echo=echoWorking}
  C_tot_base <- extractVehTotalTravelCost(VTCostBase, VehTypeWanted, "Total" ) # total base travel cost, c/mi (scalar)

  # disaggregate the comb scenarios to the vehicle subcat (FAU) level
  CaseAssumpsResults_many %>% 
  disaggCombScenResults(DetailedDemScens, disaggVoTT = global_disaggVoTT) %>% 
  # return() -> DisaggAssumpsResults #(3 DemScen * 9 TechScen * 2 VC * 2 ElasCase = 108)*(3 A * 3 F * 2 U) = 1944 subcat result rows
  # keep all rows in CaseAssumps that match row in CombScen_Combinations_subset (select desired subset)
  right_join(CombScen_Combinations_subset, by = c("Year", "ElasCase", "VC", "TechScen", "DemScen")) %>%
  # return() -> WantedDisaggAssumpsResults #  (3 DemScen-TechScen * 2 VC * 1 ElasCase) * (3 A * 3 F * 2 U) = 108 disagg result rows
  filter( # further restrict cases of interest
    DemScen %in% DemScenWanted,
    ElasCase == ElasCaseWanted,
    # Fuel == "Gas", # multiple vehicles and techs possible
    # VehType == VehTypeWanted
  ...
  # return() -> scenDisaggData # (3 DemScen * 1 VC * 1 ElasCase) * (3 A * 3 F * 2 U) = 54 disagg result rows
  addMainCostsPerVehMi(C_tot_base) %>% # create/mutate C_F, C_T, C_tot, C_H, C_V
  return() -> scenDisaggData

    # select DemScen and a few other vars, crossing with values of occupancy `o`
  scenDisaggData %>%
    select(DemScen, C_F, C_H, C_T, C_V, fracVMTIncr, NIE, EnergyUse, ElasVKT, everything()) %>% # reorder vars
    crossing(data.frame("Pooled_o" = seq(from = 0.8, to = 1.5, by = 0.05))) %>% #  occupancy, unitless (pass/veh, normalized)
    # card(DetailedDemScens) * card(ElasCaseWanted) * card(VehClassWanted) = (3 DemScen-TechScen) * 18 segments FAU * (15 o levels)
    # define ElasPKT, NIE_oref, VMT_oref, Energy_oref
    # set up reference and pooled outcome variables
    ungroup() %>%
    mutate( # convert from fractional change to multipliers
      ElasPKT = ElasVKT, # treat as the same, since ElasVKT specified for base, non-pooled travel
    ...
    mutate( # for case Pooled_o, actual occupancy o by vehicle type only varies for Shared/Ride-hailed travel
      o = ifelse(Use != "Shared", 1.0, Pooled_o)
    ) %>%
    return() -> occupancyEffectsDisagg
  # TravTimeCostBasePars["LDV.average", "speed"]

occupancyEffectsDisagg_XCroad <- occupancyEffectsDisagg %>% # examine combinations of o and C_road
#  mutate(
#    C_road = 0.0,     # hypothesized road charge, [c/veh-mi]. Here we could also cross by multiple C_road values
#  ) %>%
  # Could also establish values of ElasTourLengthWithPooling to be evaluated
  crossing(data.frame("C_road" = seq(from = -10.0, to = 20.0, by = 5.0))) %>% # Look at different road charges
  # Calculates PMT, VMT, and Energy relative outcomes (multipliers) based on occupancy o and C_road for each case
  calcPoolingEnergyFAUCases(# this case df must be a set of scenarios with also: C_road, o
    curr_Elas_VoTT_wrt_Pooling = Elas_VoTT_wrt_Pooling,
    curr_Elas_TourLength_wrt_Pooling = Elas_TourLength_wrt_Pooling,
    curr_Elas_RiderDetour_wrt_Pooling = Elas_RiderDetour_wrt_Pooling
  )

occupancyEffectsDisagg_0Croad <- occupancyEffectsDisagg_XCroad %>%
  filter(
    C_road == 0.0,     # hypothesized road charge, [c/veh-mi]. Here we could also cross by multiple C_road values
  )
```

### 6.4.2 Consider a Variety of Vehicle Travel Segments and Calculate Energy Intensity, VMT, PMT, and Energy Impacts

```{r defFn_plotEnergyVsPoolingForScenario, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
# cols <- brewer.pal(3, "BuGn")
# those three colors make up initial palette.
# Then pass them to colorRampPalette() to create interpolating function.
# pal <- colorRampPalette(cols)
...
plotEnergyVsPoolingForScenario <- function(aggAR) {
  aggAR %>%
    # arrange(o, DemScen) %>%
    # filter(C_road %in% c(-10, 0, 10, 20)) %>%
    group_by(DemScen) %>%
    ggplot(aes(x = Pooled_o, y = EnergyUse)) +
    geom_line(aes(color = DemScen), size = 1) +
    # scale_color_brewer(type = "seq", palette = "Blues") + # setting color of lines?
    ggtitle("Energy Decrease with Pooling by Scenario (No Road Charge)") +
    xlab("Ride-hailing Occupancy/Pooling Level (1 = base)") +
    ylab("Relative Total Energy Use (1.0 = base)") +
    ylim(-0.75, 0.75) +
    # annotate("text", x = 1.0, y = 25, label = "Some text") +
    # annotate("pointrange", x = 1.0, y = baseenergy, ymin = baseenergy, ymax = baseenergy, colour = "black", size = .50) +
...
```{r testCasesForSelectedSegments0A, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
  # Aggregate Case assumps and results over all vehicle sub-types (Automation, Fuel, and Use), by share-weights
  # Returns df with 1 row per Scenario, 6 Scenario selection/ID variables, and 18 intermed and final results vars
  #   CaseAssumpsResults %>% aggregateSelectedCaseAssumpsResults(CombScen_xxx) %>% AggAssumpsResults
  #   # (3 DemScen-TechScen * 2 VC * 1 ElasCase) = 6 agg result rows
occupancyEffectsDisagg_0Croad %>% 
  aggregateSelectedCaseAssumpsResultsByOAndCroad(
    select(CombScen_Combinations_subset, -Notes),
    auto_set = c("No", "Partial", "Full"),
    fuel_set = c("Gas", "PHEV", "BEV"),
    use_set = c("Private", "Shared")
  ) -> AggAssumpsResultsAll2
...
AggAssumpsResultsAll2 %>%
  # filter(DemScen == test_DemScen) %>%
  select(Pooled_o, DemScen, EnergyUse) %>% # C_road
  spread(key = DemScen, value = EnergyUse) %>%
  kable(digits = 4, caption = paste0("Aggregated Energy Change Relative to Reference, by Occupancy and Scenario"))


# XXX

# 
#   occupancyEffects2 %>% # select variables (dropping TechScen, VC, F, A, U) add `C_road` (single val or poss multiple values, crossing with cases)
#     select(DemScen, o, C_V, C_T, VMT_oref, Energy_oref, ElasPKT, VMT, PMT, Energy) %>%
#     # establish values of ElasTourLengthWithPooling to be evaluated
#     crossing(data.frame("C_road" = seq(from = -10.0, to = 20.0, by = 5.0))) %>% # Look at different road charges
#   return() -> AltVMT2
#   
#   calcPoolingEnergyFAUCases(AltVMT2,
#                          Elas_VoTT_wrt_Pooling,
#                          Elas_TourLength_wrt_Pooling,
#                          Elas_RiderDetour_wrt_Pooling) %>%
#   return() -> AltVMT2
...
#  calcPoolingEnergyCases():
#    calc C_Vpmt, C_Tpmt, C_Mpmt, Pi for various `o` given Elax_xxx_wrt_Pooling}} 
#    group_by(DemScen, VC, ElasCase) {calc PMT, VMT, Energy multipliers for various levels of `o`} ungroup

```
    
```{r}
AggAssumpsResultsAll2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "LDV Mix, Shared mobility Includes Ride-Pooling")
```

LDV Mix, Shared mobility Includes Ride-Pooling
```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsAll2)
```

#### Results for Fully-Automated Private Vehicles, Gasoline drivetrain:

```{r testCasesForSelectedSegments1A, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
occupancyEffectsDisagg_0Croad %>% 
  aggregateSelectedCaseAssumpsResultsByOAndCroad(
    select(CombScen_Combinations_subset, -Notes),
    # auto_set = c("No", "Partial", "Full"),
  ...
  return() -> AggAssumpsResultsPrivateGasCAV2

AggAssumpsResultsPrivateGasCAV2 %>%
  # filter(DemScen == test_DemScen) %>%
  select(Pooled_o, DemScen, EnergyUse) %>% # C_road
  spread(key = DemScen, value = EnergyUse) %>%
  kable(digits = 4, caption = paste0("Private Gas CAV: Aggregated Energy Change Relative to Ref, by Occupancy and Scenario"))
...
AggAssumpsResultsPrivateGasCAV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Private Gas CAV, Ride-hailing Includes Ride-Pooling")
```

Private Gas CAV, Ride-hailing Includes Ride-Pooling
```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsPrivateGasCAV2)
```

#### Results for Electrified Vehicles, BEV or PHEV drivetrain:

```{r testCasesForSelectedSegments2A, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
occupancyEffectsDisagg_0Croad %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  select(CombScen_Combinations_subset, -Notes),
  #auto_set = c("Full"),
  fuel_set = c("PHEV", "BEV"),
  #use_set = c("Private")
  ) %>%
  return() -> AggAssumpsResultsEVs2 
```

```{r}
AggAssumpsResultsEVs2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "LDV BEVs and PHEVs, Includes Pooled Ride-hailing")
```

LDV BEVs and PHEVs, Includes Pooled Ride-hailing
```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsEVs2)
```

#### Results for Shared Vehicles, All control and drivetrains:

```{r testCasesForSelectedSegments3A, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
occupancyEffectsDisagg_0Croad %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  # fuel_set = c("Gas", "PHEV", "BEV"),
  ...
  return() -> AggAssumpsResultsShared2
```

```{r}
AggAssumpsResultsShared2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Ridehailing Only, Includes Pooled Ride-hailing")
```

Shared Use (Ride-hailing), Includes Pooled Ride-hailing

```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsShared2)
```

#### Results for Private Use, (All Automation levels, All drivetrains):

```{r testCasesForSelectedSegments3B, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
occupancyEffectsDisagg_0Croad %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  # fuel_set = c("Gas", "PHEV", "BEV"),
  ...
  return() -> AggAssumpsResultsPrivateAll2
```

```{r}
AggAssumpsResultsPrivateAll2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Private Use, Includes Pooled Ride-hailing")
```

Private Use, (Includes Pooled Ride-hailing but Not Applicable)

```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsPrivateAll2)
```

#### Results for Private Non-Automated Vehicles, All drivetrains:

```{r testCasesForSelectedSegments4A, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
occupancyEffectsDisagg_0Croad %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  auto_set = c("No"),
  ...
  return() -> AggAssumpsResultsPrivateMV2
```


```{r}
AggAssumpsResultsPrivateMV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Private, Non-Automated Vehicles, Includes Pooled Ride-hailing")
```

Private, Non-Automated Vehicles, Includes Pooled Ride-hailing
```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsPrivateMV2)
```

#### Results for Private, Gas. Non-Automated Vehicles:

```{r testCasesForSelectedSegments5A, echo=echoWorking, dpi=300, fig.width=7, fig.height=7}
occupancyEffectsDisagg_0Croad %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  select(CombScen_Combinations_subset, -Notes),
  # auto_set = c("No", "Partial", "Full")
  auto_set = c("No"),
  ...
  return() -> AggAssumpsResultsPrivateGasMV2
```

```{r}
AggAssumpsResultsPrivateGasMV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Private, Gas. Non-Automated Vehicles, Includes Pooled Ride-hailing")
```

Private, Gas. Non-Automated Vehicles, Includes Pooled Ride-hailing
```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsPrivateGasMV2)
```


```{r examineNoPoolingNoRoadChargeCases}
occupancyEffectsDisagg_0Croad %>% 
  ...
  return() -> occupancyEffectsDisagg_1o_0Croad

occupancyEffectsDisagg_1o_0Croad %>%
  mutate(Share = F_A_Shares*U_Shares) %>%
  select(CombScen, VC, Pooled_o, Automation, Fuel, Use, fracVMTIncr, NIE, EnergyUse, Share) %>%
  arrange(CombScen, VC, Automation, Fuel, Use)

# %>%
#   group_by(CombScen) %>%
#   summarize(TotShare = sum(Share)) # checksum confirms that shares sum to 1.0
# names(occupancyEffectsDisagg_1o_0Croad)
```

## 6.5 Plot Energy Use Change vs. Road Charge, for Different Pooling Levels


```{r}
test_DemScen = "C_High"

occupancyEffectsDisagg_XCroad %>% 
  aggregateSelectedCaseAssumpsResultsByOAndCroad(
    select(CombScen_Combinations_subset, -Notes),
    auto_set = c("No", "Partial", "Full"),
    fuel_set = c("Gas", "PHEV", "BEV"),
    use_set = c("Private", "Shared")
  ) %>% # -> AggAssumpsResultsAll  
  filter(DemScen == test_DemScen) %>%
  rename(Pooling = Pooled_o) %>%
  arrange(C_road, Pooling) %>%
  # filter(C_road %in% c(-10, 0, 10, 20)) %>%
  filter(Pooling %in% c(0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.5)) %>%
  # mutate(C_road = as.factor(C_road)) ->
  mutate(Pooling = as.factor(Pooling)) %>%
  mutate(Energy = 1+EnergyUse) %>%
  return() -> df_EvRc_C

  plt_rE_C <- df_EvRc_C %>%
    plotRelEnergyvsC_road(curr_subtitle = paste0("Scenario ", test_DemScen, ", Mix of All LDV Vehicles"),
                          curr_ylimmin = 0.7, curr_ylimmax = 1.1)
  plt_fE_C <- df_EvRc_C %>%
    plotfracEnergyChangevsC_road(curr_subtitle = paste0("Scenario ", test_DemScen, ", Mix of All LDV Vehicles"),
                          curr_ylimmin = -0.3, curr_ylimmax = 0.2)
...
```{r plotRelEnergyDual, echo=echoWorking, dpi=300, fig.width=7, fig.height=5}
# https://wilkelab.org/cowplot/articles/aligning_plots.html
# see https://github.com/thomasp85/patchwork
(plt_rE_B + plt_rE_C) +
  plot_annotation(
    title = 'Relative Energy Use',
    subtitle = 'Variations on the central outcomes for two scenarios which have different levels of Ride-hail travel.',
    caption = 'CAVESim results'
  )
```


```{r plotfracEnergyChangesDual, echo=echoWorking, dpi=300, fig.width=8, fig.height=5}
(plt_fE_B + plt_fE_C) +
  plot_annotation(
    title = 'Fractional Change in Energy Use',
    subtitle = 'Variations on the central outcomes for two scenarios which have different levels of Ride-hail travel.',
    caption = 'CAVESim results'
  )
```
