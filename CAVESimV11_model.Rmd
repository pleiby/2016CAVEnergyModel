...
# Pivot for Zero to High RidePoolAdj case adjustments for all Use, Automation
RideHailingPoolingAdj <- refRideHailingPoolingAdj %>%
  gather(key = "RidePoolAdj", value = RidePoolAdjFrac, -c(Use, Automation)) 

# Initially, restrict focus to only Shared, Full Automation cases
temp_refRideHailingPoolingAdj <- RideHailingPoolingAdj %>%
...
#  Need to apply shares to this dataframe, by adding rows and columns for 
...
DetailedDemScens <- DetailedDemScens[!(DetailedDemScens %in% c("Zero", "A_Low", "A_High", "B_Low", "C_Low", "Test"))] # not of interest, Zero does what is says
# DetailedDemScens
```
...
} # fn appendDisaggParams

DisaggParams <- DemScenAdjustments %>%  # ***UNUSED***
  select(-Comments) %>%
...
$$\Delta I_{jtv, FAU} = \Delta I_{jtv} \cdot m_{I,Cx} \cdot m_{I,Ax} \cdot  m_{I,FAx} $$
