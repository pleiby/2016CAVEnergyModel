...
date: Rev. 2020-02-15, Printed `r Sys.Date()`
output:
  html_document:
    df_print: paged
    fig_caption: yes
...
Consider a range of average occupancy levels for ride-hailing vehicles, $o$, from
...
aggregateSelectedCaseAssumpsResultsByOAndCroad <- function(case_assumps_results, # df with precalc assumps & results
  ...
  case_assumps_results %>%
    ...
    filter(Fuel %in% fuel_set) %>%
    filter(Use %in% use_set) %>%
    group_by(DemScen, TechScen, ElasCase, VC, Year, o, C_road) %>%
    summarise_at(
      # construct vehicle share-weighted average of these vars for each "scenario"
      vars(
        fracVMTIncr, NIE, EnergyUse,
...
AggAssumpsResultsAll2 %>%
  # filter(DemScen == test_DemScen) %>%
  select(o, DemScen, EnergyUse) %>% # C_road
  # select(-DemScen, -VMT, -PMT, -VMT_oref, -Energy_oref, -PoolTimeCostInflation, -PoolTourContraction, -RideDetourExpansion) %>%
  # select(-C_V, -C_T, -C_Vpmt, -C_Tpmt, -C_Mpmt, -Pi, -ElasPKT, -C_Mratio) %>%
...
AggAssumpsResultsPrivateGasCAV2 %>%
  ...
  tableScenarioSetIntVMTEnergy(tablecaption = "LDV Mix, Shared mobility Excludes Ride-Pooling")
```
...
AggAssumpsResultsPrivateGasCAV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergy(tablecaption = "Private Gas CAV, Shared mobility Excludes Ride-Pooling")
```
...
AggAssumpsResultsPrivateCAV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergy(tablecaption = "LDV Private Gaso CAVs")
```
...
AggAssumpsResultsEVs2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergy(tablecaption = "LDV BEVs and PHEVs, Excludes Ride-Pooling")
```
...
AggAssumpsResultsShared2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergy(tablecaption = "Shared Use, Excludes Ride-Pooling")
```
...
AggAssumpsResultsPrivateMV %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergy(tablecaption = "Private, Non-Automated Vehicles")
```
...
AggAssumpsResultsPrivateGasMV %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergy(tablecaption = "Private, Gas. Non-Automated Vehicles")
```
