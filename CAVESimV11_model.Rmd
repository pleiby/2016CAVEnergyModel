...
appendDisaggParams <- function(DemScenData) {
  ...
  mutate(
    ...
    # Fraction of estimated Time Cost Effect gained by Partial or No Automation
...
```{r postProcessCombScenarioResults1, echo=echoWorking, warning=F}
...
Then compute disaggregated Intensity, VMT Demand and Energy Use impact
...
disaggCombScenResults <- function(dfCAR, disaggVoTT = T) {
  dfCAR %>% # expect input df with case results aggregated as the level of VC, rather than vehicel-subtype (A, F, U)
  mutate(
...
CaseAssumpsResults <- disaggCombScenResults(CaseAssumpsResults, disaggVoTT = F)
```
