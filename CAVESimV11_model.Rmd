...
$$\Delta I_{jtv, FAU} = \Delta I_{jtv, FAU} \cdot m_{I,Cx} \cdot m_{I,Ax} \cdot  m_{I,FAx} $$
...
$$\Delta K_{x,FAU} = (1 + C_{Total,x,FAU}^{E_x}) m_{D,x,US} * m_{D,x,U} - 1.0$$
...
temp <- CaseAssumpsResults %>%
  mutate(
    ...
    VMTEratio = # relative adj to agg change in energy intensity, treating numerical error as 0.0
...
#table(temp$VMTEratio, temp$Fuel)
#table(temp$NIE, temp$Use)
```

```{r echo=echoWorking}
hist(temp$VMTEratio, breaks = 20)
g <- temp %>%
  mutate(
    # Fuel = ordered(Fuel, levels = c("Gas", "BEV", "PHEV")),
    Automation = as.factor(Automation),
  ...
  ggplot(aes(VMTEratio)) + scale_fill_brewer(palette = "Spectral") + 
...
aggregateSelectedCaseAssumpsResults <- function(case_assumps_results, 
                                                comb_subset, # subset of cases to be aggregated
...
5.2 Final Scenario Results Table for 2050
------------------------
