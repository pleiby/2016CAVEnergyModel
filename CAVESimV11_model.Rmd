...
appendDisaggParams <- function(DemScenData) {
  ...
  #  of each vehicle category (Fuel, Automation, Use) for matching VC and DemScen)
  ...
  DemScenData %>%  # ***UNUSED***
  left_join(shares_by_F_A, by = c("VC", "DemScen")) %>% # add Fuel and Automation shares by VC and DemScen
  left_join(shares_by_U, by = c("VC", "DemScen")) %>% # add Use (Sharing, Hailing/Pooling) shares
  mutate(
...
CaseAssumpsResults <- disaggCombScenResults(CaseAssumpsResults_many, DetailedDemScens, disaggVoTT = F)

CaseAssumpsResultsDAVT <- disaggCombScenResults(CaseAssumpsResults_many, DetailedDemScens, disaggVoTT = T) # diaggregated time case
```
...
tableScenarioSetIntVMTEnergy(
  ...
  tablecaption = "Shared Use, Excludes Ride-Pooling")
```
...
```
In these figures, PKT demand responds to (full effective) cost of pooled ride-hailing
...
```{r calcDisaggResultsByIncentiveElasAndOccupancy, echo=echoWorking}
  ...
  # disaggregate the comb scenarios to the vehicle subcat (FAU) level
  CaseAssumpsResults_many %>% 
  disaggCombScenResults(DetailedDemScens, disaggVoTT = F) %>% 
...
AggAssumpsResultsShared2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Shared Use, Includes Pooled Ride-hailing")
```

Shared Use, Includes Pooled Ride-hailing
```{r}
