...
updateI_deltaCAVinDemRespParamsOld <- function(local_DemRespParams, local_nie) { # ***UNUSED***
  local_DemRespParams %>%
    gather(key = Sens, value = value, -c(VC, Parameter)) %>% # following assigns the same
    mutate(
      value = ifelse((VC == "LDV" & Parameter == "I_deltaCAV"), local_nie[local_nie$VC == "LDV", ]$NIE, value),
      value = ifelse((VC == "HDV" & Parameter == "I_deltaCAV"), local_nie[local_nie$VC == "HDV", ]$NIE, value)
    ) %>%
...
CaseAssumpsResults <- CaseAssumpsResults %>%
  mutate(
    ...
    # Unwind adjustment for Full CAV, and apply time-cost adjustment based level of Automation
    C_Time = old_C_Time*(1.0 + C_deltaVoTT*mCT_A)/(1.0 + C_deltaVoTT),
    # C_Veh = C_Fuel + C_VNonF, # vehicle capital and operating costs, c/mi (excl. time)
...
aggregateSelectedCaseAssumpsResults_Old <- function(case_assumps_results, # df with precalc assumps & results
  ...
  case_assumps_results %>%
    ...
    mutate(Joint_FAU_Share = F_A_Shares * U_Shares) %>%
    mutate( # convert all result vars to versions weighted by joint FAU share
      fracVMTIncr = fracVMTIncr * Joint_FAU_Share,
      NIE = NIE * Joint_FAU_Share,
      EnergyUse = EnergyUse * Joint_FAU_Share, # no longer already weighted by joint shares of veh sub-types
      C_Fuel = C_Fuel * Joint_FAU_Share,
      C_Time = C_Time * Joint_FAU_Share,
      C_VNonF = C_VNonF * Joint_FAU_Share,
      C_Total = C_Total * Joint_FAU_Share,
      ElasVKT = ElasVKT * Joint_FAU_Share,
      RidePoolAdjFrac = RidePoolAdjFrac * Joint_FAU_Share,
      USDmult = USDmult * Joint_FAU_Share,
      mI_A = mI_A * Joint_FAU_Share,
      mI_C = mI_C * Joint_FAU_Share,
      mI_FA = mI_FA * Joint_FAU_Share,
      mI_FB = mI_FB * Joint_FAU_Share,
      mD_U = mD_U * Joint_FAU_Share,
      mCT_A = mCT_A * Joint_FAU_Share
    ) %>%
    ...
    summarize(
      ...
      totshare = sum(Joint_FAU_Share), # needed if aggregating sub-segments
...
plotOccupancyEffects(occupancyEffects1,
                     curr_subtitle = "Partial routing efficiency, small sharing disutility, no road charge. Demand rebound to cost.")
```
...
plt2 = plotOccupancyEffects(occupancyEffects1,
                     curr_subtitle = "Partial routing efficiency, small sharing disutility. Demand rebound to cost.")
