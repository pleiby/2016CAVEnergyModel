...
AggAssumpsResultsPrivateGasCAV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "Private Gas CAV, Shared mobility Includes Ride-Pooling")
```
...
AggAssumpsResultsPrivateCAV2 <- aggregateSelectedCaseAssumpsResultsByOAndCroad(
  occupancyEffects2, 
  select(CombScen_Combinations_subset, -Notes),
  auto_set = c("Full"), # must call the aggregation fn to assure rescaled on share of these
  fuel_set = c("Gas"),
...
```{r}
AggAssumpsResultsPrivateCAV2 %>%
  mutate(CombScen = DemScen) %>%
  tableScenarioSetIntVMTEnergyVsO(tablecaption = "LDV Private Gaso CAVs, Includes Pooled Ride-hailing")
```

```{r}
plotEnergyVsPoolingForScenario(AggAssumpsResultsPrivateCAV2)
```
...
occupancyEffects2 %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  ...
  return() -> AggAssumpsResultsPrivateMV
...
occupancyEffects2 %>% aggregateSelectedCaseAssumpsResultsByOAndCroad(
  ...
  return() -> AggAssumpsResultsPrivateGasMV
