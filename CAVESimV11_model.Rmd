...
Note: a more refined analysis would account for the the different travel costs faced by ride-hailing/pooling travelers.
...
VMT demand is further adjusted base on exogenous demand shifts associated with new use categories and the efect of ride-hailing/ride-pooling on VMT.
...
CombScenInputsAndResultsForOneTechScen <- function(CurrTS, CurrYr, demScensWanted) {
  ...
  VTCShrAlt <- left_join(VTCShrAlt, select(DemScenCostChange, -Description), by = c("DemScen")) # currently _not_ Sensitivity Case-dependent, and not disagg by vehicle Automation level or Use case (sharing)
...
Also adjust costs accordingly, for correct demand response calculation.
...
$$\Delta K_{x,FAU} = (1 + R_{Total,x,FAU}^{E_x}) m_{D,x,US} * m_{D,x,U} - 1.0$$

Here $R$ is relative total cost compared to the base vehicle and $E_x$ is the elasticity of VKT
for scenario *x*.

$$R_{Total,x,FAU}(\Delta C_{Fuel}(\Delta I_{jtv, FAU}), \Delta C_{VNonFuel}, \Delta C_{Time}) = \frac {C_{Total,x,FAU}} {C_{Total,base}}$$

Energy Use impacts are based on the combined effect of intensity and VKT demand changes.
...
disaggCombScenResults <- function(dfCAR, disaggVoTT = T) {
  ...
  mutate(
    # adjust intensity change for Veh Class, Automation, Fuel tech
    old_NIE = NIE,
...
```{r defFnselectCaseAssumpsResults, echo=echoWorking}
