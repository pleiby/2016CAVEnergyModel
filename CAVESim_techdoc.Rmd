...
## Diaggregating Energy Intensity and Demand responses by subcategory
...
For full CAV penetration scenarios, without electrification, we computed fuel intensity relative to the base vehicle travel (average Region/Road)
$$\rho^I_{y,r,c,a,f,u} \equiv I_{y,r,c,a,f,u} /I_{c,base}$$
more specifically, the results are based on year and Class, and apply to a fully automated vehicle, gasoline drivetrain,  in average private (non-shared) use:
...
More generally, each vehicle's intensity will depend on the adjustments to the base for the vehicle type (Class, Fuel, Automation) and use case (Region). That is

$$I_{y,r,c,a,f,u}  = I_{y,base} \cdot (1+\Delta I_{r})(1+\Delta I_{c})(1+\Delta I_{a})(1+\Delta I_{f})(1+\Delta I_{u})$$
$$I_{y,r,c,a,f,u}  = I_{y,base} \cdot (\rho_{r})(\rho_{c})(\rho_{a})(\rho_{f})(\rho_{u})$$
although the intensity adjustment factors $\rho^I$ also depend to some degree on the vehicle/driving categories. 
