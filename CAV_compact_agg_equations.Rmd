...
We describe the combination of a technology-based decomposition of energy use and travel 
demand effects of vehicle automation with a utility-maximizing travel demand framework.
...
Rather than relying on an aggregate elasticity of travel demand with respect to generalized travel cost, we can take demand as the result of consumer or household utility maximization. 

$$max_{M,T,C,X} ~U = u((M,T,C, \bar G), X) - \phi(P(\bar F, \bar M)) - \delta(A(\bar M))$$
Utility is directly a function of VKT *M*, travel time *T*, generalized goods consumption *C*, and liesure *X*, as well as expogenous government goods $\bar G$. Consumer utilkity is also affected by damages from average pollution levels *P* and accident risk *A*.

In the traditional travel demand literature specification (e.g. Small and Verhauf) the maximization is subject a constraint on total time $\underline L$ available to the consumer and the consumer's budget constraint given income $I$. The two constraints are connected since income must be generated by time-consuming labor L.
...
\end {align}
$$
Technically, the separability of leisure time $u(M,T,C, \bar G,X) = u(\psi(M,T,C, \bar G), X)$ has certain implications that simplify analysis.

Functions for the production of VKT from fuel and other monetary expenditures on vehicles and travel link fuel use *F* and other travel financial costs *H* to VKT *M*.
